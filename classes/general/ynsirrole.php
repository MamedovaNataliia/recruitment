...
class YNSIRRole
{
	...
	static public function GetList($arOrder = Array('ID' => 'DESC'), $arFilter = Array())
	{
		...
		// Join
		$sJoin = '';
		if(isset($arFilter['ENTITY']) && strlen($arFilter['ENTITY']) > 0){
			$sJoin = ' INNER JOIN b_ynsir_role_perms RP ON RP.ROLE_ID=R.ID AND RP.ENTITY = \''.$DB->ForSql($arFilter['ENTITY']).'\' ';
		}
	...
	static public function GetRelation()
	{
		global $DB;
		// Join
		$sJoin = ' INNER JOIN b_ynsir_role_perms RP ON RP.ROLE_ID=RR.ROLE_ID AND RP.ENTITY = \''.$DB->ForSql($sEntity).'\' ';

		$sSql = '
			SELECT RR.* FROM b_ynsir_role R, b_ynsir_role_relation RR 
	...
	{
		global $DB;
		$sWhere = ' WHERE ROLE_ID IN(SELECT ROLE_ID FROM b_ynsir_role_perms WHERE ENTITY = "'.$sEntity.'")';
		$sSql = 'DELETE FROM b_ynsir_role_relation';
		...
		foreach ($arRelation as $sRel => $arRole)
		{
			foreach ($arRole as $iRoleID)
			{
				...
				$DB->Add('b_ynsir_role_relation', $arFields, array(), 'FILE: '.__FILE__.'<br /> LINE: '.__LINE__);
			}
		}
	...
	static public function GetRolePerms($ID)
	{
		global $DB;
		$ID = (int)$ID;
		$sSql = 'SELECT * FROM b_ynsir_role_perms WHERE role_id = '.$ID;
		$obRes = $DB->Query($sSql, false, 'FILE: '.__FILE__.'<br /> LINE: '.__LINE__);
		$_arResult = array();
	...
	static public function GetCalculateRolePermsByRelation($arRel)
	{
		...
		$sSql = "
			SELECT RP.*
			FROM b_ynsir_role_perms RP, b_ynsir_role_relation RR
	...
	static public function GetUserPerms($userID)
	{
		...
		$obRes = $DB->Query(
			"SELECT RP.* FROM b_ynsir_role_perms RP INNER JOIN b_ynsir_role_relation RR ON RR.ROLE_ID = RP.ROLE_ID INNER JOIN b_user_access UA ON UA.ACCESS_CODE = RR.RELATION AND UA.USER_ID = $userID",
			false,
	...
	public function Add(&$arFields)
	{
		...
		else
		{
			...
			$ID = (int)$DB->Add('b_ynsir_role', $arFields, array(), 'FILE: '.__FILE__.'<br /> LINE: '.__LINE__);
	...
	protected function SetRoleRelation($ID, $arRelation)
	{
		...
		$sSql = 'DELETE FROM b_ynsir_role_perms WHERE ROLE_ID = '.$ID;
		$DB->Query($sSql, false, 'FILE: '.__FILE__.'<br /> LINE: '.__LINE__);
				...
				foreach ($arFields as $sField => $arFieldValue)
				{
					if ($sField == '-')
					{
						...
						if ($arFieldValue != '-')
						{
							...
							$DB->Add('b_ynsir_role_perms', $arInsert, array(), 'FILE: '.__FILE__.'<br /> LINE: '.__LINE__);
						}
					}
					else
					{
	...
	public function Update($ID, &$arFields)
	{
		...
		else
		{
			if (!isset($arFields['RELATION']) || !is_array($arFields['RELATION']))
				$arFields['RELATION'] = array();
			$sUpdate = $DB->PrepareUpdate('b_ynsir_role', $arFields, 'FILE: '.__FILE__.'<br /> LINE: '.__LINE__);
			if (strlen($sUpdate) > 0)
				$DB->Query("UPDATE b_ynsir_role SET $sUpdate WHERE ID = $ID", false, 'FILE: '.__FILE__.'<br /> LINE: '.__LINE__);
	...
	public function Delete($ID)
	{
		global $DB;
		$ID = (int)$ID;
		$sSql = 'DELETE FROM b_ynsir_role_relation WHERE ROLE_ID = '.$ID;
		$DB->Query($sSql, false, 'FILE: '.__FILE__.'<br /> LINE: '.__LINE__);
		$sSql = 'DELETE FROM b_ynsir_role_perms WHERE ROLE_ID = '.$ID;
		$DB->Query($sSql, false, 'FILE: '.__FILE__.'<br /> LINE: '.__LINE__);
		$sSql = 'DELETE FROM b_ynsir_role WHERE ID = '.$ID;
		$DB->Query($sSql, false, 'FILE: '.__FILE__.'<br /> LINE: '.__LINE__);
	...
	public function CheckFields(&$arFields, $ID = false)
	{
		...
		if (($ID == false || isset($arFields['NAME'])) && empty($arFields['NAME']))
			$this->LAST_ERROR .= GetMessage('YNSIR_ERROR_FIELD_IS_MISSING', array('%FIELD_NAME%' => GetMessage('YNSIR_FIELD_NAME')))."<br />";
	...
	public static function EraseEntityPermissons($entity)
	{
		...
		$connection->queryExecute("DELETE FROM b_ynsir_role_perms WHERE ENTITY = '{$entity}'");
		self::ClearCache();
	}
}