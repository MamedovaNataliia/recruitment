...
class CYNSIRViewHelper
{
    ...
    public static function GetHiddenEntityCaption($entityTypeID)
    {
        if($entityTypeID === CCrmOwnerType::Company)
        {
            return GetMessage('YNSIR_CLIENT_SUMMARY_HIDDEN_COMPANY');
        }
        elseif($entityTypeID === CCrmOwnerType::Contact)
        {
            return GetMessage('YNSIR_CLIENT_SUMMARY_HIDDEN_CONTACT');
        }
        elseif($entityTypeID === CCrmOwnerType::Lead)
        {
            return GetMessage('YNSIR_CLIENT_SUMMARY_HIDDEN_LEAD');
        }
        elseif($entityTypeID === CCrmOwnerType::Deal)
        {
            return GetMessage('YNSIR_CLIENT_SUMMARY_HIDDEN_DEAL');
        }
        elseif($entityTypeID === CCrmOwnerType::Quote)
        {
            return GetMessage('YNSIR_CLIENT_SUMMARY_HIDDEN_QUOTE');
        }
        return GetMessage('YNSIR_CLIENT_SUMMARY_HIDDEN');
    }
    ...
    public static function RenderClientSummaryPanel($arParams, $arOptions = array())
    {
            ...
            if($name === '')
            {
                $name = GetMessage(
                    $entityTypeName === CCrmOwnerType::CompanyName
                        ? "YNSIR_CLIENT_SUMMARY_COMPANY_NOT_SPECIFIED"
                        : "YNSIR_CLIENT_SUMMARY_CONTACT_NOT_SPECIFIED"
                );
            }
        ...
        if(!isset($arOptions['ENABLE_MULTIFIELDS']) || $arOptions['ENABLE_MULTIFIELDS'] === true)
        {
            if(isset($fields['PHONE']) && is_array($fields['PHONE']) && !empty($fields['PHONE']))
            {
                ...
                echo '<span class="crm-detail-info-item-name">', GetMessage('YNSIR_ENTITY_INFO_PHONE'), ':', '</span>';
            ...
            {
                echo '<div class="crm-detail-info-item">';
                echo '<span class="crm-detail-info-item-name">', GetMessage('YNSIR_ENTITY_INFO_EMAIL'), ':', '</span>';
    ...
    public static function RenderNearestActivity($arParams)
    {
        ...
        if($ID > 0)
        {
            ...
            $timeFormatted = $timestamp > 0 ? CCrmComponentHelper::TrimDateTimeString(FormatDate('FULL', $timestamp)) : GetMessage('YNSIR_ACTIVITY_TIME_NOT_SPECIFIED');
            ...
            if($useGridExtension)
            {
				...
				onclick="BX.YNSIRUIGridExtension.viewActivity(\''.CUtil::JSEscape($gridManagerID).'\', '.$ID.', { enableEditButton:'.($allowEdit ? 'true' : 'false').' }); return false;">'
            ...
            if($allowEdit && !empty($menuItems))
            {
                if($useGridExtension)
                {
                    $result .= '<div class="crm-nearest-activity-plus" onclick="BX.YNSIRUIGridExtension.showContextMenu(\''.$menuID.'\', this);"></div>
					<script type="text/javascript">BX.YNSIRUIGridExtension.createContextMenu("'.$menuID.'", '.CUtil::PhpToJSObject($menuItems).');</script>';
                }
                else
                {
            ...
            if($responsibleID > 0)
            {
                ...
                $result .= '<div class="crm-list-deal-responsible"><span class="crm-list-deal-responsible-grey">'.htmlspecialcharsbx(GetMessage('YNSIR_ENTITY_ACTIVITY_FOR_RESPONSIBLE')).'</span><a class="crm-list-deal-responsible-name" target="_blank" href="'.htmlspecialcharsbx($responsibleShowUrl).'">'.htmlspecialcharsbx($responsibleFullName).'</a></div>';
            }
            return $result;
        }
        elseif($allowEdit && !empty($menuItems))
        {
            if($useGridExtension)
            {
                return '<span class="crm-activity-add-hint">'.htmlspecialcharsbx(GetMessage('YNSIR_ENTITY_ADD_ACTIVITY_HINT')).'</span>
				<a class="crm-activity-add" onclick="BX.YNSIRUIGridExtension.showContextMenu(\''.$menuID.'\', this); return false;">'.htmlspecialcharsbx(GetMessage('YNSIR_ENTITY_ADD_ACTIVITY')).'</a>
				<script type="text/javascript">BX.YNSIRUIGridExtension.createContextMenu("'.$menuID.'", '.CUtil::PhpToJSObject($menuItems).');</script>';
            }
            else
            {
                return '<span class="crm-activity-add-hint">'.htmlspecialcharsbx(GetMessage('YNSIR_ENTITY_ADD_ACTIVITY_HINT')).'</span>
				<a class="crm-activity-add" onclick="BX.CrmInterfaceGridManager.showMenu(\''.$menuID.'\', this); return false;">'.htmlspecialcharsbx(GetMessage('YNSIR_ENTITY_ADD_ACTIVITY')).'</a>
				<script type="text/javascript">BX.CrmInterfaceGridManager.createMenu("'.$menuID.'", '.CUtil::PhpToJSObject($menuItems).');</script>';
            }
        }
    ...
    private static function RenderListMultiFieldValues($ID, &$arValues, $typeName, &$arValueTypes, $arOptions = null)
    {
        ...
        if($qty > 1)
        {
            ...
            $result .= '<span id="'.htmlspecialcharsbx($buttonID)
                ...
                .CUtil::JSEscape($ID).'\', { \'anchorId\':\''.CUtil::JSEscape($buttonID).'\', \'items\':'.CUtil::PhpToJSObject($arPopupItems).', \'typeName\':\''.CUtil::JSEscape($typeName).'\' }).show();">'.htmlspecialcharsbx(GetMessage('YNSIR_ENTITY_MULTI_FIELDS_MORE')).' '.($qty - 1).'</span>';
            $result .= '</div>';
        }
    ...
    public static function RenderResponsiblePanel($arParams)
    {
        ...
        $caption = isset($arParams['CAPTION']) && is_string($arParams['CAPTION']) && $arParams['CAPTION'] !== ''
            ? $arParams['CAPTION'] : GetMessage('YNSIR_ENTITY_INFO_RESPONSIBLE');
        ...
        if($editable)
        {
            ...
            echo '<span class="crm-detail-info-resp-edit" id="', htmlspecialcharsbx($editButtonID), '">', htmlspecialcharsbx(GetMessage('YNSIR_ENTITY_INFO_RESPONSIBLE_CHANGE')), '</span>';
        }
        ...
        if($userInfoProviderID !== '')
        {
            if(!self::$USER_INFO_PROVIDER_MESSAGES_REGISTRED)
            {
                ...
                '{ "generalError":"', GetMessageJS('YNSIR_GET_USER_INFO_GENERAL_ERROR'), '" }',
                '});',
                '</script>';
    ...
    public static function RenderDealStageSettings()
    {
        ...
        foreach(DealCategory::getAllIDs() as $categoryID)
        {
            ...
            foreach(self::PrepareDealStages($categoryID) as $stage)
            {
                ...
                if($stage['STATUS_ID'] === $successStageID)
                {
                    ...
                    $info['hint'] = GetMessage('YNSIR_DEAL_STAGE_MANAGER_WON_STEP_HINT');
                }
        ...
        $messages = array(
            'dialogTitle' => GetMessage('YNSIR_DEAL_STAGE_MANAGER_DLG_TTL'),
            //'apologyTitle' => GetMessage('YNSIR_DEAL_STAGE_MANAGER_APOLOGY_TTL'),
            'failureTitle' => GetMessage('YNSIR_DEAL_STAGE_MANAGER_FAILURE_TTL'),
            'selectorTitle' => GetMessage('YNSIR_DEAL_STAGE_MANAGER_SELECTOR_TTL')
        );
    ...
    public static function RenderLeadStatusSettings()
    {
        ...
        foreach(self::PrepareLeadStatuses() as $status)
        {
            ...
            if($status['STATUS_ID'] === 'CONVERTED')
            {
                $isTresholdPassed = true;
                $info['semantics'] = 'success';
                $info['name'] = GetMessage('YNSIR_LEAD_STATUS_MANAGER_CONVERTED_STEP_NAME');
                $info['hint'] = GetMessage('YNSIR_LEAD_STATUS_MANAGER_CONVERTED_STEP_HINT');
                $info['isFrozen'] = true;
            }
        ...
        $messages = array(
            'dialogTitle' => GetMessage('YNSIR_LEAD_STATUS_MANAGER_DLG_TTL'),
            //'apologyTitle' => GetMessage('YNSIR_LEAD_STATUS_MANAGER_APOLOGY_TTL'),
            'failureTitle' => GetMessage('YNSIR_LEAD_STATUS_MANAGER_FAILURE_TTL'),
            'selectorTitle' => GetMessage('YNSIR_LEAD_STATUS_MANAGER_SELECTOR_TTL')
        );
    ...
    public static function RenderInvoiceStatusSettings()
    {
        ...
        foreach(self::PrepareInvoiceStatuses() as $status)
        {
            ...
            if($status['STATUS_ID'] === 'P')
            {
                $isTresholdPassed = true;
                $info['semantics'] = 'success';
                $info['hint'] = GetMessage('YNSIR_INVOICE_STATUS_MANAGER_F_STEP_HINT');
                $info['hasParams'] = true;
            }
        ...
        $messages = array(
            'dialogTitle' => GetMessage('YNSIR_INVOICE_STATUS_MANAGER_DLG_TTL'),
            'failureTitle' => GetMessage('YNSIR_INVOICE_STATUS_MANAGER_FAILURE_TTL'),
            'selectorTitle' => GetMessage('YNSIR_INVOICE_STATUS_MANAGER_SELECTOR_TTL'),
            'setDate' =>  GetMessage('YNSIR_INVOICE_STATUS_MANAGER_SET_DATE'),
            'dateLabelText' => GetMessage('YNSIR_INVOICE_STATUS_MANAGER_DATE_LABEL'),
            'payVoucherNumLabelText' => GetMessage('YNSIR_INVOICE_STATUS_MANAGER_PAY_VOUCHER_NUM_LABEL'),
            'commentLabelText' => GetMessage('YNSIR_INVOICE_STATUS_MANAGER_COMMENT_LABEL'),
            'notSpecified' => GetMessage('YNSIR_INVOICE_STATUS_MANAGER_NOT_SPECIFIED')
        );
    ...
    public static function RenderInvoiceStatusInfo($params)
    {
        ...
        foreach ($params['items'] as $k => $item)
        {
            ...
            $html .= "\t\t".'<span class="crm-detail-info-item-name">'.htmlspecialcharsbx(GetMessage('YNSIR_INVOICE_FIELD_'.$k)).':</span>'.PHP_EOL;
    ...
    public static function RenderDealPaidSumField($params)
    {
        ...
        $html .= "\t\t".'<span class="crm-detail-info-item-name">'.htmlspecialcharsbx(GetMessage('YNSIR_DEAL_SUM_PAID_FIELD')).':</span>'.PHP_EOL;
    ...
    public static function RenderQuoteStatusSettings()
    {
        ...
        foreach(self::PrepareQuoteStatuses() as $status)
        {
            ...
            if($status['STATUS_ID'] === 'APPROVED')
            {
                $isTresholdPassed = true;
                $info['semantics'] = 'success';
                $info['hint'] = GetMessage('YNSIR_QUOTE_STATUS_MANAGER_APPROVED_STEP_HINT');
            }
        ...
        $messages = array(
            'dialogTitle' => GetMessage('YNSIR_QUOTE_STATUS_MANAGER_DLG_TTL'),
            'failureTitle' => GetMessage('YNSIR_QUOTE_STATUS_MANAGER_FAILURE_TTL'),
            'selectorTitle' => GetMessage('YNSIR_QUOTE_STATUS_MANAGER_SELECTOR_TTL')
        );
