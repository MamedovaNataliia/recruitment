...
class YNSIRDocument
{
	...
	static public function GetDocumentFieldTypes($documentType)
	{
		...
		$ignoredUserTypes = array('string', 'double', 'boolean', 'integer', 'datetime', 'file', 'employee', 'enumeration', 'video', 'string_formatted', 'webdav_element_history', 'disk_version', 'disk_file', 'vote', 'url_preview');
	...
	static public function GetDocumentAdminPage($documentId)
	{
		...
		$entityTypeID = $entityTypeName !== '' ? YNSIROwnerType::ResolveID($entityTypeName) : YNSIROwnerType::Undefined;
		$entityID = isset($arDocumenInfo['ID']) ? intval($arDocumenInfo['ID']) : 0;

		return $entityTypeID !== YNSIROwnerType::Undefined && $entityID > 0
			? YNSIROwnerType::GetShowUrl($entityTypeID, $entityID, false) : null;
	}
	...
	static public function GetDocument($documentId)
	{
		...
		{
			case YNSIR_JOB_ORDER:
				$dbDocumentList = YNSIRJobOrder::GetList(
					array(),
		...
		if (($objDocument = $dbDocumentList->Fetch()) !== false)
		{
			...
			{
				case 'DEAL':
					YNSIRDocumentDeal::PrepareDocument($objDocument);
					break;
				case 'LEAD':
					YNSIRDocumentLead::PrepareDocument($objDocument);
					break;
				case 'CONTACT':
					YNSIRDocumentContact::PrepareDocument($objDocument);
					break;
				case 'COMPANY':
					YNSIRDocumentCompany::PrepareDocument($objDocument);
					break;
			}
			...
			case 'CONTACT':
				if (!empty($arResult['PHOTO']))
					$arResult['PHOTO'] = CBPDocument::PrepareFileForHistory(
						array('crm', 'YNSIRDocument'.ucfirst(strtolower($arDocumentID['TYPE'])), $documentId),
						$arResult['PHOTO'],
						$historyIndex
					);
			break;
...
//		if($entityTypeName === YNSIR_JOB_ORDER)
//		{
//			return YNSIR_JOB_ORDER;
//		}
	...
	static function CanUserOperateDocument($operation, $userId, $documentId, $arParameters = array())
	{
		...
		if (!array_key_exists('AllUserGroups', $arParameters))
		{
			if (!array_key_exists('UserGroups', $arParameters))
			{
				...
				{
					$responsibleID = YNSIROwnerType::GetResponsibleID(
						YNSIROwnerType::ResolveID($arDocumentID['TYPE']),
						$arDocumentID['ID'],
						false
					);
					if($responsibleID <= 0)
		...
		$userPermissions = YNSIRPerms_::GetUserPermissions($userId);
		if ($arDocumentID['ID'] > 0)
		{
			...
			{
				$result = YNSIRAuthorizationHelper::CheckReadPermission($permissionEntity, $arDocumentID['ID'], $userPermissions, $entityAttrs);
			}
			else
			{
				$result = YNSIRAuthorizationHelper::CheckUpdatePermission($permissionEntity, $arDocumentID['ID'], $userPermissions, $entityAttrs);
			}
		}
		else
		{
			$result = YNSIRAuthorizationHelper::CheckCreatePermission($permissionEntity, $userPermissions);
		}
	...
	static function CanUserOperateDocumentType($operation, $userId, $documentType, $arParameters = array())
	{
		...
		{
			return YNSIRAuthorizationHelper::CheckReadPermission($permissionEntity, 0, $userPermissions);
		}
		return YNSIRAuthorizationHelper::CheckCreatePermission($permissionEntity, $userPermissions);
	}
	...
	static public function DeleteDocument($documentId)
	{
		...
		{
			case YNSIR_JOB_ORDER:
			    YNSIRJobOrder::DeActivebyArray(array($arDocumentID['ID']));
				break;
            default:
				break;
		}
	...
	static public function GetAllowableUserGroups($documentType)
	{
		...
		$arGroupsID = array(1);
		$arUsersID = array();
		$arRelations = YNSIRPerms_::GetEntityRelations($documentType, BX_CRM_PERM_SELF);
				...
				switch ($arDocumentID['TYPE'])
				{
					case YNSIR_JOB_ORDER:
						$dbDocumentList = YNSIRJobOrder::GetList(
							...
							false,
							false,
							array(YNSIRJobOrder::JO_ENTITY_OWNER)
						);
					break;
				}
			}
			$arFields = is_object($dbDocumentList) ? $dbDocumentList->Fetch() : null;
			return is_array($arFields) && isset($arFields[YNSIRJobOrder::JO_ENTITY_OWNER])
				? array($arFields[YNSIRJobOrder::JO_ENTITY_OWNER]) : array();
		}
	...
	static public function GetDocumentType($documentId)
	{
		...
		{
			case YNSIR_JOB_ORDER:
				$exists = YNSIRJobOrder::Exists($arDocumentID['ID']);
				break;
	...
	protected static function GetDocumentInfo($documentId)
	{
		...
		if (!in_array($arDocumentId[0], array(YNSIR_JOB_ORDER)))
			return false;
		$arDocumentId[1] = intval($arDocumentId[1]);

		static $arMap = array(YNSIR_JOB_ORDER => "YNSIRDocumentJobOrder");

		return array(
			'TYPE' => $arDocumentId[0],
			'ID' => $arDocumentId[1],
			'DOCUMENT_TYPE' => array("ynsirecruitment", $arMap[$arDocumentId[0]], $arDocumentId[0])
		);
	}
	...
	protected static function PrepareCrmUserTypeValueView($value, $defaultTypeName = '')
	{
		...
		if(count($parts) > 1)
		{
			return YNSIROwnerType::GetCaption(
				YNSIROwnerType::ResolveID(YNSIROwnerTypeAbbr::ResolveName($parts[0])),
				$parts[1],
				false
			);
		}
		...
		{
			return YNSIROwnerType::GetCaption(
				YNSIROwnerType::ResolveID($defaultTypeName),
				$value,
				false
			);
		}
	...
	public static function GetDocumentAuthorID($documentId)
	{
		...
		return YNSIROwnerType::GetResponsibleID(
			YNSIROwnerType::ResolveID($entityTypeName),
			$entityId,
			false
		);
	}
