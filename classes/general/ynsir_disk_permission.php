...
class YNSIRDiskPermission extends \Bitrix\Disk\Internals\Controller
{
    ...
    public function processActionShowRightsOnStorageDetail($storageId)
    {
        $storage = Storage::loadById((int)$storageId, array('ROOT_OBJECT'));
        if(!$storage)
        {
    ...
    public function processActionShowRightsOnObjectDetail($objectId)
    {
        $arResult = array();
        $object = BaseObject::loadById((int)$objectId);
        if(!$object)
        {
            $arResult['ERROR'] = 'Object null';
        }
        $storage = $object->getStorage();
        if(!$storage)
        {
            $arResult['ERROR'] = 'Storage null';
        }
        ...
        $arResult['rights'] = $rightsByAccessCode;
        $arResult['accessCodeNames'] = $names;
        $arResult['tasks'] = $rightsManager->getTasks();
    ...
    public function processActionSaveRightsOnStorage($storageId)
    {
        $storage = Storage::loadById((int)$storageId, array('ROOT_OBJECT'));
        if(!$storage)
        {
    ...
    public function processActionSaveRightsOnObject($objectId)
    {
        ...
        $objectNewRights = array (
            'IU477' => array (
                'item' => array (
                    'id' => 'IU477',
                    'name' => 'Employee and supervisors: Super Admin',
                ),
                'type' => 'group',
                'right' => array (
                    'title' => 'Full access',
            ...
            'G12' => array (
                'item' =>
                    array (
                        'id' => 'G12',
                        'name' => 'Group: My Company: Employees',
                    ),
                'type' => 'group',
                'right' => array (
                    'id' => '42',
    ...
    public function processActionShowSharingDetailChangeRights($objectId)
    {
        $arResult = array();
        /** @var \Bitrix\Disk\File|\Bitrix\Disk\Folder $object */
        $object = BaseObject::loadById((int)$objectId, array('STORAGE'));
        ...
        $arResult['member'] = is_array($entityList) && !empty($entityList) ? $entityList : array();
        return $arResult;
    }
    ...
    public function processActionChangeSharingAndRights($objectId = 0, $entityToNewShared = array(), $sFullApply = true)
    {
        $iResult = 0;
        /** @var \Bitrix\Disk\File|\Bitrix\Disk\Folder $object */
        $object = BaseObject::loadById((int)$objectId, array('STORAGE'));
        ...
        if(!empty($entityToNewShared) && is_array($entityToNewShared) || $sFullApply)
        {
            $extendedRights = $entityToNewShared;
            $newExtendedRightsReformat = array();
            foreach($extendedRights as $entityId => $right)
    ...
    private function getFileAndExternalLink($objectId)
    {
        /** @var File $file */
    ...
    protected function processActionDisableExternalLink($objectId)
    {
        ...
        list($file, $extLink) = $this->getFileAndExternalLink($objectId);
