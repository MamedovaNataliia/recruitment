...
class CAllYNSIRActivity
{
    const CACHE_NAME = 'RECRUITMENT_ACTIVITY_CACHE';
    const ACTIVITY_CACHE_TIME = 86400;
    const ACTIVITY_CACHE_URL = '/ynsirecruitment/activity/';
    const TABLE_ALIAS = 'A';
    const UF_ENTITY_TYPE = 'RECRUITMENT_ACTIVITY';
    const FREFIX_CACHE = 'RECRUITMENT_ACTIVITY_';
    //const UF_WEBDAV_FIELD_NAME = 'UF_YNSIR_ACTIVITY_WDAV';
    const COMMUNICATION_TABLE_ALIAS = 'AC';
    const IS_USE_CACHE = false;
    ...
    private static $STORAGE_TYPE_ID = 0;
    ...
    // CRUD -->
    public static function checkRoom($arFields)
    {
        CModule::IncludeModule('calendar');
        $settings = CCalendar::GetSettings(array('request' => false));
        $from = CCalendar::Date(CCalendar::Timestamp($arFields['START_TIME']));
        $to = CCalendar::Date(CCalendar::Timestamp($arFields['END_TIME']));
        $loc_old = $arFields['LOCATION']['OLD'] ? CCalendar::ParseLocation(trim($arFields['LOCATION']['OLD'])) : false;
        $loc_new = CCalendar::ParseLocation(trim($arFields['LOCATION']['NEW']));
        $Params = array(
            'dateFrom' => $from,
            'dateTo' => $to,
            'regularity' => 'NONE',
            'members' => isset($arFields['RESPONSIBLE_ID']) ? $arFields['RESPONSIBLE_ID'] : false,
        );
    ...
    public static function Add(&$arFields, $checkPerms = true, $regEvent = true, $options = array())
    {
        ...
        if (isset($arFields['PARENT_ID']) && $arFields['PARENT_ID'] > 0) {
            $parent = $DB->query(sprintf(
                ...
                \YNSIRActivity::TABLE_NAME, $arFields['PARENT_ID']
            ))->fetch();
        }
        ...
        $arAttenDeesCodes = $arFields['ATTENDEES_CODES'];
        self::NormalizeAttendeesFields($arFields);
        $ID = $DB->Add(YNSIRActivity::TABLE_NAME, $arFields, array('DESCRIPTION', 'STORAGE_ELEMENT_IDS', 'SETTINGS', 'PROVIDER_PARAMS', 'PROVIDER_DATA'));
        if (is_string($ID) && $ID !== '') {
        ...
        $arAttenDees = array(
            'ATTENDEES_CODES' => $arAttenDeesCodes,
            'ACTIVITY_ID' =>$ID
        );

        $ATTENDEES_LAST_ID = YNSIRActivityAttendees::Add($arAttenDees);
        unset($arAttenDees);
        ...
        YNSIRActivity::DoSaveElementIDs($ID, $arFields['STORAGE_TYPE_ID'], $storageElementIDs);
        ...
        if ($completed && isset($arFields['STATUS'])) {
            $completed = $arFields['STATUS'] == YNSIRActivityStatus::Completed;
        }
        ...
        //        Bitrix\Crm\Search\SearchContentBuilderFactory::create(YNSIROwnerType::Activity)->build($ID);
        Bitrix\YNSIR\Search\YNSIRSearchContentBuilderFactory::create(YNSIROwnerType::Activity)->build($ID);
        //endregion

        // Synchronize user activity -->
        $responsibleID = isset($arFields['RESPONSIBLE_ID']) ? intval($arFields['RESPONSIBLE_ID']) : 0;
        if ($responsibleID > 0) {
            $counterCodes = EntityCounterManager::prepareCodes(YNSIROwnerType::Activity, EntityCounterType::CURRENT);
            foreach ($arBindings as $arBinding) {
        ...
        if ($provider !== null && $provider::canUseCalendarEvents($providerTypeId)) {
            ...
            if (
                ...
                $arLocation_tmp = array();
                if(isset($arFields['LOCATION']) && strlen($arFields['LOCATION']) > 0) {
                    $arLocation_tmp['NEW'] = $arFields['LOCATION'];
                } else {
                    $arLocation_tmp['NEW'] = "";
                }
                $arLocation_tmp['OLD'] = "false";
                $arFields['LOCATION'] = $arLocation_tmp;
                $eventID = self::SaveCalendarEvent($arFields);

                if (is_int($eventID) && $eventID > 0) {
                    self::SetCalendarEventId($eventID, $ID,$arFields['LOCATION']);
                }
            }
        }
        if ($regEvent) {
        ...
        if (is_int($ID) && $ID > 0) {
            ...
            if (isset($options['REGISTER_SONET_EVENT']) && $options['REGISTER_SONET_EVENT'] === true) {
                ...
                if ($responsibleID > 0) {
//                    YNSIRSonetSubscription::RegisterSubscription(
//                        YNSIROwnerType::Activity,
//                        $ID,
//                        YNSIRSonetSubscriptionType::Responsibility,
//                        $responsibleID
//                    );
                }
            }
            ...
            $rsEvents = GetModuleEvents('ynsirecruitment', 'OnActivityAdd');
            while ($arEvent = $rsEvents->Fetch()) {
                ExecuteModuleEventEx($arEvent, array($ID, &$arFields));
            }

            if (\Bitrix\Main\Loader::includeModule('pull')) {
                \CPullStack::AddByUser($arFields['RESPONSIBLE_ID'], array(
                    ...
                    'params' => array(
                        ...
                        'OWNER_TYPE_NAME' => YNSIROwnerType::ResolveName($arFields['OWNER_TYPE_ID']),
    ...
    public static function Update($ID, $arFields, $checkPerms = true, $regEvent = true, $options = array())
    {
        ...
        if (is_array($arBindings)) {
            ...
            if ($bindingQty === 1) {
                ...
                $arFields['OWNER_ID'] = 0;
                $arFields['OWNER_TYPE_ID'] = YNSIROwnerType::Undefined;
            }
        }
        ...
        $arAttendeesCode = $arFields['ATTENDEES_CODES'];
        self::NormalizeDateTimeFields($arFields);
        self::NormalizeAttendeesFields($arFields);
        ...
        $sql = 'UPDATE ' . YNSIRActivity::TABLE_NAME . ' SET ' . $DB->PrepareUpdate(YNSIRActivity::TABLE_NAME, $arFields) . ' WHERE ID = ' . $ID;
        ...
        if (isset($arFields['PARENT_ID']) && $arFields['PARENT_ID'] > 0 && $arPrevEntity['PARENT_ID'] == 0) {
            $DB->query(sprintf(
                ...
                \YNSIRActivity::TABLE_NAME, $arFields['THREAD_ID'], $ID
            ));
        }

        //update attendees CODE
        $arAttenDees = array(
            'ATTENDEES_CODES' => $arAttendeesCode,
            'ACTIVITY_ID' =>$ID
        );
        $DB->StartTransaction();
        $delRes = YNSIRActivityAttendees::DeleteByActID($ID);
        $ATTENDEES_LAST_ID = YNSIRActivityAttendees::Add($arAttenDees);
        if (is_object($delRes) && $ATTENDEES_LAST_ID > 0) {
            $DB->Commit();
        } else {
            $DB->Rollback();
        }
        unset($delRes);
        unset($ATTENDEES_LAST_ID);
        //end update attendees

        $arFields['SETTINGS'] = isset($arFields['SETTINGS']) ? unserialize($arFields['SETTINGS']) : array();
        $arFields['PROVIDER_PARAMS'] = isset($arFields['PROVIDER_PARAMS']) ? unserialize($arFields['PROVIDER_PARAMS']) : array();

        YNSIRCacheHelper::ClearCached(self::FREFIX_CACHE.$ID, self::ACTIVITY_CACHE_URL);
        ...
        if (is_array($storageElementIDs)) {
            YNSIRActivity::DoSaveElementIDs($ID, $storageTypeID, $storageElementIDs);
        }
        ...
        if ($prevCompleted && isset($arPrevEntity['STATUS'])) {
            $prevCompleted = $arPrevEntity['STATUS'] == YNSIRActivityStatus::Completed;
        }

        $curCompleted = isset($arCurEntity['COMPLETED']) && $arCurEntity['COMPLETED'] === 'Y';
        if ($curCompleted && isset($arCurEntity['STATUS'])) {
            $prevCompleted = $arCurEntity['STATUS'] == YNSIRActivityStatus::Completed;
        }
            ...
            if (!$skipCalendarEvent) {
                ...
                if (!$completed || $provider::canKeepCompletedInCalendar($providerTypeId)) {
                    ...
                    if(isset($arFields['LOCATION'])) {
                        $arLocation_tmp['NEW'] = $arCurEntity['LOCATION'];
                        $arLocation_tmp['OLD'] = $arPrevEntity['LOCATION'];
                        $arCurEntity['LOCATION'] = $arLocation_tmp;
                    } else {
                        $arLocation_tmp['NEW'] = "";
                        unset($arCurEntity['LOCATION']);
                    }
                ...
                if (is_int($eventID)) {
                    $newLOCATION = $arCurEntity['LOCATION']['NEW'];
                    self::SetCalendarEventId($eventID, $ID,$newLOCATION);
                    $arCurEntity['LOCATION'] = $newLOCATION;
                }
            }
        }
        if ($regEvent) {
        ...
        if ($registerSonetEvent) {
            $isSonetEventSynchronized = self::SynchronizeLiveFeedEvent(
                ...
                array(
                    ...
                    'EDITOR_ID' => (intval($arFields["EDITOR_ID"]) > 0 ? $arFields["EDITOR_ID"] : YNSIRSecurityHelper::GetCurrentUserID()),
        ...
        $rsEvents = GetModuleEvents('ynsirecruitment', 'OnActivityUpdate');
        while ($arEvent = $rsEvents->Fetch()) {
            ExecuteModuleEventEx($arEvent, array($ID, &$arFields));
        }

        $event = new \Bitrix\Main\Event(
            'ynsirecruitment', 'OnActivityModified',
            array(
                'before' => $arPrevEntity,
                'current' => $arCurEntity,
            )
        ...
        if (defined("BX_COMP_MANAGED_CACHE")) {
            $GLOBALS["CACHE_MANAGER"]->ClearByTag("YNSIR_ACTIVITY_" . $ID);
        }
    ...
    public static function Delete($ID, $checkPerms = true, $regEvent = true, $options = array())
    {
        ...
        $events = GetModuleEvents('ynsirecruitment', 'OnBeforeActivityDelete');
        ...
        //DELETE YNSIR ATTENDEE
        YNSIRActivityAttendees::DeleteByActID($ID);

//        self::UnregisterLiveFeedEvent($ID);
//        YNSIRSonetSubscription::UnRegisterSubscriptionByEntity(YNSIROwnerType::Activity, $ID);
            ...
            if ($regEvent) {
                foreach ($arBindings as $arBinding) {
                    self::RegisterRemoveEvent(
                        ...
                        $ary,
                        $checkPerms,
                        YNSIRSecurityHelper::GetCurrentUserID()
                    );
                }
                unset($arBinding);
            }
        ...
        $rsEvents = GetModuleEvents('ynsirecruitment', 'OnActivityDelete');
        while ($arEvent = $rsEvents->Fetch()) {
    ...
    protected static function InnerDelete($ID, $options = array())
    {
        ...
        if ($result && !(isset($options['SKIP_FILES']) && $options['SKIP_FILES'])) {
            ...
            if ($result) {
                $result = $DB->Query(
                    'DELETE FROM ' . YNSIRActivity::ELEMENT_TABLE_NAME . ' WHERE ACTIVITY_ID = ' . $ID,
                    false,
        ...
        if ($result) {
            $result = $DB->Query('DELETE FROM ' . YNSIRActivity::TABLE_NAME . ' WHERE ID = ' . $ID, true) !== false;
        }
    ...
    protected static function NormalizeAttendeesFields(&$arFields)
    {

        if (isset($arFields['ATTENDEES_CODES'])) {
            if (is_array($arFields['ATTENDEES_CODES']) && !empty($arFields['ATTENDEES_CODES']))
            {
                array_walk($arFields['ATTENDEES_CODES'], function(&$item) { $item = 'U'.$item; });
    ...
    protected static function GetFields()
    {
        if (!isset(self::$FIELDS)) {
            ...
            $bindingTableName = YNSIRActivity::BINDING_TABLE_NAME;
            $bindingJoin = "INNER JOIN {$bindingTableName} AB ON A.ID = AB.ACTIVITY_ID";
            //JOIN WITH ATTENDEES CODE

            self::$FIELDS = array(
                'ID' => array('FIELD' => 'A.ID', 'TYPE' => 'int'),
                'OWNER_ID' => array('FIELD' => 'A.OWNER_ID', 'TYPE' => 'int'),
                'OWNER_TYPE_ID' => array('FIELD' => 'A.OWNER_TYPE_ID', 'TYPE' => 'int'),
                'BINDING_OWNER_ID' => array('FIELD' => 'AB.OWNER_ID', 'TYPE' => 'int', 'FROM' => $bindingJoin, 'DEFAULT' => 'Y'),
                'BINDING_OWNER_TYPE_ID' => array('FIELD' => 'AB.OWNER_TYPE_ID', 'TYPE' => 'int', 'FROM' => $bindingJoin, 'DEFAULT' => 'Y'),
                ...
                'REFERENCE_ID' => array('FIELD' => 'A.REFERENCE_ID', 'TYPE' => 'int'),
                'REFERENCE_TYPE_ID' => array('FIELD' => 'A.REFERENCE_TYPE_ID', 'TYPE' => 'int'),
                ...
                //calendar ATTENDEES CODE
                'ATTENDEES_CODES' => array('FIELD' => 'A.ATTENDEES_CODES', 'TYPE' => 'string'),
                'ROUND_ID' => array('FIELD' => 'A.ROUND_ID', 'TYPE' => 'int'),
                'PARENT_AUTO_ID' => array('FIELD' => 'A.PARENT_AUTO_ID', 'TYPE' => 'int'),

            );
        }

        $arFields = self::$FIELDS;
        YNSIRActivity::CreateLogicalField('TYPE_NAME', $arFields);
        return $arFields;
    }
    ...
    public static function CheckStorageElementExists($activityID, $storageTypeID, $elementID)
    {
        ...
        $dbResult = $DB->Query(
            'SELECT 1 FROM ' . YNSIRActivity::ELEMENT_TABLE_NAME . ' WHERE ACTIVITY_ID = ' . $activityID . ' AND STORAGE_TYPE_ID = ' . $storageTypeID . ' AND ELEMENT_ID = ' . $elementID,
            false,
            'File: ' . __FILE__ . '<br/>Line: ' . __LINE__
        );
    ...
    public static function HandleStorageElementDeletion($storageTypeID, $elementID)
    {
        ...
        $dbResult = $DB->Query(
            'SELECT ACTIVITY_ID FROM ' . YNSIRActivity::ELEMENT_TABLE_NAME . ' WHERE STORAGE_TYPE_ID = ' . $storageTypeID . ' AND ELEMENT_ID = ' . $elementID,
            false,
            'File: ' . __FILE__ . '<br/>Line: ' . __LINE__
        );
    ...
    private static function CheckFields($action, &$fields, $ID, $params = null)
    {
        ...
        $prevFields = null;
        //update LOCATION for ADD and EDIT
//        edit by nhatth2
        if (isset($fields['LOCATION'])) {
            if(is_array($fields['LOCATION']) && strlen($fields['LOCATION']['NEW']) > 0)
                $fields['LOCATION'] = $fields['LOCATION']['NEW'];
        ...
        if ($action == 'ADD') {
            ...
            if (isset($fields['PROVIDER_ID']) && empty($fields['TYPE_ID']))
                $fields['TYPE_ID'] = YNSIRActivityType::Provider;

            if (!isset($fields['TYPE_ID'])) {
                self::RegisterError(array('text' => 'TYPE_ID is not assigned.'));
            } elseif (!YNSIRActivityType::IsDefined($fields['TYPE_ID'])) {
                self::RegisterError(array('text' => 'TYPE_ID is not supported.'));
            }

            if ((int)$fields['TYPE_ID'] === YNSIRActivityType::Provider && ($provider = self::GetActivityProvider($fields)) === null) {
...
//			}
            ...
            if (!isset($fields['NOTIFY_TYPE'])) {
                $fields['NOTIFY_TYPE'] = YNSIRActivityNotifyType::None;
            }

            if ($fields['NOTIFY_TYPE'] == YNSIRActivityNotifyType::None) {
            ...
            if (!isset($fields['STATUS'])) {
                $fields['STATUS'] = ($fields['COMPLETED'] === 'Y')
                    ? YNSIRActivityStatus::Completed : YNSIRActivityStatus::Waiting;
            }
            ...
            if (!isset($fields['PRIORITY'])) {
                $fields['PRIORITY'] = YNSIRActivityPriority::Low;
            }

            if (!isset($fields['DIRECTION'])) {
                $fields['DIRECTION'] = YNSIRActivityDirection::Undefined;
            }

            if (!isset($fields['DESCRIPTION_TYPE'])) {
                $fields['DESCRIPTION_TYPE'] = YNSIRContentType::PlainText;
            }
        ...
        } else//if($action == 'UPDATE')
        {
            ...
            if (isset($fields['COMPLETED'])) {
                ...
                if (!isset($fields['STATUS'])) {
                    if ($fields['COMPLETED'] === 'Y'
                        && isset($prevFields['STATUS'])
                        && (int)$prevFields['STATUS'] === YNSIRActivityStatus::Waiting
                    )
                        $fields['STATUS'] = YNSIRActivityStatus::Completed;

                    if ($fields['COMPLETED'] === 'N'
                        && isset($prevFields['STATUS'])
                        && (int)$prevFields['STATUS'] !== YNSIRActivityStatus::Waiting
                    )
                        $fields['STATUS'] = YNSIRActivityStatus::Waiting;
                }
            }
        ...
        //DEADLINE
        if (!isset($fields['DEADLINE'])) {
            $start = isset($fields['START_TIME']) && $fields['START_TIME'] !== '' ? $fields['START_TIME'] : null;
            if ($start !== null) {
                $fields['DEADLINE'] = $start;
            } elseif ($action == 'ADD') {
    ...
    public static function DeleteBindings($activityID)
    {
        ...
        $DB->Query(
            'DELETE FROM ' . YNSIRActivity::BINDING_TABLE_NAME . ' WHERE ACTIVITY_ID = ' . $activityID,
            false,
            'File: ' . __FILE__ . '<br/>Line: ' . __LINE__
        );
    ...
    public static function DeleteCommunications($activityID)
    {
        ...
        global $DB;
        $commTableName = YNSIRActivity::COMMUNICATION_TABLE_NAME;
    ...
    public static function GetProviderByType($typeId)
    {
        ...
        switch ($typeId) {
            ...
            case YNSIRActivityType::Task:
                $provider = \Bitrix\YNSIR\Activity\Provider\Task::className();
                break;
            case YNSIRActivityType::Email:
    ...
    public static function GetByID($ID, $checkPerms = true)
    {
        ...
        if(self::IS_USE_CACHE) {
            $res = YNSIRCacheHelper::GetCached(self::ACTIVITY_CACHE_TIME, self::FREFIX_CACHE . $ID, self::ACTIVITY_CACHE_URL);
        ...
        if (is_array($res = $dbRes->Fetch())) {
            $arAttendees = array();
            if (intval($res['CALENDAR_EVENT_ID']) > 0)
                $arAttendees = CCalendarEvent::GetAttendees($res['CALENDAR_EVENT_ID']);
            $res['ATTENDEES'] = $arAttendees;
            if (self::IS_USE_CACHE) {
                YNSIRCacheHelper::SetCached($res, self::ACTIVITY_CACHE_TIME, self::FREFIX_CACHE . $ID, self::ACTIVITY_CACHE_URL);
            }
        }
    ...
    public static function GetList($arOrder = array(), $arFilter = array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array(), $arOptions = array())
    {
        $lb = new YNSIRSQLHelper(
            YNSIRActivity::DB_TYPE,
            YNSIRActivity::TABLE_NAME,
            self::TABLE_ALIAS,
            self::GetFields(),
            '',
            '',
            array(),
            array('CAllYNSIRActivity', '__AfterPrepareSql')
        );
        ...
        return (is_object($result) && is_subclass_of($result, 'CAllDBResult'))
            ? new YNSIRActivityDbResult($result, $arSelectFields)
            : $result;
    }
...
//        if (!YNSIRPerms_::IsAccessEnabled($userPermissions)) {
...
//        }
        ...
        $entitiesSql[strval(YNSIROwnerType::Candidate)] = CCrmLead::BuildPermSql($aliasPrefix, $permType, $permOptions);
        $entitiesSql[strval(YNSIROwnerType::Deal)] = CCrmDeal::BuildPermSql($aliasPrefix, $permType, $permOptions);
        $entitiesSql[strval(YNSIROwnerType::Contact)] = CCrmContact::BuildPermSql($aliasPrefix, $permType, $permOptions);
        $entitiesSql[strval(YNSIROwnerType::Company)] = CCrmCompany::BuildPermSql($aliasPrefix, $permType, $permOptions);
        //Invoice does not have activities
        //$entitiesSql[strval(YNSIROwnerType::Invoice)] = CCrmInvoice::BuildPermSql($aliasPrefix, $permType, $permOptions);
        ...
        if (isset($arOptions['RAW_QUERY']) && $arOptions['RAW_QUERY'] === true) {
            $tableName = \YNSIRActivity::TABLE_NAME;
            $sql = "SELECT {$aliasPrefix}.ID FROM {$tableName} {$aliasPrefix} WHERE {$sql}";
        }
        return $sql;
    }

    public static function __AfterPrepareSql(/*YNSIRSQLHelper*/
        $sender, $arOrder, $arFilter, $arGroupBy, $arSelectFields)
    {
        ...
        if (isset($arFilter['BINDINGS'])) {
            $sql = YNSIRActivity::PrepareBindingsFilterSql($arFilter['BINDINGS'], $sender->GetTableAlias());
            if ($sql !== '') {
                $sqlData['FROM'][] = $sql;
            }
        }
    ...
    public static function SaveBindings($ID, $arBindings, $registerEvents = true, $checkPerms = true)
    {
        $result = array();
        foreach ($arBindings as $arBinding) {
            $key = (int)$arBinding['OWNER_ID'] . '_' . (int)$arBinding['OWNER_TYPE_ID'];
        ...
        YNSIRActivity::DoSaveBindings($ID, array_values($result));
    }
    ...
    public static function GetBindings($ID)
    {
        ...
        $dbRes = $DB->Query(
            'SELECT ID, OWNER_ID, OWNER_TYPE_ID FROM ' . YNSIRActivity::BINDING_TABLE_NAME . ' WHERE ACTIVITY_ID = ' . $DB->ForSql($ID),
            false,
            'FILE: ' . __FILE__ . '<br /> LINE: ' . __LINE__
        );
    ...
    public static function Rebind($ownerTypeID, $oldOwnerID, $newOwnerID)
    {
        ...
        $tableName = YNSIRActivity::TABLE_NAME;
        $bindingTableName = YNSIRActivity::BINDING_TABLE_NAME;
        $communicationTableName = YNSIRActivity::COMMUNICATION_TABLE_NAME;

        $sql = "SELECT ID FROM " . YNSIRActivity::BINDING_TABLE_NAME . " WHERE OWNER_TYPE_ID = {$ownerTypeID} AND OWNER_ID = {$oldOwnerID}";
        CSqlUtil::PrepareSelectTop($sql, 1, YNSIRActivity::DB_TYPE);
    ...
    public static function ChangeOwner($oldOwnerTypeID, $oldOwnerID, $newOwnerTypeID, $newOwnerID)
    {
        ...
        $sql = "SELECT ID FROM " . YNSIRActivity::BINDING_TABLE_NAME . " WHERE OWNER_TYPE_ID = {$oldOwnerTypeID} AND OWNER_ID = {$oldOwnerID}";
        CSqlUtil::PrepareSelectTop($sql, 1, YNSIRActivity::DB_TYPE);
        $dbResult = $DB->Query($sql, false, 'File: ' . __FILE__ . '<br>Line: ' . __LINE__);
        if (!(is_object($dbResult) && is_array($dbResult->Fetch()))) {
            return;
    ...
    private static function PrepareCommunicationSettings(&$arComm, $arFields = null)
    {
        ...
        if ($commEntityID > 0 && $commEntityTypeID > 0) {
            if ($commEntityTypeID === YNSIROwnerType::Candidate) {
                ...
                return true;
            } elseif ($commEntityTypeID === YNSIROwnerType::Contact) {
                ...
                return true;
            } elseif ($commEntityTypeID === YNSIROwnerType::Company) {
    ...
    public static function SaveCommunications($ID, $arComms, $arFields = array(), $registerEvents = true, $checkPerms = true)
    {
        ...
        YNSIRActivity::DoSaveCommunications($ID, $arComms, $arFields, $registerEvents, $checkPerms);
    }
    ...
    public static function GetCommunications($activityID, $top = 0)
    {
        ...
        $top = intval($top);
        if ($top > 0) {
            CSqlUtil::PrepareSelectTop($sql, $top, YNSIRActivity::DB_TYPE);
        }
    ...
    {
//        YNSIRActivity::DoResetEntityCommunicationSettings($entityTypeID, $entityID);
    }

    protected static function SaveEntityCommunicationSettings($entityTypeID, $entityID, array $settings)
    {
        $settings = serialize($settings);
//        YNSIRActivity::DoSaveEntityCommunicationSettings($entityTypeID, $entityID, $settings);
    }
            ...
            foreach ($IDs as $ID) {
                if (!isset(self::$CLIENT_INFOS[$ID])) {
                    ...
                    if (isset($info['NAME_DATA']) && $nameTemplate !== $info['NAME_DATA']['NAME_TEMPLATE']) {
                        ...
                        $ownerTypeID = isset($info['ENTITY_TYPE_ID']) ? $info['ENTITY_TYPE_ID'] : YNSIROwnerType::Undefined;
                        if ($ownerTypeID === YNSIROwnerType::Candidate) {
                        ...
                        } else//if($ownerTypeID === YNSIROwnerType::Contact)
                        {
        ...
        if (!empty($selectIDs)) {
            ...
            if (is_object($dbResult)) {
                ...
                while ($comm = $dbResult->Fetch()) {
                    ...
                    if ($entityID <= 0 || $entityTypeID <= 0 || $entityTypeID === YNSIROwnerType::Deal) {
                        continue;
                    }
                    ...
                    $info = array(
                        ...
                        'TITLE' => '',
                        'SHOW_URL' => YNSIROwnerType::GetShowUrl($entityTypeID, $entityID, false)
                    );
    ...
    public static function GetCommunicationList($arOrder = array(), $arFilter = array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array(), $arOptions = array())
    {
        $lb = new YNSIRSQLHelper(
            YNSIRActivity::DB_TYPE,
            YNSIRActivity::COMMUNICATION_TABLE_NAME,
    ...
    public static function PrepareCommunicationInfo(&$arComm, $arFields = null)
    {
        ...
        $title = '';
        $description = '';
        $entityTypeID = isset($arComm['ENTITY_TYPE_ID']) ? intval($arComm['ENTITY_TYPE_ID']) : YNSIROwnerType::Undefined;
        if ($entityTypeID === YNSIROwnerType::Candidate) {
            $honorific = '';
            $name = '';
            $secondName = '';
        ...
        if ($storageTypeID === StorageType::File) {
            ...
            foreach ($storageElementIDs as $elementID) {
                $infos[] = Bitrix\Crm\Integration\DiskManager::getFileInfo(
                    $elementID,
                    false,
                    array('OWNER_TYPE_ID' => YNSIROwnerType::Activity, 'OWNER_ID' => $arFields['ID'])
                );
            }
    ...
    public static function SaveRecentlyUsedCommunication($arComm, $userID = 0)
    {
        ...
        if (!YNSIROwnerType::IsDefined($entityTypeID)) {
            ...
            if ($entityTypeName !== '') {
                $entityTypeID = YNSIROwnerType::ResolveID($entityTypeName);
            }
        }
        ...
        if (!YNSIROwnerType::IsDefined($entityTypeID) || $entityID <= 0) {
            return false;
        }

        $optionName = $communicationType !== '' ? 'lru_' . strtolower($communicationType) : 'lru_person';

        $ary = CUserOptions::GetOption('ynsir_activity', $optionName, array(), $userID);
        $qty = count($ary);
        if ($qty > 0) {
            for ($i = 0; $i < $qty; $i++) {
                $item = $ary[$i];
        ...
        CUserOptions::SetOption('ynsir_activity', $optionName, $ary);
        return true;
    }
    ...
    public static function GetRecentlyUsedCommunications($communicationType, $userID = 0)
    {
        ...
        $optionName = $communicationType !== '' ? 'lru_' . strtolower($communicationType) : 'lru_person';
        return CUserOptions::GetOption('ynsir_activity', $optionName, array(), $userID);
    }
    ...
    public static function PrepareStorageElementIDs(&$arFields)
    {
        ...
        if (is_array($field)) {
            ...
            if ($ID <= 0) {
                ...
                $table = YNSIRActivity::TABLE_NAME;
                global $DB;
                $DB->QueryBind(
    ...
    public static function Exists($ID, $checkPerms = true)
    {
        ...
        $dbRes = YNSIRActivity::GetList(array(), $filter, false, false, array('ID'));
    ...
    public static function SetAutoCompleted($ID, $options = array())
    {
        ...
        return self::Update($ID, array('COMPLETED' => 'Y', 'STATUS' => YNSIRActivityStatus::AutoCompleted), true, true, $options);
    }

    public static function SetAutoCompletedByOwner($ownerTypeID, $ownerID)
    {
        $ownerID = (int)$ownerID;
        ...
        global $DB;
        $dbRes = $DB->Query(
            "SELECT A.ID FROM {$bindingTableName} B INNER JOIN {$tableName} A ON A.ID = B.ACTIVITY_ID WHERE B.OWNER_ID = {$ownerID} AND B.OWNER_TYPE_ID = {$ownerTypeID} AND A.COMPLETED = 'N'",
            false,
            'File: ' . __FILE__ . '<br/>Line: ' . __LINE__
        );
        ...
        while ($arRes = $dbRes->Fetch()) {
            self::SetAutoCompleted($arRes['ID']);
        }
    }
    ...
    public static function TryResolveUserFieldOwners(&$arUsefFieldData, &$arOwnerData, $arField = null)
    {
        ...
        $defaultTypeName = '';
        if (is_array($arField)
            && isset($arField['USER_TYPE_ID']) && $arField['USER_TYPE_ID'] === 'ynsirecruitment'
        ...
        foreach ($arUsefFieldData as $value) {
            ...
            if (preg_match('/^([A-Z]+)_([0-9]+)$/', strtoupper(trim($value)), $match) === 1) {
                $ownerTypeName = $match[1];//YNSIROwnerTypeAbbr::ResolveName($match[1]);
    ...
    {
        return \Bitrix\YNSIR\Activity\Provider\Task::createFromTask($taskID, $arTaskFields, $checkPerms, $regEvent);
    }
    ...
    public static function CreateFromDealEvent(&$arDeal)
    {
        ...
        $typeID = $arDeal['EVENT_ID'] === 'PHONE' ? YNSIRActivityType::Call : YNSIRActivityType::Activity;
        $subject = GetMessage($typeID === YNSIRActivityType::Call ? 'YNSIR_ACTIVITY_FROM_DEAL_EVENT_CALL' : 'YNSIR_ACTIVITY_FROM_DEAL_EVENT_INFO');
        ...
        $arFields = array(
            'TYPE_ID' => $typeID,
            'OWNER_TYPE_ID' => YNSIROwnerType::Deal,
            'OWNER_ID' => $dealID,
            'SUBJECT' => $subject,
            'START_TIME' => $dateFmt,
            'END_TIME' => $dateFmt,
            ...
            'LOCATION' => '',
            'DIRECTION' => $typeID === YNSIRActivityType::Call ? YNSIRActivityDirection::Outgoing : YNSIRActivityDirection::Undefined,
    ...
    private static function SetFromCalendarEvent($eventID, &$arEventFields, &$arFields)
    {
        ...
        if ($isNew) {
            //Meeting by default
            $arFields['TYPE_ID'] = YNSIRActivityType::Meeting;
        ...
        $fromTs = CCalendar::Timestamp($arEventFields['DATE_FROM']);
        $toTs = CCalendar::Timestamp($arEventFields['DATE_TO']);
        ...
        if ($isNew || isset($arEventFields['ATTENDEES_CODES'])) {
            $arAttendees = array();
            foreach ($arEventFields['ATTENDEES_CODES'] as $code) {
                if (substr($code, 0, 1) === 'U')
                {
                    $arAttendees[] = intval(substr($code, 1));
                }
            }
            unset($code);
            $arFields['ATTENDEES_CODES'] = isset($arAttendees) ? $arAttendees : array();
            unset($arAttendees);
        }

        if ($isNew || isset($arEventFields['IMPORTANCE'])) {
            $arFields['PRIORITY'] = YNSIRActivityPriority::FromCalendarEventImportance(isset($arEventFields['IMPORTANCE']) ? $arEventFields['IMPORTANCE'] : '');
        }
        ...
        if ($isNew || isset($arEventFields['REMIND'])) {
            ...
            if (empty($remindData)) {
                $arFields['NOTIFY_TYPE'] = YNSIRActivityNotifyType::None;
            } else {
                $remindInfo = $remindData[0];
                $remindType = YNSIRActivityNotifyType::FromCalendarEventRemind(isset($remindInfo['type']) ? $remindInfo['type'] : '');
                $remindValue = isset($remindInfo['count']) ? intval($remindInfo['count']) : 0;
                if ($remindType !== YNSIRActivityNotifyType::None && $remindValue > 0) {
    ...
    {
        if(intval($_GET['UF_YNSIR_TASK']) > 0) {
            $arTaskFields['UF_YNSIR_TASK'] = $_GET['UF_YNSIR_TASK'];
        }
        if(intval($_GET['UF_YNSIR_ASSOCIATE_ID']) > 0) {
            $arTaskFields['UF_YNSIR_ASSOCIATE_ID'] = $_GET['UF_YNSIR_ASSOCIATE_ID'];
        }
        \Bitrix\YNSIR\Activity\Provider\Task::onBeforeTaskAdd($arTaskFields);
    }
    ...
    {
        \Bitrix\YNSIR\Activity\Provider\Task::onTaskUpdate($taskID, $arTaskFields);
    }
    ...
    {
        \Bitrix\YNSIR\Activity\Provider\Task::onTaskDelete($taskID);
    }
...
//                }
    ...
    private static function ResolveEventTypeName($entityTypeID, $default = 'EVENT')
    {
        ...
        if ($entityTypeID === YNSIRActivityType::Call) {
            return 'CALL';
        } elseif ($entityTypeID === YNSIRActivityType::Meeting) {
            return 'INTERVIEW';
        } elseif ($entityTypeID === YNSIRActivityType::Task) {
            return 'TASK';
        } elseif ($entityTypeID === YNSIRActivityType::Email) {
            return 'EMAIL';
        }
    ...
    private static function PrepareFileEvent($storageTypeID, $elementID, $action, &$arRow, &$arEvents)
    {
        ...
        $arEvents[] = array(
            'EVENT_NAME' => GetMessage(
                "YNSIR_ACTIVITY_{$typeName}_FILE_{$action}",
                array('#NAME#' => $name)
            ),
    ...
    private static function PrepareUpdateEvent($fieldName, $arNewRow, $arOldRow, &$arEvents)
    {
        ...
        if ($fieldName === 'LOCATION') {
            $arOldRow['LOCATION'] = CCalendar::GetTextLocation($arOldRow['LOCATION']);
            $arNewRow['LOCATION'] = CCalendar::GetTextLocation($arNewRow['LOCATION']);
        }

        $typeID = self::GetActivityType($arNewRow);
        $EVENT_TYPE_ID = YNSIREvent::resolveEventType($typeID);

        $changed = false;
        $oldText = $newText = '';

        if ($fieldName === 'NOTIFY') {
            $oldType = isset($arOldRow['NOTIFY_TYPE']) ? intval($arOldRow['NOTIFY_TYPE']) : YNSIRActivityNotifyType::None;
            $newType = isset($arNewRow['NOTIFY_TYPE']) ? intval($arNewRow['NOTIFY_TYPE']) : YNSIRActivityNotifyType::None;
            ...
            if ($oldType !== $newType || $oldVal !== $newVal) {
                ...
                $oldText =
                    $oldType === YNSIRActivityNotifyType::None
                        ? YNSIRActivityNotifyType::ResolveDescription(YNSIRActivityNotifyType::None)
                        : (strval($oldVal) . ' ' . YNSIRActivityNotifyType::ResolveDescription($oldType));

                $newText =
                    $newType === YNSIRActivityNotifyType::None
                        ? YNSIRActivityNotifyType::ResolveDescription(YNSIRActivityNotifyType::None)
                        : (strval($newVal) . ' ' . YNSIRActivityNotifyType::ResolveDescription($newType));
            }
        } else if ($fieldName === 'ATTENDEES_CODES') {
            //update by nhatth2: track guests
            if ($arNewRow['ATTENDEES_CODES'] != '')
            {
                $arNewRow['ATTENDEES_CODES'] = explode(',', $arNewRow['ATTENDEES_CODES']);
            }
            if ($arOldRow['ATTENDEES_CODES'] != '')
            {
                $arOldRow['ATTENDEES_CODES'] = explode(',', $arOldRow['ATTENDEES_CODES']);
            }

            $newVal = isset($arNewRow['ATTENDEES_CODES'])&& is_array($arNewRow['ATTENDEES_CODES']) ? $arNewRow['ATTENDEES_CODES'] : array();
            $oldVal = isset($arOldRow['ATTENDEES_CODES'])&& is_array($arOldRow['ATTENDEES_CODES']) ? $arOldRow['ATTENDEES_CODES'] : array();

            //compare delete
            $arAttendeesDeleted = array_diff($oldVal,$newVal);
            $arAttendeesAdded = array_diff($newVal,$oldVal);

            array_walk($arAttendeesDeleted,
                function(&$item) { $item = (substr($item, 0, 1) === 'U')? intval(substr($item, 1)):''; });

            array_walk($arAttendeesAdded,
                function(&$item) { $item = (substr($item, 0, 1) === 'U')? intval(substr($item, 1)):''; });
            
            $strAttendeesDeleted = '';
            $strAttendeesAdded = '';

            if(!empty($arAttendeesDeleted)) {
                $strAttendeesDeleted = implode('|',$arAttendeesDeleted);
            }
            if(!empty($arAttendeesAdded)) {
                $strAttendeesAdded = implode('|',$arAttendeesAdded);
            }

            $dbUser = CUser::GetList(
                ($by = 'id'),
                ($order = 'asc'),
                array('ID' => "{$strAttendeesDeleted}|{$strAttendeesAdded}"),
                array(
                    'FIELDS' => array(
                        'ID',
                        'LOGIN',
            ...
            while (is_array($arUser = $dbUser->Fetch())) {
                ...
                $USERNAME = CUser::FormatName($template, $arUser);
                if (in_array($userID,$arAttendeesDeleted)) {
                    $actionName = 'DELETED';
                } elseif (in_array($userID,$arAttendeesAdded)) {
                    $actionName = 'ADDED';
                }

                $arEvents[] = array(
                    'EVENT_NAME' => GetMessage(
                        "YNSIR_ACTIVITY_CHANGE_{$typeName}_{$actionName}_{$fieldName}",
                        array('#NAME#' => $name)
                    ),
                    'EVENT_TYPE'=>$EVENT_TYPE_ID,
                    'EVENT_TEXT_1' => $USERNAME !== '' ? $USERNAME : GetMessage('YNSIR_ACTIVITY_FIELD_COMPARE_EMPTY'),
                    'EVENT_TEXT_2' => '',
                    'USER_ID' => isset($arNewRow['EDITOR_ID']) ? $arNewRow['EDITOR_ID'] : 0
                );
            }
            unset($arUser);
            //Dont add Event again.
            $bAreadyChecked = true;
            ...
            if (strcmp($old, $new) !== 0) {
                ...
                if ($fieldName === 'COMPLETED') {
                    $oldText = YNSIRActivityStatus::ResolveDescription(
                        $old === 'Y' ? YNSIRActivityStatus::Completed : YNSIRActivityStatus::Waiting,
                        self::GetActivityType($arOldRow)
                    );

                    $newText = YNSIRActivityStatus::ResolveDescription(
                        $new === 'Y' ? YNSIRActivityStatus::Completed : YNSIRActivityStatus::Waiting,
                        self::GetActivityType($arNewRow)
                    );
                } elseif ($fieldName === 'PRIORITY') {
                    $oldText = YNSIRActivityPriority::ResolveDescription($old);
                    $newText = YNSIRActivityPriority::ResolveDescription($new);
                } elseif ($fieldName === 'DIRECTION') {
                    $oldText = YNSIRActivityDirection::ResolveDescription($old, $typeID);
                    $newText = YNSIRActivityDirection::ResolveDescription($new, $typeID);
        ...
        if ($changed && !$bAreadyChecked) {
            ...
            $arEvents[] = array(
                'EVENT_NAME' => GetMessage(
                    "YNSIR_ACTIVITY_CHANGE_{$typeName}_{$fieldName}",
                    array('#NAME#' => $name)
                ),
                'EVENT_TYPE'=>$EVENT_TYPE_ID,
                'EVENT_TEXT_1' => $oldText !== '' ? $oldText : GetMessage('YNSIR_ACTIVITY_FIELD_COMPARE_EMPTY'),
                'EVENT_TEXT_2' => $newText !== '' ? $newText : GetMessage('YNSIR_ACTIVITY_FIELD_COMPARE_EMPTY'),
                'USER_ID' => isset($arNewRow['EDITOR_ID']) ? $arNewRow['EDITOR_ID'] : 0
            );
        }
    ...
    private static function RegisterAddEvent($ownerTypeID, $ownerID, $arRow, $checkPerms)
    {
        ...
        $location = isset($arRow['LOCATION']) ? CCalendar::GetTextLocation($arRow['LOCATION']) : '';
        $description = isset($arRow['DESCRIPTION']) ? $arRow['DESCRIPTION'] : '';
        $descriptionType = isset($arRow['DESCRIPTION_TYPE']) ? (int)$arRow['DESCRIPTION_TYPE'] : YNSIRContentType::PlainText;

        $eventText = '';
        if ($subject !== '') {
            $eventText .= GetMessage('YNSIR_ACTIVITY_SUBJECT') . ': ' . $subject . PHP_EOL;
        }

        if ($location !== '') {
            $eventText .= GetMessage('YNSIR_ACTIVITY_LOCATION') . ': ' . $location . PHP_EOL;
        }
        ...
        } elseif ($descriptionType === YNSIRContentType::PlainText) {
            $eventText .= $description;
        }

        $TYPE_ID = YNSIREvent::resolveEventType($typeID);
        $arEvents = array(
            array(
                'EVENT_TYPE' => $TYPE_ID,
                'EVENT_NAME' => GetMessage("YNSIR_ACTIVITY_{$typeName}_ADD"),
    ...
    private static function RegisterUpdateEvent($ownerTypeID, $ownerID, $arNewRow, $arOldRow, $checkPerms)
    {
        ...
        self::PrepareUpdateEvent('ATTENDEES_CODES', $arNewRow, $arOldRow, $arEvents);
        ...
        if ($newStorageTypeID !== $oldStorageTypeID) {
            ...
            if (!empty($arAddedElementIDs)) {
                foreach ($arAddedElementIDs as $elementID) {
                    self::PrepareFileEvent($oldStorageTypeID, $elementID, 'ADD', $arNewRow, $arEvents);
                }
            }
        }
    ...
    private static function RegisterRemoveEvent($ownerTypeID, $ownerID, $arRow, $checkPerms, $userID = 0)
    {
        ...
        return self::RegisterEvents(
            ...
            array(
                array(
                    'EVENT_NAME' => GetMessage("YNSIR_ACTIVITY_{$typeName}_REMOVE"),
                    'EVENT_TEXT_1' => $name,
                    'EVENT_TEXT_2' => '',
                    'USER_ID' => $userID
                )
    ...
    protected static function RegisterFileEvent($ID, $arFields, $fileInfo, $eventType, $checkPerms = true)
    {
        ...
        $eventName = GetMessage(
            "YNSIR_ACTIVITY_FILE_{$eventType}{$suffix}",
    ...
    protected static function RegisterCommunicationEvent($ID, $arFields, $arComm, $eventType, $checkPerms = true)
    {
        ...
        $eventName = GetMessage(
            "YNSIR_ACTIVITY_COMM_{$arComm['TYPE']}_{$eventType}{$suffix}",
            array('#NAME#' => self::GetEventName($arFields))
        );
    ...
    {
        return is_array($arFields) && isset($arFields['TYPE_ID']) ? intval($arFields['TYPE_ID']) : YNSIRActivityType::Undefined;
    }
    ...
    protected static function RegisterEvents($ownerTypeID, $ownerID, $arEvents, $checkPerms)
    {
        ...
        foreach ($arEvents as $arEvent) {
            $arEvent['EVENT_TYPE'] = (isset($arEvent['EVENT_TYPE'])) ? $arEvent['EVENT_TYPE']:1;
            $arEvent['ENTITY_TYPE'] = YNSIROwnerType::ResolveName($ownerTypeID);
            ...
            $YNSIREvent->Add($arEvent, $checkPerms);
        }
...
//            ? YNSIROwnerType::ResolveName((int)$ownerType)
...
//        }
//
//        $permissionEntityType = YNSIRPerms_::ResolvePermissionEntityType($ownerTypeName, $ownerID);
//        return CCrmAuthorizationHelper::CheckUpdatePermission($permissionEntityType, $ownerID, $userPermissions);
    ...
    public static function CheckItemUpdatePermission(array $fields, $userPermissions = null)
    {
        ...
        $ownerTypeID = isset($fields['OWNER_TYPE_ID']) ? (int)$fields['OWNER_TYPE_ID'] : YNSIROwnerType::Undefined;
        $ownerID = isset($fields['OWNER_ID']) ? (int)$fields['OWNER_ID'] : 0;

        return $ownerID > 0
            && YNSIROwnerType::IsDefined($ownerTypeID)
...
//        return CCrmAuthorizationHelper::CheckReadPermission($permissionEntityType, $ownerID, $userPermissions);
    ...
    protected static function ReadContactCommunication(&$arRes, $communicationType)
    {
        $item = array(
            ...
            'ENTITY_TYPE_ID' => YNSIROwnerType::Contact,
            'TYPE' => $communicationType,
            'VALUE' => $arRes['VALUE'],
            'ENTITY_SETTINGS' => array(
    ...
    protected static function ReadCompanyCommunication(&$arRes, $communicationType)
    {
        $item = array(
            'ENTITY_ID' => $arRes['ELEMENT_ID'],
            'ENTITY_TYPE_ID' => YNSIROwnerType::Company,
            'TYPE' => $communicationType,
            'VALUE' => $arRes['VALUE'],
            'ENTITY_SETTINGS' => array(
    ...
    protected static function ReadLeadCommunication(&$arRes, $communicationType)
    {
        $item = array(
            'ENTITY_ID' => $arRes['ELEMENT_ID'],
            'ENTITY_TYPE_ID' => YNSIROwnerType::Candidate,
            'TYPE' => $communicationType,
            'VALUE' => $arRes['VALUE'],
            'ENTITY_SETTINGS' => array(
    ...
    protected static function CreateLogicalField($fieldName, &$arFields)
    {
        ...
        if ($fieldName === 'TYPE_NAME') {
            ...
            $arTypeDescr = YNSIRActivityType::GetAllDescriptions();
    ...
    public static function GetCommunicationsByOwner($entityType, $entityID, $communicationType)
    {
        ...
        while ($arRes = $dbRes->Fetch()) {
            $result[] = array(
                ...
                'ENTITY_TYPE' => YNSIROwnerType::ResolveName($arRes['ENTITY_TYPE_ID']),
                'TYPE' => $communicationType,
                'VALUE' => $arRes['VALUE']
            );
        }
    ...
    public static function FindContactCommunications($needle, $communicationType, $top = 50)
    {
        ...
        global $DB;
        $fieldMultiTableName = YNSIRActivity::FIELD_MULTI_TABLE_NAME;
        ...
        if ($communicationType === '') {
            //Search by FULL_NAME
            $sql = "SELECT C.ID AS ELEMENT_ID, '' AS VALUE_TYPE, '' AS VALUE, C.NAME, C.SECOND_NAME, C.LAST_NAME, C.HONORIFIC, CO.TITLE COMPANY_TITLE FROM {$contactTableName} C LEFT OUTER JOIN {$companyTableName} CO ON C.COMPANY_ID = CO.ID WHERE C.FULL_NAME LIKE '{$DB->ForSqlLike('%'.$needle.'%')}'";
            if ($top > 0) {
            ...
            while ($arRes = $dbRes->Fetch()) {
                $result[] = CAllYNSIRActivity::ReadContactCommunication($arRes, $communicationType);
            }
        ...
        //Search by FULL_NAME
        $sql = "SELECT FM.ELEMENT_ID, FM.VALUE_TYPE, FM.VALUE, C.NAME, C.SECOND_NAME, C.LAST_NAME, C.HONORIFIC, CO.TITLE COMPANY_TITLE FROM {$fieldMultiTableName} FM INNER JOIN {$contactTableName} C ON FM.ELEMENT_ID = C.ID AND FM.ENTITY_ID = 'CONTACT' AND FM.TYPE_ID = '{$DB->ForSql($communicationType)}' AND C.FULL_NAME LIKE '{$DB->ForSqlLike('%'.$needle.'%')}' LEFT OUTER JOIN {$companyTableName} CO ON C.COMPANY_ID = CO.ID";
        if ($top > 0) {
            $sql = $DB->TopSql($sql, $top);
        }
    ...
    public static function FindCompanyCommunications($needle, $communicationType, $top = 50)
    {
        ...
        if ($communicationType === '') {
            ...
            while ($arRes = $dbRes->Fetch()) {
                $result[] = CAllYNSIRActivity::ReadCompanyCommunication($arRes, $communicationType);
            }
    ...
    public static function FindLeadCommunications($needle, $communicationType, $top = 50)
    {
        ...
        if ($communicationType === '') {
            ...
            while ($arRes = $dbRes->Fetch()) {
                $result[] = CAllYNSIRActivity::ReadLeadCommunication($arRes, $communicationType);
            }
    ...
    public static function GetCompanyCommunications($companyID, $communicationType)
    {
        ...
        while ($arRes = $dbRes->Fetch()) {
            $result[] = array(
                'ENTITY_ID' => $arRes['ELEMENT_ID'],
                'ENTITY_TYPE_ID' => YNSIROwnerType::Contact,
                'ENTITY_TYPE' => YNSIROwnerType::ResolveName(YNSIROwnerType::Contact),
                'TYPE' => $communicationType,
                'VALUE' => $arRes['VALUE'],
    ...
    {
        $ID = intval($ID);
        $dbRes = YNSIRActivity::GetList(array(), array('ID' => $ID), false, false, array('STORAGE_TYPE_ID'));
        $arRes = $dbRes->Fetch();
    ...
    public static function GetDefaultStorageTypeID()
    {
        if (self::$STORAGE_TYPE_ID === StorageType::Undefined) {
            self::$STORAGE_TYPE_ID = intval(CUserOptions::GetOption('ynsirecruitment', 'activity_storage_type_id', StorageType::Undefined));
            if (self::$STORAGE_TYPE_ID === StorageType::Undefined
    ...
    public static function SetDefaultStorageTypeID($storageTypeID)
    {
        ...
        CUserOptions::SetOption('ynsirecruitment', 'activity_storage_type_id', $storageTypeID);
    }
    ...
    public static function InjectUrnInMessage(&$messageData, $urn, $codeAllocation = false)
    {
        ...
        if ($codeAllocation === CCrmEMailCodeAllocation::Subject) {
            $messageData['SUBJECT'] = YNSIRActivity::InjectUrnInSubject(
                $urn,
            ...
            $messageData['BODY'] = YNSIRActivity::InjectUrnInBody(
                $urn,
    ...
    public static function ExtractUrnFromMessage(&$messageData, $codeAllocation = false)
    {
        ...
        $result = '';
        if ($codeAllocation === CCrmEMailCodeAllocation::Subject) {
            $result = YNSIRActivity::ExtractUrnFromSubject($subject);
            if ($result === '') {
                $result = YNSIRActivity::ExtractUrnFromBody($body);
            }
        } elseif ($codeAllocation === CCrmEMailCodeAllocation::Body) {
            $result = YNSIRActivity::ExtractUrnFromBody($body);
            if ($result === '') {
    ...
    public static function SynchronizeUserActivity($ownerTypeID, $ownerID, $userID)
    {
        ...
        if ($ownerTypeID <= YNSIROwnerType::Undefined || $ownerID <= 0) {
            return;
        }

        $arResult = YNSIRActivity::GetNearest($ownerTypeID, $ownerID, $userID);
        ...
        if ($activityID > 0 && $deadline !== '') {
            YNSIRActivity::DoSaveNearestUserActivity(
                array(
                    'USER_ID' => $userID,
                    'OWNER_ID' => $ownerID,
                    'OWNER_TYPE_ID' => $ownerTypeID,
            ...
            global $DB;
            $tableName = YNSIRActivity::USER_ACTIVITY_TABLE_NAME;
            $DB->Query(
    ...
    {
        if ($arFields['ATTENDEES_CODES'] != '')
        {
            $arFieldsAttendeesCode = explode(',', $arFields['ATTENDEES_CODES']);
        }
        ...
        $arCalEventFields = array(
            'CAL_TYPE' => 'user',
            'LOCATION' => $arFields['LOCATION'],//$arFields['LOCATION'],
            ...
            'IMPORTANCE' => YNSIRActivityPriority::ToCalendarEventImportance(
                isset($arFields['PRIORITY'])
                    ? intval($arFields['PRIORITY'])
                    : YNSIRActivityPriority::Low
            ),
            'DESCRIPTION' => isset($arFields['DESCRIPTION']) ? $arFields['DESCRIPTION'] : ''
        );
        //add ATTENDEE CODE
        $arCalEventFields['IS_MEETING'] = !empty($arFieldsAttendeesCode) && $arFieldsAttendeesCode != array('U'.$responsibleID);
        if ($arCalEventFields['IS_MEETING'])
        {
            $arCalEventFields['ATTENDEES_CODES'] = $arFieldsAttendeesCode;
//            array_walk($arCalEventFields['ATTENDEES_CODES'], function(&$item) { $item = 'U'.$item; });
            $arCalEventFields['ATTENDEES'] = CCalendar::GetDestinationUsers($arCalEventFields['ATTENDEES_CODES']);

            $arCalEventFields['MEETING_HOST'] = $responsibleID;
            $arCalEventFields['MEETING'] = array(
                'HOST_NAME' => CCalendar::GetUserName($arCalEventFields['MEETING_HOST']),
                'OPEN' => false,
                'NOTIFY' => true,
                'REINVITE' => false
            );
        ...
        if (isset($arFields['NOTIFY_TYPE']) && $arFields['NOTIFY_TYPE'] != YNSIRActivityNotifyType::None) {
            $arCalEventFields['REMIND'] = array(
                array(
                    'type' => YNSIRActivityNotifyType::ToCalendarEventRemind($arFields['NOTIFY_TYPE']),
        ...
        if ($entityID > 0) {
            if (!empty($arBindings)) {
                ...
                foreach ($arBindings as &$arBinding) {
                    // edit by gianglh
//                    $arUserFields[] = CUserTypeCrm::GetShortEntityType(YNSIROwnerType::ResolveName($arBinding['OWNER_TYPE_ID'])) . '_' . $arBinding['OWNER_ID'];
                }
                unset($arBinding);

                CCalendarEvent::UpdateUserFields(
                    $entityID,
                    array('UF_YNSIR_CAL_EVENT' => $arUserFields)
                );
            }
            ...
            if ($calendarEventId > 0 && $calendarEventId !== $entityID) {
                if ($provider::canKeepReassignedInCalendar($providerTypeId)) {
					...
					CCalendarEvent::UpdateUserFields($entityID, array('UF_YNSIR_CAL_EVENT' => null));
					*/
                } else {
    ...
    protected static function SetCalendarEventId($eventId, $activityId,&$newLocation)
    {
        global $DB;
        ...
        $toUpdate = array('CALENDAR_EVENT_ID' => $eventId,
            'LOCATION' => $arEventFields['LOCATION'] );
        $newLocation = $arEventFields['LOCATION'];
        /*
         * END edit by nhatth2
         */
        $DB->Query(
            'UPDATE ' . YNSIRActivity::TABLE_NAME . ' SET ' . $DB->PrepareUpdate(YNSIRActivity::TABLE_NAME, $toUpdate) . ' WHERE ID = ' . $activityId,
            false,
            'File: ' . __FILE__ . '<br>Line: ' . __LINE__
        );
    ...
    protected static function SaveTask(&$arFields)
    {
        ...
        if ($typeID !== YNSIRActivityType::Task || $associatedEntityID <= 0)
            return false;
    ...
    public static function RefreshCalendarBindings()
    {
        ...
        global $DB;
        $dbResult = $DB->Query(
            'SELECT OWNER_ID, OWNER_TYPE_ID, ASSOCIATED_ENTITY_ID FROM ' . YNSIRActivity::TABLE_NAME . ' WHERE OWNER_ID > 0 AND OWNER_TYPE_ID > 0 AND ASSOCIATED_ENTITY_ID > 0 AND TYPE_ID IN (' . YNSIRActivityType::Call . ', ' . YNSIRActivityType::Meeting . ')',
            false,
            'File: ' . __FILE__ . '<br>Line: ' . __LINE__
        );
        ...
        while ($arResult = $dbResult->Fetch()) {
            ...
            if ($ownerID > 0 && $ownerTypeID > 0 && $assocEntityID > 0) {
                CCalendarEvent::UpdateUserFields(
                    $assocEntityID,
                    array(
                        'UF_YNSIR_CAL_EVENT' => array(
                            CUserTypeCrm::GetShortEntityType(YNSIROwnerType::ResolveName($ownerTypeID)) . '_' . $ownerID
                        )
                    )
                );
            }
    ...
    public static function Notify(&$arFields, $schemeTypeID, $tag = '')
    {
        ...
        if ($schemeTypeID === YNSIRNotifierSchemeType::IncomingEmail) {
            $showUrl = YNSIROwnerType::GetShowUrl(
            ...
            if (!empty($communications)) {
                ...
                if (isset($comm['ENTITY_TYPE_ID']) && isset($comm['ENTITY_ID'])) {
                    $caption = YNSIROwnerType::GetCaption($comm['ENTITY_TYPE_ID'], $comm['ENTITY_ID'], false);
                }
                if ($caption === '') {
                    $caption = $comm['VALUE'];
                }

                $addresserShowUrl = YNSIROwnerType::GetShowUrl(
                    $comm['ENTITY_TYPE_ID'],
                    $comm['ENTITY_ID']
                );
            ...
            if ($addresserHtml === '') {
                $messageTemplate = GetMessage('YNSIR_ACTIVITY_NOTIFY_MESSAGE_INCOMING_EMAIL');
                return YNSIRNotifier::Notify(
                    $responsibleID,
                    str_replace(
                        '#VIEW_URL#',
            ...
            $messageTemplate = GetMessage('YNSIR_ACTIVITY_NOTIFY_MESSAGE_INCOMING_EMAIL_EXT');
            return YNSIRNotifier::Notify(
                $responsibleID,
                str_replace(
                    array(
    ...
    public static function GetCurrentQuantity($userID, $ownerTypeID)
    {
        ...
        $activityTable = YNSIRActivity::USER_ACTIVITY_TABLE_NAME;
    ...
    public static function GetDefaultCommunicationValue($ownerTypeID, $ownerID, $commType)
    {
        $dbMultiFields = CCrmFieldMulti::GetList(
            array('ID' => 'asc'),
            array('ENTITY_ID' => YNSIROwnerType::ResolveName($ownerTypeID), 'ELEMENT_ID' => $ownerID, 'TYPE_ID' => $commType)
        );
    ...
    private static function RegisterLiveFeedEvent(&$arFields)
    {
        ...
        $ownerTypeID = isset($arFields['OWNER_TYPE_ID']) ? intval($arFields['OWNER_TYPE_ID']) : YNSIROwnerType::Undefined;
        ...
        $liveFeedFields = array(
            'ENTITY_TYPE_ID' => YNSIROwnerType::Activity,
            'ENTITY_ID' => $ID,
            'USER_ID' => $userID,
            'MESSAGE' => '',
            'TITLE' => '',
        ...
        if(!empty($bindings))
        {
            ...
            foreach($bindings as &$binding)
            {
                $ownerTypeID = isset($binding['OWNER_TYPE_ID']) ? intval($binding['OWNER_TYPE_ID']) : YNSIROwnerType::Undefined;
                $ownerID = isset($binding['OWNER_ID']) ? intval($binding['OWNER_ID']) : 0;

                if (
                    $ownerTypeID != YNSIROwnerType::Undefined
                    && $ownerID > 0
                )
                {
                    $arOwners[] = array(
                ...
                if(
                    $ownerTypeID === YNSIROwnerType::Contact
                    && $ownerID > 0
                )
                {
                    $owners = array();
                    if(YNSIROwnerType::TryGetOwnerInfos(YNSIROwnerType::Contact, $ownerID, $owners, $ownerInfoOptions))
                    {
        ...
        if ($eventID === 0)
        {
            ...
            $eventID = YNSIRLiveFeed::CreateLogEvent($liveFeedFields, YNSIRLiveFeedEvent::Add, $arOptions);
        }
        ...
        else
        {
            ...
            )
            {
                ...
                $perms = YNSIRPerms_::GetUserPermissions($arFields["RESPONSIBLE_ID"]);
                foreach ($arOwners as $arOwner)
                {
                    if (YNSIRACtivity::CheckReadPermission($arOwner["OWNER_TYPE_ID"], $arOwner["OWNER_ID"], $perms))
                    {
...
//        if ($processBindings) {
//            YNSIRSonetRelation::UnRegisterRelationsByEntity(YNSIROwnerType::Activity, $activityID, array('QUICK' => $hasBindings));
//        }
//
//        $arOwners = ($hasBindings ? $bindings : self::GetBindings($activityID));
//
//        $slEntities = YNSIRLiveFeed::GetLogEvents(
//            array(),
//            array(
//                'ENTITY_TYPE_ID' => YNSIROwnerType::Activity,
...
//                        "LOG_ID" => $slID,
//                        "NOTIFY_EVENT" => "activity_add",
//                        "NOTIFY_TAG" => "YNSIRECRUITMENT|ACTIVITY|" . $activityID
//                    );
//
//                    if (intval($params['START_RESPONSIBLE_ID']) != $userID) {
//                        $bHasPermissions = false;
//                        $perms = YNSIRPerms_::GetUserPermissions($params['START_RESPONSIBLE_ID']);
//                        foreach ($arOwners as $arOwner) {
//                            if (YNSIRActivity::CheckReadPermission($arOwner["OWNER_TYPE_ID"], $arOwner["OWNER_ID"], $perms)) {
//                                $bHasPermissions = true;
//                                break;
...
//                        $bHasPermissions = false;
//                        $perms = YNSIRPerms_::GetUserPermissions($params['FINAL_RESPONSIBLE_ID']);
//                        foreach ($arOwners as $arOwner) {
//                            if (YNSIRActivity::CheckReadPermission($arOwner["OWNER_TYPE_ID"], $arOwner["OWNER_ID"], $perms)) {
//                                $bHasPermissions = true;
//                                break;
...
//                YNSIRSonetRelation::RegisterRelationBundle(
//                    $slID,
//                    $slEventType,
//                    YNSIROwnerType::Activity,
//                    $activityID,
...
//                        'ENTITY_TYPE_ID_KEY' => 'OWNER_TYPE_ID',
//                        'ENTITY_ID_KEY' => 'OWNER_ID',
//                        'TYPE_ID' => CYNSIRSonetRelationType::Ownership
//                    )
//                );
//            }
//        }
//        unset($slEntity);
    ...
    public static function OnBeforeIntantMessangerChatAdd(\Bitrix\Main\Entity\Event $event)
    {
        ...
        $m = null;
        if (preg_match('/^YNSIR_([A-Z]+)$/i', $entityType, $m) === 1) {
            $entityTypeName = isset($m[1]) ? $m[1] : '';
            $ownerTypeID = YNSIROwnerType::ResolveID($entityTypeName);
            $ownerID = isset($fields['ENTITY_ID']) ? intval($fields['ENTITY_ID']) : 0;
            $ownerInfo = null;
            if (YNSIROwnerType::IsDefined($ownerTypeID)
                && $ownerID > 0
                && YNSIROwnerType::TryGetInfo($ownerTypeID, $ownerID, $ownerInfo, false)
            ) {
    ...
    public static function LoadElementIDs($ID)
    {
        ...
        global $DB;
        $result = array();
        $table = YNSIRActivity::ELEMENT_TABLE_NAME;
    ...
    public static function GetEntityList($entityTypeID, $userID, $sortOrder, array $filter, $navParams = false, array $options = null)
    {
        ...
        } else if ($entityTypeID === YNSIROwnerType::Deal) {
            $lb = CCrmDeal::CreateListBuilder($fieldOptions);
        } else if ($entityTypeID === YNSIROwnerType::Contact) {
            $lb = CCrmContact::CreateListBuilder($fieldOptions);
        } else if ($entityTypeID === YNSIROwnerType::Company) {
        ...
        $join = 'LEFT JOIN ' . YNSIRActivity::USER_ACTIVITY_TABLE_NAME . ' UA ON UA.USER_ID IN (' . implode(',', $userIDs) . ') AND UA.OWNER_ID = ' . $entityAlias . '.ID AND UA.OWNER_TYPE_ID = ' . $entityTypeID;
    ...
    public static function GetAllDescriptions()
    {
        if (!self::$ALL_DESCRIPTIONS) {
            self::$ALL_DESCRIPTIONS = array(
                self::Undefined => '',
                self::Meeting => GetMessage('YNSIR_ACTIVITY_TYPE_MEETING'),
                self::Call => GetMessage('YNSIR_ACTIVITY_TYPE_CALL'),
                self::Task => GetMessage('YNSIR_ACTIVITY_TYPE_TASK'),
                self::Email => GetMessage('YNSIR_ACTIVITY_TYPE_EMAIL'),
                self::Activity => GetMessage('YNSIR_ACTIVITY_TYPE_ACTIVITY'),
                self::Provider => GetMessage('YNSIR_ACTIVITY_TYPE_PROVIDER')
            );
        }
...
class YNSIRActivityStatus
{
    ...
    public static function GetAllDescriptions()
    {
        if (!self::$ALL_DESCRIPTIONS) {
            self::$ALL_DESCRIPTIONS = array(
                self::Undefined => '',
                self::Waiting => GetMessage('YNSIR_ACTIVITY_STATUS_WAITING'),
                self::Completed => GetMessage('YNSIR_ACTIVITY_STATUS_COMPLETED'),
                self::AutoCompleted => GetMessage('YNSIR_ACTIVITY_STATUS_AUTO_COMPLETED')
            );
        }
...
class YNSIRActivityDirection
{
    ...
    public static function GetAllDescriptions($typeID = YNSIRActivityType::Undefined)
    {
        if (!isset(self::$ALL_DESCRIPTIONS[$typeID])) {
            $typeID = intval($typeID);

            $incomingID = 'YNSIR_ACTIVITY_DIRECTION_INCOMING';
            $outgoingID = 'YNSIR_ACTIVITY_DIRECTION_OUTGOING';

            if ($typeID === YNSIRActivityType::Email) {
                $incomingID = 'YNSIR_ACTIVITY_EMAIL_DIRECTION_INCOMING';
                $outgoingID = 'YNSIR_ACTIVITY_EMAIL_DIRECTION_OUTGOING';
            } elseif ($typeID === YNSIRActivityType::Call) {
                $incomingID = 'YNSIR_ACTIVITY_CALL_DIRECTION_INCOMING';
                $outgoingID = 'YNSIR_ACTIVITY_CALL_DIRECTION_OUTGOING';
            }
...
class YNSIRActivityPriority
{
    ...
    public static function GetAllDescriptions()
    {
        if (!self::$ALL_DESCRIPTIONS) {
            self::$ALL_DESCRIPTIONS = array(
                self::None => '',
                self::Low => GetMessage('YNSIR_PRIORITY_LOW'),
                self::Medium => GetMessage('YNSIR_PRIORITY_MEDIUM'),
                self::High => GetMessage('YNSIR_PRIORITY_HIGH')
            );
        }
    ...
    {
        $priorityID = intval($priorityID);
        if ($priorityID === YNSIRActivityPriority::Low) {
            return 'low';
        } elseif ($priorityID === YNSIRActivityPriority::High) {
            return 'high';
        }
    ...
    public static function FromCalendarEventImportance($importance)
    {
        ...
        if ($importance === '') {
            return YNSIRActivityPriority::Medium;
        }

        if ($importance === 'low') {
            return YNSIRActivityPriority::Low;
        } elseif ($importance === 'high') {
            return YNSIRActivityPriority::High;
        }
...
class YNSIRActivityNotifyType
{
    ...
    public static function GetAllDescriptions()
    {
        if (!self::$ALL_DESCRIPTIONS) {
            self::$ALL_DESCRIPTIONS = array(
                self::None => '',
                self::Min => GetMessage('YNSIR_NOTIFY_TYPE_MIN'),
                self::Hour => GetMessage('YNSIR_NOTIFY_TYPE_HOUR'),
                self::Day => GetMessage('YNSIR_NOTIFY_TYPE_DAY')
            );
        }
    ...
    public static function FromCalendarEventRemind($type)
    {
        ...
        if ($type === 'min') {
            return YNSIRActivityNotifyType::Min;
        } elseif ($type === 'hour') {
            return YNSIRActivityNotifyType::Hour;
        } elseif ($type === 'day') {
            return YNSIRActivityNotifyType::Day;
        }

        return YNSIRActivityNotifyType::None;
    }
}
 ...
 */
class YNSIRActivityStorageType
{
    const Undefined = 0;
    const File = 1;
    const WebDav = 2;
...
class YNSIRActivityEmailSender
{
    ...
    public static function TrySendEmail($ID, &$arFields, &$arErrors)
    {
        ...
        $typeID = isset($arFields['TYPE_ID']) ? intval($arFields['TYPE_ID']) : YNSIRActivityType::Undefined;
        if ($typeID !== YNSIRActivityType::Email) {
            $arErrors[] = array('CODE' => self::ERR_INVALID_DATA);
            return false;
        }
        ...
        if (!($urn !== ''
            && YNSIRActivity::Update($ID, array('URN' => $urn), false, false))
        ) {
        ...
        $subject = isset($arFields['SUBJECT']) ? $arFields['SUBJECT'] : '';
        $description = isset($arFields['DESCRIPTION']) ? $arFields['DESCRIPTION'] : '';
        $descriptionType = isset($arFields['DESCRIPTION_TYPE']) ? intval($arFields['DESCRIPTION_TYPE']) : YNSIRContentType::PlainText;
        ...
        YNSIRActivity::InjectUrnInMessage(
            $postingData,
            $urn,
        ...
        if (!YNSIRActivity::Update($ID, $arUpdateFields, false, false)) {
            $arErrors[] = array('CODE' => self::ERR_CANT_UPDATE_ACTIVITY);
            return false;
        }
        // <-- Creating Email
        ...
        if ($ownerID > 0 && $ownerTypeID > 0) {
            $eventText = '';
            $eventText .= GetMessage('YNSIR_ACTIVITY_EMAIL_SUBJECT') . ': ' . $subject . "\n\r";
            $eventText .= GetMessage('YNSIR_ACTIVITY_EMAIL_FROM') . ': ' . $from . "\n\r";
            $eventText .= GetMessage('YNSIR_ACTIVITY_EMAIL_TO') . ': ' . implode(',', $to) . "\n\r\n\r";
            $eventText .= $description;
            ...
            $YNSIREvent->Add(
                array(
                    'ENTITY' => array(
                        $ownerID => array(
                            'ENTITY_TYPE' => YNSIROwnerType::ResolveName($ownerTypeID),
                            'ENTITY_ID' => $ownerID
                        )
                    ),
                    'EVENT_ID' => 'MESSAGE',
...
class YNSIRActivityDbResult extends CDBResult
{
    private $selectFields = null;
    private $selectCommunications = false;

    function YNSIRActivityDbResult($res, $selectFields = array())
    {
    ...
    function Fetch()
    {
        if ($result = parent::Fetch()) {
            ...
            if ($this->selectCommunications) {
                $result['COMMUNICATIONS'] = YNSIRActivity::GetCommunications($result['ID']);
            }
        }
        return $result;
    }
