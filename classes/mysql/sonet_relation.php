 ...
 * YNSIRSonetRelation
 */
class YNSIRSonetRelation extends CAllYNSIRSonetRelation
{
	const TABLE_NAME = 'b_ynsir_sl_rel';
	const DB_TYPE = 'MYSQL';

	public function Register($logEntityID, $logEventID, $entityTypeID, $entityID, $parentEntityTypeID, $parentEntityID, $typeID = CYNSIRSonetRelationType::Ownership, $level = 1)
	{
		if(!YNSIROwnerType::IsDefined($entityTypeID) || !YNSIROwnerType::IsDefined($parentEntityTypeID))
		{
			return;
		}
		...
		if(!CYNSIRSonetRelationType::IsDefined($typeID))
		{
			$typeID = CYNSIRSonetRelationType::Ownership;
		}
		...
		$slParentEntityType = $DB->ForSql(YNSIRLiveFeedEntity::GetByEntityTypeID($parentEntityTypeID));
		$logEventID = $DB->ForSql($logEventID);

		CTimeZone::Disable();
		$logLastUpdateTime = YNSIRLiveFeed::GetLogEventLastUpdateTime($logEntityID, true);
	...
	public function RegisterBundle($logEntityID, $logEventID, $entityTypeID, $entityID, &$parents, $options = array())
	{
		...
		$defaultTypeID = isset($options['TYPE_ID']) ? intval($options['TYPE_ID']) : YNSIROwnerType::Undefined;
		if(!CYNSIRSonetRelationType::IsDefined($defaultTypeID))
		{
			$defaultTypeID = CYNSIRSonetRelationType::Ownership;
		}

		$items = array();
		$slEntityType = YNSIRLiveFeedEntity::GetByEntityTypeID($entityTypeID);
		$logLastUpdateTime = YNSIRLiveFeed::GetLogEventLastUpdateTime($logEntityID, false);
		foreach($parents as &$parent)
		{
			$parentEntityTypeID = isset($parent[$entityTypeIDKey]) ? intval($parent[$entityTypeIDKey]) : YNSIROwnerType::Undefined;
			$parentEntityID = isset($parent[$entityIDKey]) ? intval($parent[$entityIDKey]) : 0;

			if(!YNSIROwnerType::IsDefined($parentEntityTypeID) || $parentEntityID <= 0)
			{
				continue;
			}
			...
			$typeID = isset($parent['TYPE_ID']) ? intval($parent['TYPE_ID']) : CYNSIRSonetRelationType::Undefined;
			if(!CYNSIRSonetRelationType::IsDefined($typeID))
			{
			...
			$items[$key] = array(
				...
				'ENTITY_ID' => $entityID,
				'SL_PARENT_ENTITY_TYPE' => YNSIRLiveFeedEntity::GetByEntityTypeID($parentEntityTypeID),
	...
	public function Replace($entityTypeID, $entityID, $currentParent, $previousParent, $options = array())
	{
		...
		$currentParentEntityTypeID = isset($currentParent[$entityTypeIDKey]) ? intval($currentParent[$entityTypeIDKey]) : YNSIROwnerType::Undefined;
		$currentParentEntityID = isset($currentParent[$entityIDKey]) ? intval($currentParent[$entityIDKey]) : 0;

		$previousParentEntityTypeID = isset($previousParent[$entityTypeIDKey]) ? intval($previousParent[$entityTypeIDKey]) : YNSIROwnerType::Undefined;
		$previousParentEntityID = isset($previousParent[$entityIDKey]) ? intval($previousParent[$entityIDKey]) : 0;

		if(!YNSIROwnerType::IsDefined($currentParentEntityTypeID) || !YNSIROwnerType::IsDefined($previousParentEntityTypeID))
		{
			return false;
		}
		...
		$slEntityType = $DB->ForSql(YNSIRLiveFeedEntity::GetByEntityTypeID($entityTypeID));
		$currentSlParentEntityType = $DB->ForSql(YNSIRLiveFeedEntity::GetByEntityTypeID($currentParentEntityTypeID));
		$previousSlParentEntityType = $DB->ForSql(YNSIRLiveFeedEntity::GetByEntityTypeID($previousParentEntityTypeID));
	...
	public function UnRegisterByLogEntityID($logEntityID, $typeID = CYNSIRSonetRelationType::Undefined)
	{
		$logEntityID = intval($logEntityID);
		if($logEntityID <= 0)
		{
		...
		$deleteSql = CYNSIRSonetRelationType::IsDefined($typeID)
