...
if(typeof(BX.YNSIRUserSearchPopup) === "undefined")
{
	BX.YNSIRUserSearchPopup = function()
	{
		this._id = '';
	...
	BX.YNSIRUserSearchPopup.prototype =
	{
		...
		initialize: function(id, settings)
		{
			...
			{
				throw  "BX.YNSIRUserSearchPopup: 'search_input' is not defined!";
			}
			...
			{
				throw  "BX.YNSIRUserSearchPopup: 'data_input' is not defined!";
			}
			this._data_input = settings['dataInput'];

			if(!BX.type.isNotEmptyString(settings['componentName']))
			{
				throw  "BX.YNSIRUserSearchPopup: 'componentName' is not defined!";
			}
		...
		_initializeUserSelector: function()
		{
			...
			if(!window[objName])
			{
				...
				{
					throw "BX.YNSIRUserSearchPopup: Could not find '"+ objName +"' user selector!";
				}
	...
	BX.YNSIRUserSearchPopup.items = {};

	BX.YNSIRUserSearchPopup.create = function(id, settings, delay)
	{
		if(isNaN(delay))
		{
			delay = 0;
		...
		{
			window.setTimeout(
				function(){ BX.YNSIRUserSearchPopup.create(id, settings, 0); },
				delay
			);
			return null;
		}

		var self = new BX.YNSIRUserSearchPopup();
		self.initialize(id, settings);
		this.items[id] = self;
		return self;
	};

	BX.YNSIRUserSearchPopup.createIfNotExists = function(id, settings)
	{
		...
		{
			self = new BX.YNSIRUserSearchPopup();
			self.initialize(id, settings);
			this.items[id] = self;
		}
		return self;
	...
	BX.YNSIRUserSearchPopup.deletePopup = function(id)
	{
...
if(typeof(BX.YNSIRNotifier) === "undefined")
{
	BX.YNSIRNotifier = function()
	{
		this._sender = null;
		this._listeners = [];
	};

	BX.YNSIRNotifier.prototype =
	{
	...
	BX.YNSIRNotifier.create = function(sender)
	{
		var self = new BX.YNSIRNotifier();
...
if(typeof(BX.CmrSelectorMenuItem) === "undefined")
{
	...
	BX.CmrSelectorMenuItem.prototype =
	{
		initialize: function(settings)
		{
			...
			this._onSelectNotifier = BX.YNSIRNotifier.create(this);
...
if(typeof(BX.YNSIRSelector) === "undefined")
{
	BX.YNSIRSelector = function()
	{
		this._id = "";
		this._selectedValue = "";
		this._settings = {};
	...
	BX.YNSIRSelector.create = function(id, settings)
	{
		var self = new BX.YNSIRSelector();
		self.initialize(id, settings);
		this.items[self.getId()] = self;
		return self;
	};

	BX.YNSIRSelector.deleteItem = function(id)
	{
	if(this.items[id])
	{
	...
	BX.YNSIRSelector.items = {};
}

if(typeof(BX.YNSIRInterfaceFormUtil) === "undefined")
{
	BX.YNSIRInterfaceFormUtil = function(){};
	BX.YNSIRInterfaceFormUtil.disableThemeSelection = function(formId)
	{
	...
	BX.YNSIRInterfaceFormUtil.showFormRow = function(show, element)
	{
...
if(typeof(BX.YNSIRParamBag) === "undefined")
{
	BX.YNSIRParamBag = function()
	{
		this._params = {};
	};

	BX.YNSIRParamBag.prototype =
	{
	...
	BX.YNSIRParamBag.create = function(params)
	{
		var self = new BX.YNSIRParamBag();
		self.initialize(params);
		return self;
	}
}

if(typeof(BX.YNSIRSubscriber) === "undefined")
{
	BX.YNSIRSubscriber = function()
	{
		this._id = "";
		this._element = null;
		this._eventName = "";
	...
	BX.YNSIRSubscriber.prototype =
	{
		initialize: function(id, element, eventName, callback, settings)
		{
			...
			this._settings = settings ? settings : BX.YNSIRParamBag.create(null);
		},
	...
	BX.YNSIRSubscriber.items = {};
	BX.YNSIRSubscriber.create = function(id, element, eventName, callback, settings)
	{
		var self = new BX.YNSIRSubscriber();
		self.initialize(id, element, eventName, callback, settings);
		this.items[id] = self;
		return self;
	};

	BX.YNSIRSubscriber.subscribe = function(id, element, eventName, callback, settings)
	{
...
if(typeof(BX.YNSIRMultiFieldViewer) === "undefined")
{
	BX.YNSIRMultiFieldViewer = function()
	{
		this._id = '';
		this._shown = false;
		this._layout = '';
	...
	BX.YNSIRMultiFieldViewer.prototype =
	{
		initialize: function(id, settings)
		{
			this._id = id;
		...
		show: function()
		{
			...
			var dlg = BX.YNSIRMultiFieldViewer.dialogs[this._id] ? BX.YNSIRMultiFieldViewer.dialogs[this._id] : null;
			if(!dlg)
			{
					...
					anchor,
					{
						...
						events:
						{
							...
							onPopupClose: BX.delegate(
								...
								{
									this._shown = false;
									BX.YNSIRMultiFieldViewer.dialogs[this._id].destroy();
								},
								this
							),
							onPopupDestroy: BX.delegate(
								function()
								{
									delete(BX.YNSIRMultiFieldViewer.dialogs[this._id]);
								},
								this
							)
						},
				...
				BX.YNSIRMultiFieldViewer.dialogs[this._id] = dlg;
			}
		...
		close: function()
		{
			if(this._shown && typeof(BX.YNSIRMultiFieldViewer.dialogs[this._id]) !== 'undefined')
			{
				BX.YNSIRMultiFieldViewer.dialogs[this._id].close();
			}
		}
	};
	BX.YNSIRMultiFieldViewer.items = {};
	BX.YNSIRMultiFieldViewer.create = function(id, settings)
	{
		var self = new BX.YNSIRMultiFieldViewer();
		self.initialize(id, settings);
		this.items[id] = self;
		return self;
	};
	BX.YNSIRMultiFieldViewer.ensureCreated = function(id, settings)
	{
		return this.items[id] ? this.items[id] : this.create(id, settings);
	};
	BX.YNSIRMultiFieldViewer.dialogs = {};
}

if(typeof(BX.YNSIRSipManager) === "undefined")
{
	BX.YNSIRSipManager = function()
	{
		this._id = "";
		this._settings = null;
		this._serviceUrls = {};
	...
	BX.YNSIRSipManager.prototype =
	{
		...
		openPreCallDialog: function(recipient, params, anchor, callback)
		{
			...
			var dlg = BX.YNSIRPreCallDialog.create(dlgId,
		...
		makeCall: function(recipient, params)
		{
			...
			if(!(entityTypeName !== "" && entityId > 0))
			{
				...
				if(entityTypeName !== "")
				{
					entityTypeName = "YNSIR_" + entityTypeName.toUpperCase();
				}
			...
			if(enablePreCallDialog)
			{
				...
				if(enableInfoLoading)
				{
					...
					else
					{
						...
						if(serviceUrl !== "")
						{
							var loader = BX.YNSIRSipRecipientInfoLoader.create(
								BX.YNSIRParamBag.create(
									{
		...
		{
			return BX.YNSIRSipManager.messages && BX.YNSIRSipManager.messages.hasOwnProperty(name) ? BX.YNSIRSipManager.messages[name] : "";
		},
	...
	BX.YNSIRSipManager.items = {};
	BX.YNSIRSipManager.create = function(id, settings)
	{
		var self = new BX.YNSIRSipManager();
		self.initialize(id, settings);
		this.items[id] = self;
		return self;
	};
	BX.YNSIRSipManager.current = null;
	BX.YNSIRSipManager.getCurrent = function()
	{
		if(!this._current)
		{
	...
	BX.YNSIRSipManager.startCall = function(recipient, params, enablePreCallDialog, anchor)
	{
		this.getCurrent().startCall(recipient, params, enablePreCallDialog, anchor);
	};
	BX.YNSIRSipManager.resolveSipEntityTypeName = function(typeName)
	{
		return BX.type.isNotEmptyString(typeName) ? ("YNSIR_" + typeName.toUpperCase()) : "";
	}
}

if(typeof(BX.YNSIRSipRecipientInfoLoader) === "undefined")
{
	BX.YNSIRSipRecipientInfoLoader = function()
	{
		this._settings = null;
	...
	BX.YNSIRSipRecipientInfoLoader.prototype =
	{
		initialize: function(settings)
		{
			this._settings = settings ? settings : BX.YNSIRParamBag.create(null);
	...
	BX.YNSIRSipRecipientInfoLoader.create = function(settings)
	{
		var self = new BX.YNSIRSipRecipientInfoLoader();
		self.initialize(settings);
		return self;
	};
}

if(typeof(BX.YNSIRPreCallDialog) === "undefined")
{
	BX.YNSIRPreCallDialog = function()
	{
		this._id = "";
		this._settings = null;
		this._recipient = null;
	...
	BX.YNSIRPreCallDialog.prototype =
	{
		initialize: function(id, settings)
		{
			this._id = id;
	...
	BX.YNSIRPreCallDialog.create = function(id, settings)
	{
		var self = new BX.YNSIRPreCallDialog();
		self.initialize(id, settings);
		return self;
	};
}

if(typeof(BX.YNSIRBizprocDispatcher) == "undefined")
{
	BX.YNSIRBizprocDispatcher = function()
	{
		this._id = "";
		this._settings = {};
	...
	BX.YNSIRBizprocDispatcher.prototype =
	{
		initialize: function(id, settings)
		{
			...
			if(!this._container)
			{
				throw "BX.YNSIRBizprocDispatcher. Could not find container.";
			}
			...
			{
				BX.addCustomEvent(window, "YNSIRInterfaceFormCreated", this._formManagerHandler);
			}
		...
		setFormManager: function(formManager)
		{
			...
			{
				BX.addCustomEvent(window, 'BX_YNSIR_INTERFACE_FORM_TAB_SELECTED', BX.delegate(this._onFormTabSelect, this));
			}
			else
			{
		...
		_onFormManagerCreate: function(formManager)
		{
			...
			{
				BX.removeCustomEvent(window, "YNSIRInterfaceFormCreated", this._formManagerHandler);
				this.setFormManager(formManager);
			}
		},
	...
	BX.YNSIRBizprocDispatcher.items = {};
	BX.YNSIRBizprocDispatcher.create = function(id, settings)
	{
		var self = new BX.YNSIRBizprocDispatcher();
		self.initialize(id, settings);
		this.items[self.getId()] = self;
		return self;
	};
...
if(typeof(BX.YNSIREntityTreeDispatcher) == 'undefined')
{
	BX.YNSIREntityTreeDispatcher = function()
	{
		this._id = '';
		this._settings = {};
		this._container = null;
	...
	BX.YNSIREntityTreeDispatcher.prototype =
	{
		initialize: function(id, settings)
		{
			...
			if(!this._container)
			{
				throw 'BX.YNSIREntityTreeDispatcher. Could not find container.';
			}
			this._wrapper = BX.findParent(this._container, { 'tagName': 'DIV', 'className': 'bx-edit-tab-inner' });
			...
			if(!BX.type.isNotEmptyString(this._serviceUrl))
			{
				throw 'BX.YNSIREntityTreeDispatcher. Could not find service url.';
			}

			this._entityTypeName = this.getSetting('entityTypeName', '');
			if(!BX.type.isNotEmptyString(this._entityTypeName))
			{
				throw 'BX.YNSIREntityTreeDispatcher. Could not find entity type name.';
			}

			this._entityId = parseInt(this.getSetting('entityID', 0));
			if(!BX.type.isNumber(this._entityId) || this._entityId <= 0)
			{
				throw 'BX.YNSIREntityTreeDispatcher. Could not find entity id.';
			}

			this._formId = this.getSetting('formID', '');
			if(!BX.type.isNotEmptyString(this._formId))
			{
				throw 'BX.YNSIREntityTreeDispatcher. Could not find form id.';
			}
	...
	BX.YNSIREntityTreeDispatcher.items = {};
	BX.YNSIREntityTreeDispatcher.create = function(id, settings)
	{
		var self = new BX.YNSIREntityTreeDispatcher();
		self.initialize(id, settings);
		this.items[self.getId()] = self;
		return self;
	};
...
if(typeof(BX.YNSIRLongRunningProcessState) == "undefined")
{
	BX.YNSIRLongRunningProcessState =
	{
		intermediate: 0,
		running: 1,
		completed: 2,
...
if(typeof(BX.YNSIRLongRunningProcessDialog) == "undefined")
{
	BX.YNSIRLongRunningProcessDialog = function()
	{
		this._id = "";
		this._settings = {};
		this._serviceUrl = "";
		...
		this._summary = null;
		this._isShown = false;
		this._state = BX.YNSIRLongRunningProcessState.intermediate;
		this._cancelRequest = false;
		this._requestIsRunning = false;
	};
	BX.YNSIRLongRunningProcessDialog.prototype =
	{
		initialize: function(id, settings)
		{
			...
			if(!BX.type.isNotEmptyString(this._serviceUrl))
			{
				throw "BX.YNSIRLongRunningProcess. Could not find service url.";
			}

			this._action = this.getSetting("action", "");
			if(!BX.type.isNotEmptyString(this._action))
			{
				throw "BX.YNSIRLongRunningProcess. Could not find action.";
			}
		...
		getMessage: function(name)
		{
			return BX.YNSIRLongRunningProcessDialog.messages && BX.YNSIRLongRunningProcessDialog.messages.hasOwnProperty(name) ? BX.YNSIRLongRunningProcessDialog.messages[name] : "";
		},
		getState: function()
		{
			return this._state;
		...
		start: function()
		{
			if(this._state === BX.YNSIRLongRunningProcessState.intermediate || this._state === BX.YNSIRLongRunningProcessState.stoped)
			{
				this._startRequest();
			}
		},
		stop: function()
		{
			if(this._state === BX.YNSIRLongRunningProcessState.running)
			{
		...
		_onDialogClose: function(e)
		{
			...
			this._setState(BX.YNSIRLongRunningProcessState.intermediate);
			this._buttons = {};
			this._summary = null;
		...
		_setState: function(state)
		{
			...
			this._state = state;
			if(state === BX.YNSIRLongRunningProcessState.intermediate || state === BX.YNSIRLongRunningProcessState.stoped)
			{
				this._lockButton("start", false);
				this._lockButton("stop", true);
				this._showButton("close", true);
			}
			else if(state === BX.YNSIRLongRunningProcessState.running)
			{
				this._lockButton("start", true);
				this._lockButton("stop", false);
				this._showButton("close", false);
			}
			else if(state === BX.YNSIRLongRunningProcessState.completed || state === BX.YNSIRLongRunningProcessState.error)
			{
				this._lockButton("start", true);
				this._lockButton("stop", true);
				this._showButton("close", true);
		...
		_startRequest: function()
		{
			...
			this._setState(BX.YNSIRLongRunningProcessState.running);
			BX.ajax(
				{
					url: this._serviceUrl,
					method: "POST",
		...
		_onRequestSuccsess: function(result)
		{
			...
			if(!result)
			{
				...
				this._setState(BX.YNSIRLongRunningProcessState.error);
				return;
			}
			...
			if(status === "PROGRESS")
			{
				...
				{
					this._setState(BX.YNSIRLongRunningProcessState.stoped);
					this._cancelRequest = false;
				}
				else
				{
			...
			{
				this._setState(BX.YNSIRLongRunningProcessState.completed);
				if(summary !== "")
				{
					this._setSummary(summary);
				}
	...
	if(typeof(BX.YNSIRLongRunningProcessDialog.messages) == "undefined")
	{
		BX.YNSIRLongRunningProcessDialog.messages = {};
	}
	BX.YNSIRLongRunningProcessDialog.items = {};
	BX.YNSIRLongRunningProcessDialog.create = function(id, settings)
	{
		var self = new BX.YNSIRLongRunningProcessDialog();
		self.initialize(id, settings);
		this.items[self.getId()] = self;
		return self;
	};
}
if(typeof(BX.YNSIREntityType) == "undefined")
{
	BX.YNSIREntityType = function()
	{
	};

	BX.YNSIREntityType.enumeration =
	{
		undefined: 0,
		lead: 1,
		deal: 2,
	...
	BX.YNSIREntityType.abbreviations =
	{
		undefined: "",
		lead: "L",
		deal: "D",
	...
	BX.YNSIREntityType.isDefined = function(typeId)
	{
	...
	BX.YNSIREntityType.resolveName = function(typeId)
	{
		if(!BX.type.isNumber(typeId))
		{
			typeId = parseInt(typeId);
		...
		if(typeId === BX.YNSIREntityType.enumeration.lead)
		{
			return BX.YNSIREntityType.names.lead;
		}
		else if(typeId === BX.YNSIREntityType.enumeration.deal)
		{
			return BX.YNSIREntityType.names.deal;
		}
		else if(typeId === BX.YNSIREntityType.enumeration.contact)
		{
			return BX.YNSIREntityType.names.contact;
		}
		else if(typeId === BX.YNSIREntityType.enumeration.company)
		{
			return BX.YNSIREntityType.names.company;
		}
		else if(typeId === BX.YNSIREntityType.enumeration.invoice)
		{
			return BX.YNSIREntityType.names.invoice;
		}
		else
		{
			return "";
	...
	BX.YNSIREntityType.resolveId = function(name)
	{
		name = name.toUpperCase();
		if(name === BX.YNSIREntityType.names.lead)
		{
			return this.enumeration.lead;
		}
		else if(name === BX.YNSIREntityType.names.deal)
		{
			return this.enumeration.deal;
		}
		else if(name === BX.YNSIREntityType.names.contact)
		{
			return this.enumeration.contact;
		}
		else if(name === BX.YNSIREntityType.names.company)
		{
			return this.enumeration.company;
		}
		else if(name === BX.YNSIREntityType.names.invoice)
		{
	...
	BX.YNSIREntityType.resolveAbbreviation = function(name)
	{
		name = name.toUpperCase();
		if(name === BX.YNSIREntityType.names.lead)
		{
	...
	BX.YNSIREntityType.verifyName = function(name)
	{
	...
	BX.YNSIREntityType.setCaptions = function(captions)
	{
	...
	BX.YNSIREntityType.prepareEntityKey = function(entityTypeName, entityId)
	{
		var abbr = this.resolveAbbreviation(entityTypeName);
		return abbr !== "" ? (abbr + "_" + entityId.toString()) : "";
	};

	if(typeof(BX.YNSIREntityType.captions) === "undefined")
	{
		BX.YNSIREntityType.captions = {};
	}
	if(typeof(BX.YNSIREntityType.categoryCaptions) === "undefined")
	{
		BX.YNSIREntityType.categoryCaptions = {};
	}
}
if(typeof(BX.YNSIRDuplicateManager) == "undefined")
{
	BX.YNSIRDuplicateManager = function()
	{
		this._id = "";
		this._settings = {};
		this._entityTypeName = "";
	...
	BX.YNSIRDuplicateManager.prototype =
	{
		initialize: function(id, settings)
		{
			...
			if(!BX.type.isNotEmptyString(this._entityTypeName))
			{
				throw "BX.YNSIRDuplicateManager. Could not find entity type name.";
			}
		...
		getMessage: function(name)
		{
			return BX.YNSIRDuplicateManager.messages && BX.YNSIRDuplicateManager.messages.hasOwnProperty(name) ? BX.YNSIRDuplicateManager.messages[name] : "";
		},
		rebuildIndex: function()
		{
			...
			{
				throw "BX.YNSIRDuplicateManager. Could not find service url.";
			}
	...
	if(typeof(BX.YNSIRDuplicateManager.messages) == "undefined")
	{
		BX.YNSIRDuplicateManager.messages = {};
	}
	BX.YNSIRDuplicateManager.items = {};
	BX.YNSIRDuplicateManager.create = function(id, settings)
	{
		var self = new BX.YNSIRDuplicateManager();
		self.initialize(id, settings);
		this.items[self.getId()] = self;
		return self;
	};
}
if(typeof(BX.YNSIRDupController) == "undefined")
{
	BX.YNSIRDupController = function()
	{
		this._id = "";
		this._settings = {};
		this._entityTypeName = "";
	...
	BX.YNSIRDupController.prototype =
	{
		initialize: function(id, settings)
		{
			...
			if(!BX.type.isNotEmptyString(this._serviceUrl))
			{
				throw "BX.YNSIRDupController. Could not find service url.";
			}
			...
			if(groups)
			{
				for(var key in groups)
				{
					...
					try
					{
						if(type === "single")
						{
							ctrl = BX.YNSIRDupCtrlSingleField.create(key, group);
						}
						else if(type === "fullName")
						{
							ctrl = BX.YNSIRDupCtrlFullName.create(key, group);
						}
						else if(type === "communication")
						{
							ctrl = BX.YNSIRDupCtrlCommunication.create(key, group);
						}
					}
					catch(ex)
					{
		...
		_openWarningDialog: function()
		{
			...
			{
				this._warningDialog = BX.YNSIRDuplicateWarningDialog.create(
					this._id + "_warn",
					{
						"controller": this,
		...
		_showSearchSummary: function(anchorField)
		{
			...
			this._searchSummary = BX.YNSIRDuplicateSummaryPopup.create(
				this._id + "_summary",
				{
					"controller": this,
					"anchor": anchor
	...
	BX.YNSIRDupController.create = function(id, settings)
	{
		var self = new BX.YNSIRDupController();
		self.initialize(id, settings);
		return self;
	};
}
if(typeof(BX.YNSIRDupCtrlField) == "undefined")
{
	BX.YNSIRDupCtrlField = function()
	{
		this._id = "";
		this._group = null;
		this._element = null;
	...
	BX.YNSIRDupCtrlField.prototype =
	{
		initialize: function(id, element, elementTitle)
		{
			if(!BX.type.isNotEmptyString(id))
			{
				throw "BX.YNSIRDupCtrlField. Invalid parameter 'id': is not defined.";
			}
			this._id = id;

			if(!BX.type.isElementNode(element))
			{
				throw "BX.YNSIRDupCtrlField. Invalid parameter 'element': is not defined.";
			}
			this._element = element;
	...
	BX.YNSIRDupCtrlField.create = function(id, element, elementTitle)
	{
		var self = new BX.YNSIRDupCtrlField();
		self.initialize(id, element, elementTitle);
		return self;
	}
}
if(typeof(BX.YNSIRDupCtrlFieldGroup) == "undefined")
{
	BX.YNSIRDupCtrlFieldGroup = function()
	{
		this._id = "";
		this._settings = {};
	...
	BX.YNSIRDupCtrlFieldGroup.prototype =
	{
		initialize: function(id, settings)
		{
			if(!BX.type.isNotEmptyString(id))
			{
				throw "BX.YNSIRDupCtrlFieldGroup. Invalid parameter 'id': is not defined.";
			}
			this._id = id;
...
if(typeof(BX.YNSIRDupCtrlSingleField) == "undefined")
{
	BX.YNSIRDupCtrlSingleField = function()
	{
		BX.YNSIRDupCtrlSingleField.superclass.constructor.apply(this);
		this._paramName = "";
		this._field = null;
	};
	BX.extend(BX.YNSIRDupCtrlSingleField, BX.YNSIRDupCtrlFieldGroup);
	BX.YNSIRDupCtrlSingleField.prototype._afterInitialize = function()
	{
		this._paramName = this.getSetting("parameterName", "");
		if(!BX.type.isNotEmptyString(this._paramName))
		{
			throw "BX.YNSIRDupCtrlSingleField. Could not find parameter name.";
		}

		var element = BX(this.getSetting("element", null));
		if(BX.type.isDomNode(element))
		{
			this._field = this.addField(BX.YNSIRDupCtrlField.create(this._paramName, element, BX(this.getSetting("elementCaption", null))));
		}
	};
	BX.YNSIRDupCtrlSingleField.prototype.getValue = function()
	{
		return this._field ? BX.util.trim(this._field.getValue()) : "";
	};
	BX.YNSIRDupCtrlSingleField.prototype.prepareSearchParams = function()
	{
	...
	BX.YNSIRDupCtrlSingleField.prototype.getSearchHashCode = function()
	{
		var value = this.getValue();
		if(value === "")
		{
	...
	BX.YNSIRDupCtrlSingleField.prototype.getDefaultSearchSummaryFieldId = function()
	{
		return this._field ? this._field.getId() : ""
	};
	BX.YNSIRDupCtrlSingleField.prototype.processFieldDelay = function(field)
	{
		this._fireChangeEvent(field);
	};
	BX.YNSIRDupCtrlSingleField.prototype.processFieldFocusLoss = function(field)
	{
		this._fireChangeEvent(field);
	};
	BX.YNSIRDupCtrlSingleField.prototype._fireChangeEvent = function(field)
	{
		if(this._controller)
		{
	...
	BX.YNSIRDupCtrlSingleField.create = function(id, settings)
	{
		var self = new BX.YNSIRDupCtrlSingleField();
		self.initialize(id, settings);
		return self;
	};
}
if(typeof(BX.YNSIRDupCtrlFullName) == "undefined")
{
	BX.YNSIRDupCtrlFullName = function()
	{
		BX.YNSIRDupCtrlFullName.superclass.constructor.apply(this);
		this._nameField = null;
		this._secondNameField = null;
		this._lastNameField = null;
	};

	BX.extend(BX.YNSIRDupCtrlFullName, BX.YNSIRDupCtrlFieldGroup);
	BX.YNSIRDupCtrlFullName.prototype._afterInitialize = function()
	{
		var element = BX(this.getSetting("name", null));
		if(BX.type.isDomNode(element))
		{
			this._nameField = this.addField(BX.YNSIRDupCtrlField.create("NAME", element, BX(this.getSetting("nameCaption", null))));
		}
		element = BX(this.getSetting("secondName", null));
		if(BX.type.isDomNode(element))
		{
			this._secondNameField = this.addField(BX.YNSIRDupCtrlField.create("SECOND_NAME", element, BX(this.getSetting("secondNameCaption", null))));
		}
		element = BX(this.getSetting("lastName", null));
		if(BX.type.isDomNode(element))
		{
			this._lastNameField = this.addField(BX.YNSIRDupCtrlField.create("LAST_NAME", element, BX(this.getSetting("lastNameCaption", null))));
		}
	};
	BX.YNSIRDupCtrlFullName.prototype.getName = function()
	{
		return this._nameField ? BX.util.trim(this._nameField.getValue()) : "";
	};
	BX.YNSIRDupCtrlFullName.prototype.getSecondName = function()
	{
		return this._secondNameField ? BX.util.trim(this._secondNameField.getValue()) : "";
	};
	BX.YNSIRDupCtrlFullName.prototype.getLastName = function()
	{
		return this._lastNameField ? BX.util.trim(this._lastNameField.getValue()) : "";
	};
	BX.YNSIRDupCtrlFullName.prototype.prepareSearchParams = function()
	{
	...
	BX.YNSIRDupCtrlFullName.prototype.getSearchHashCode = function()
	{
		var lastName = this.getLastName();
		if(lastName === "")
		{
	...
	BX.YNSIRDupCtrlFullName.prototype.getDefaultSearchSummaryFieldId = function()
	{
		return this._lastNameField ? this._lastNameField.getId() : ""
	};
	BX.YNSIRDupCtrlFullName.prototype.processFieldDelay = function(field)
	{
		this._fireChangeEvent(field);
	};
	BX.YNSIRDupCtrlFullName.prototype.processFieldFocusLoss = function(field)
	{
		this._fireChangeEvent(field);
	};
	BX.YNSIRDupCtrlFullName.prototype._fireChangeEvent = function(field)
	{
		if(this._controller)
		{
			this._controller.processGroupChange(this, field);
	...
	BX.YNSIRDupCtrlFullName.create = function(id, settings)
	{
		var self = new BX.YNSIRDupCtrlFullName();
		self.initialize(id, settings);
		return self;
	};
}
if(typeof(BX.YNSIRDupCtrlCommunication) == "undefined")
{
	BX.YNSIRDupCtrlCommunication = function()
	{
		...
		BX.YNSIRDupCtrlCommunication.superclass.constructor.apply(this);
	};
	BX.extend(BX.YNSIRDupCtrlCommunication, BX.YNSIRDupCtrlFieldGroup);
	BX.YNSIRDupCtrlCommunication.prototype._afterInitialize = function()
	{
		this._communicationType = this.getSetting("communicationType", "");
		if(!BX.type.isNotEmptyString(this._communicationType))
		{
			throw "BX.YNSIRDupCtrlCommunication. Could not find communication type.";
		}

		this._editorId = this.getSetting("editorId", "");
		if(!BX.type.isNotEmptyString(this._editorId))
		{
			throw "BX.YNSIRDupCtrlCommunication. Could not find editor Id.";
		}
		...
		if(!BX.type.isElementNode(this._container))
		{
			throw "BX.YNSIRDupCtrlCommunication. Could not find container.";
		}

		BX.addCustomEvent(window, "YNSIRFieldMultiEditorItemCreated", this._editorCreateItemHandler);
		BX.addCustomEvent(window, "YNSIRFieldMultiEditorItemDeleted", this._editorDeleteItemHandler);

		this._initializeFields();
	};
	BX.YNSIRDupCtrlCommunication.prototype._initializeFields = function()
	{
		this.clearFields();
		...
		{
			var field = this.addField(BX.YNSIRDupCtrlField.create("VALUE_" + (i + 1).toString(), inputs[i], caption));
			if(i === 0)
			{
	...
	BX.YNSIRDupCtrlCommunication.prototype.prepareSearchParams = function()
	{
	...
	BX.YNSIRDupCtrlCommunication.prototype.getSearchHashCode = function()
	{
		var values = this.getFieldValues();
		return (values.length > 0 ? BX.util.hashCode(values.join("$")) : 0);
	};
	BX.YNSIRDupCtrlCommunication.prototype.getDefaultSearchSummaryFieldId = function()
	{
		return this._firstField ? this._firstField.getId() : ""
	};
	BX.YNSIRDupCtrlCommunication.prototype.processFieldDelay = function(field)
	{
		if(this._controller)
		{
			this._controller.processGroupChange(this, field);
	...
	BX.YNSIRDupCtrlCommunication.prototype.processFieldFocusLoss = function(field)
	{
		if(this._controller)
		{
			this._controller.processGroupChange(this, field);
	...
	BX.YNSIRDupCtrlCommunication.prototype.onCommunicaionEditorItemCreate = function(sender, editorId)
	{
	...
	BX.YNSIRDupCtrlCommunication.prototype.onCommunicaionEditorItemDelete = function(sender, editorId)
	{
		if(this._editorId !== editorId)
		{
			return;
	...
	BX.YNSIRDupCtrlCommunication.create = function(id, settings)
	{
		var self = new BX.YNSIRDupCtrlCommunication();
		self.initialize(id, settings);
		return self;
	};
}
if(typeof(BX.YNSIRDuplicateSummaryItem) == "undefined")
{
	BX.YNSIRDuplicateSummaryItem = function()
	{
		this._id = "";
		this._settings = {};
		this._groupId = "";
	...
	BX.YNSIRDuplicateSummaryItem.prototype =
	{
		initialize: function(id, settings)
		{
			this._id = id;
			...
			if(!this._controller)
			{
				throw "BX.YNSIRDuplicateListPopup. Parameter 'controller' is not found.";
			}

			this._container = this.getSetting("container", null);
			if(!this._controller)
			{
				throw "BX.YNSIRDuplicateSummaryItem. Parameter 'container' is not found.";
			}

			this._link = this.getSetting("link", null);
			if(!this._link)
			{
				throw "BX.YNSIRDuplicateSummaryItem. Parameter 'link' is not found.";
			}
		...
		_onLinkClick: function(e)
		{
			if(this._groupId !== "")
			{
				var popup = BX.YNSIRDuplicateListPopup.create(
					this._id,
					{
	...
	BX.YNSIRDuplicateSummaryItem.create = function(id, settings)
	{
		var self = new BX.YNSIRDuplicateSummaryItem();
		self.initialize(id, settings);
		return self;
	};
}
if(typeof(BX.YNSIRDuplicateSummaryPopup) == "undefined")
{
	BX.YNSIRDuplicateSummaryPopup = function()
	{
		this._id = "";
		this._settings = {};
		this._controller = null;
	...
	BX.YNSIRDuplicateSummaryPopup.prototype =
	{
		initialize: function(id, settings)
		{
			this._id = id;
			...
			if(!this._controller)
			{
				throw "BX.YNSIRDuplicateSummaryPopup. Parameter 'controller' is not found.";
			}
		},
		getSetting: function (name, defaultval)
		{
		...
		show: function()
		{
			...
			var id = this.getId();
			if(BX.YNSIRDuplicateSummaryPopup.windows[id])
			{
				BX.YNSIRDuplicateSummaryPopup.windows[id].destroy();
			}
			...
			BX.YNSIRDuplicateSummaryPopup.windows[id] = this._popup;
			this._popup.show();
		...
		getMessage: function(name)
		{
			return BX.YNSIRDuplicateSummaryPopup.messages && BX.YNSIRDuplicateSummaryPopup.messages.hasOwnProperty(name) ? BX.YNSIRDuplicateSummaryPopup.messages[name] : "";
		},
		_prepareContent: function()
		{
			...
			for(groupId in infos)
			{
				...
				this._items[groupId] = BX.YNSIRDuplicateSummaryItem.create(
					groupId,
					{
						controller: this._controller,
	...
	BX.YNSIRDuplicateSummaryPopup.windows = {};
	if(typeof(BX.YNSIRDuplicateSummaryPopup.messages) == "undefined")
	{
		BX.YNSIRDuplicateSummaryPopup.messages = {};
	}
	BX.YNSIRDuplicateSummaryPopup.create = function(id, settings)
	{
		var self = new BX.YNSIRDuplicateSummaryPopup();
		self.initialize(id, settings);
		return self;
	};
}
if(typeof(BX.YNSIRDuplicateWarningDialog) == "undefined")
{
	BX.YNSIRDuplicateWarningDialog = function()
	{
		this._id = "";
		this._settings = {};
		this._controller = null;
	...
	BX.YNSIRDuplicateWarningDialog.prototype =
	{
		initialize: function(id, settings)
		{
			this._id = id;
			...
			if(!this._controller)
			{
				throw "BX.YNSIRDuplicateWarningDialog. Parameter 'controller' is not found.";
			}
		},
		getSetting: function (name, defaultval)
		{
		...
		show: function()
		{
			...
			var id = this.getId();
			if(BX.YNSIRDuplicateWarningDialog.windows[id])
			{
				BX.YNSIRDuplicateWarningDialog.windows[id].destroy();
			}
			...
			BX.YNSIRDuplicateWarningDialog.windows[id] = this._popup;
			this._popup.show();
		...
		getMessage: function(name)
		{
			return BX.YNSIRDuplicateWarningDialog.messages && BX.YNSIRDuplicateWarningDialog.messages.hasOwnProperty(name) ? BX.YNSIRDuplicateWarningDialog.messages[name] : "";
		},
		_prepareContent: function()
		{
			this._contentWrapper = BX.YNSIRDuplicateRenderer.prepareListContent(this._controller.getDuplicateData());
	...
	BX.YNSIRDuplicateWarningDialog.windows = {};
	if(typeof(BX.YNSIRDuplicateWarningDialog.messages) === "undefined")
	{
		BX.YNSIRDuplicateWarningDialog.messages = {};
	}
	BX.YNSIRDuplicateWarningDialog.create = function(id, settings)
	{
		var self = new BX.YNSIRDuplicateWarningDialog();
		self.initialize(id, settings);
		return self;
	};
}
if(typeof(BX.YNSIRDuplicateListPopup) === "undefined")
{
	BX.YNSIRDuplicateListPopup = function()
	{
		this._id = "";
		this._settings = {};
		this._controller = null;
	...
	BX.YNSIRDuplicateListPopup.prototype =
	{
		initialize: function(id, settings)
		{
			this._id = id;
		...
		show: function()
		{
			...
			var id = this.getId();
			if(BX.YNSIRDuplicateListPopup.windows[id])
			{
				BX.YNSIRDuplicateListPopup.windows[id].destroy();
			}
			...
			BX.YNSIRDuplicateListPopup.windows[id] = this._popup;
			this._popup.show();
		},
		close: function()
		{
		...
		getMessage: function(name)
		{
			return BX.YNSIRDuplicateListPopup.messages && BX.YNSIRDuplicateListPopup.messages.hasOwnProperty(name) ? BX.YNSIRDuplicateListPopup.messages[name] : "";
		},
		_prepareContent: function()
		{
			this._contentWrapper = BX.YNSIRDuplicateRenderer.prepareListContent(
	...
	BX.YNSIRDuplicateListPopup.windows = {};
	if(typeof(BX.YNSIRDuplicateListPopup.messages) == "undefined")
	{
		BX.YNSIRDuplicateListPopup.messages = {};
	}
	BX.YNSIRDuplicateListPopup.create = function(id, settings)
	{
		var self = new BX.YNSIRDuplicateListPopup();
		self.initialize(id, settings);
		return self;
	};
}
if(typeof(BX.YNSIRDuplicateRenderer) === "undefined")
{
	BX.YNSIRDuplicateRenderer = function()
	{
	};
	BX.YNSIRDuplicateRenderer._onCommunicationBlockClick = function(e)
	{
		var element = null;
		if(e)
		{
		...
		if(BX.type.isElementNode(element))
		{
			...
			BX.unbind(element, "click", BX.YNSIRDuplicateRenderer._onCommunicationBlockClickHandler);
		}
	};
	BX.YNSIRDuplicateRenderer._onCommunicationBlockClickHandler = BX.delegate(BX.YNSIRDuplicateRenderer._onCommunicationBlockClick, BX.YNSIRDuplicateRenderer);
	BX.YNSIRDuplicateRenderer._prepareCommunications = function(comms)
	{
	...
	BX.YNSIRDuplicateRenderer.prepareListContent = function(data, params)
	{
		if(!params)
		{
			params = {};
		...
		for(var groupId in data)
		{
			...
			for(var i = 0; i < itemQty; i++)
			{
				...
				for(var j = 0; j < entityQty; j++)
				{
					...
					if(!BX.YNSIREntityType.isDefined(entityTypeID))
					{
						continue;
					}

					var entityTypeName = BX.YNSIREntityType.resolveName(entityTypeID);
		...
		for(var key in infoByType)
		{
			...
			ttlCell.colspan = 4;
			ttlCell.innerHTML = BX.util.htmlspecialchars(BX.YNSIREntityType.categoryCaptions[key]);
...
if(typeof(BX.YNSIRInterfaceMode) === "undefined")
{
	BX.YNSIRInterfaceMode = { edit: 1, view: 2 };
}
...
if(typeof(BX.YNSIRLongRunningProcessManager) == "undefined")
{
	BX.YNSIRLongRunningProcessManager = function()
	{
		this._id = "";
		this._settings = {};
		this._serviceUrl = "";
	...
	BX.YNSIRLongRunningProcessManager.prototype =
	{
		initialize: function(id, settings)
		{
			...
			if(!BX.type.isNotEmptyString(this._serviceUrl))
			{
				throw "BX.YNSIRLongRunningProcessManager. Could not find 'serviceUrl' parameter in settings.";
			}

			this._actionName = this.getSetting("actionName", "");
			if(!BX.type.isNotEmptyString(this._actionName))
			{
				throw "BX.YNSIRLongRunningProcessManager. Could not find 'actionName' parameter in settings.";
			}
		},
		getId: function()
		{
		...
		getMessage: function(name)
		{
			var m = BX.YNSIRLongRunningProcessManager.messages;
		...
		run: function()
		{
			if(!this._dialog)
			{
				...
				this._dialog = BX.YNSIRLongRunningProcessDialog.create(
					this.getId(),
					{
		...
		_onProcessStateChange: function(sender)
		{
			...
			{
				if(this._dialog.getState() === BX.YNSIRLongRunningProcessState.completed)
				{
	...
	if(typeof(BX.YNSIRLongRunningProcessManager.messages) == "undefined")
	{
		BX.YNSIRLongRunningProcessManager.messages = {};
	}
	BX.YNSIRLongRunningProcessManager.items = {};
	BX.YNSIRLongRunningProcessManager.create = function(id, settings)
	{
		var self = new BX.YNSIRLongRunningProcessManager();
		self.initialize(id, settings);
		this.items[self.getId()] = self;
		return self;
	};
...
if(typeof(BX.YNSIRLongRunningProcessPanel) === "undefined")
{
	BX.YNSIRLongRunningProcessPanel = function()
	{
		this._id = "";
		this._settings = {};
		this._prefix = "";
	...
	BX.YNSIRLongRunningProcessPanel.prototype =
	{
		initialize: function(id, settings)
		{
			this._id = id;
			...
			if(!this._container)
			{
				throw "YNSIRLongRunningProcessPanel: Could not find container.";
			}
			...
			this._manager = BX.YNSIRLongRunningProcessManager.create(this._id, this.getSetting("manager"));
		},
		layout: function()
		{
			if(this._hasLayout)
	...
	BX.YNSIRLongRunningProcessPanel.create = function(id, settings)
	{
		var self = new BX.YNSIRLongRunningProcessPanel();
		self.initialize(id, settings);
		return self;
	};
}
...
if(typeof(BX.YNSIRConversionSchemeSelector) === "undefined")
{
	BX.YNSIRConversionSchemeSelector = function()
	{
		this._id = "";
		this._settings = {};
		this._entityId = 0;
	...
	BX.YNSIRConversionSchemeSelector.prototype =
	{
		initialize: function(id, settings)
		{
			this._id = id;
			...
			{
				throw "BX.YNSIRConversionSchemeSelector: entity id is not found!";
			}
			...
			if(!BX.type.isElementNode(this._container))
			{
				throw "BX.YNSIRConversionSchemeSelector: container element is not found!";
			}
			...
			{
				throw "BX.YNSIRConversionSchemeSelector: button element is not found!";
			}
...
if(typeof(BX.YNSIREntityConversionScheme) === "undefined")
{
	BX.YNSIREntityConversionScheme = function()
	{
	};
	BX.YNSIREntityConversionScheme.mergeConfigs = function(source, target)
	{
		this.mergeEntityConfigs(BX.YNSIREntityType.names.deal, source, target);
		this.mergeEntityConfigs(BX.YNSIREntityType.names.contact, source, target);
		this.mergeEntityConfigs(BX.YNSIREntityType.names.company, source, target);
		this.mergeEntityConfigs(BX.YNSIREntityType.names.invoice, source, target);
		this.mergeEntityConfigs(BX.YNSIREntityType.names.quote, source, target);
	};
	BX.YNSIREntityConversionScheme.mergeEntityConfigs = function(entityTypeName, source, target)
	{
	...
	BX.YNSIREntityConversionScheme.removeEntityConfigs = function(entityTypeName, config)
	{
		var key = entityTypeName.toLowerCase();
...
if(typeof(BX.YNSIRLeadConversionScheme) === "undefined")
{
	BX.YNSIRLeadConversionScheme =
	{
		undefined: "",
		...
		{
			BX.YNSIREntityConversionScheme.mergeEntityConfigs(BX.YNSIREntityType.names.deal, source, target);
			BX.YNSIREntityConversionScheme.mergeEntityConfigs(BX.YNSIREntityType.names.contact, source, target);
			BX.YNSIREntityConversionScheme.mergeEntityConfigs(BX.YNSIREntityType.names.company, source, target);

			BX.YNSIREntityConversionScheme.removeEntityConfigs(BX.YNSIREntityType.names.invoice, target);
			BX.YNSIREntityConversionScheme.removeEntityConfigs(BX.YNSIREntityType.names.quote, target);
		}
	};

	if(typeof(BX.YNSIRLeadConversionScheme.messages) === "undefined")
	{
		BX.YNSIRLeadConversionScheme.messages = {};
	}
}

if(typeof(BX.YNSIRDealConversionScheme) === "undefined")
{
	BX.YNSIRDealConversionScheme =
	{
		undefined: "",
		invoice: "INVOICE",
		quote: "QUOTE",
		...
		mergeConfigs: function(source, target)
		{
			BX.YNSIREntityConversionScheme.mergeEntityConfigs(BX.YNSIREntityType.names.invoice, source, target);
			BX.YNSIREntityConversionScheme.mergeEntityConfigs(BX.YNSIREntityType.names.quote, source, target);

			BX.YNSIREntityConversionScheme.removeEntityConfigs(BX.YNSIREntityType.names.deal, target);
			BX.YNSIREntityConversionScheme.removeEntityConfigs(BX.YNSIREntityType.names.contact, target);
			BX.YNSIREntityConversionScheme.removeEntityConfigs(BX.YNSIREntityType.names.company, target);
		}
	};
	if(typeof(BX.YNSIRDealConversionScheme.messages) === "undefined")
	{
		BX.YNSIRDealConversionScheme.messages = {};
	}
}

if(typeof(BX.YNSIRQuoteConversionScheme) === "undefined")
{
	BX.YNSIRQuoteConversionScheme =
	{
		undefined: "",
		deal: "DEAL",
		invoice: "INVOICE",
	...
	if(typeof(BX.YNSIRQuoteConversionScheme.messages) === "undefined")
	{
		BX.YNSIRQuoteConversionScheme.messages = {};
	}
}

if(typeof(BX.YNSIREntityConverterMode) === "undefined")
{
	BX.YNSIREntityConverterMode =
	{
		intermediate: 0,
		schemeSetup: 1,
		syncSetup: 2,
...
if(typeof(BX.YNSIREntityConverter) === "undefined")
{
	BX.YNSIREntityConverter = function()
	{
		this._id = "";
		this._settings = {};
		this._config = {};
		this._contextData = null;
		this._mode = BX.YNSIREntityConverterMode.intermediate;
		this._entityId = 0;
		this._originUrl = "";
	...
	BX.YNSIREntityConverter.prototype =
	{
		initialize: function(id, settings)
		{
			this._id = id;
		...
		{
			this._mode = BX.YNSIREntityConverterMode.syncSetup;
			if(this._syncEditor)
			{
			...
			{
				this._syncEditor = BX.YNSIREntityFieldSynchronizationEditor.create(
					this._id + "_config",
					{
						converter: this,
						config: this._config,
		...
		convert: function(entityId, config, originUrl, contextData)
		{
			...
			if(!BX.YNSIRLeadConversionScheme.isEntityActive(this._config, BX.YNSIREntityType.names.deal))
			{
				this.startRequest();
			}
			else
			{
				...
				{
					this._dealCategorySelectDialog = BX.YNSIRDealCategorySelectDialog.create(
		...
		{
			BX.YNSIREntityConversionScheme.mergeConfigs(config, this._config);
		},
		...
		startRequest: function()
		{
			...
			this._mode = BX.YNSIREntityConverterMode.request;
		},
	...
	BX.YNSIREntityConverter.create = function(id, settings)
	{
		var self = new BX.YNSIREntityConverter();
		self.initialize(id, settings);
		return self;
	};
}

if(typeof(BX.YNSIRLeadConverter) === "undefined")
{
	BX.YNSIRLeadConverter = function()
	{
		BX.YNSIRLeadConverter.superclass.constructor.apply(this);
	...
	BX.extend(BX.YNSIRLeadConverter, BX.YNSIREntityConverter);
	BX.YNSIRLeadConverter.prototype.registerConfig = function(config)
	{
		BX.YNSIRLeadConversionScheme.mergeConfigs(config, this._config);
	};
	BX.YNSIRLeadConverter.prototype.getMessage = function(name)
	{
		var m = BX.YNSIRLeadConverter.messages;
		return m.hasOwnProperty(name) ? m[name] : name;
	};
	if(typeof(BX.YNSIRLeadConverter.messages) === "undefined")
	{
		BX.YNSIRLeadConverter.messages = {};
	}
	BX.YNSIRLeadConverter.prototype.openEntitySelector = function(callback)
	{
		this._entitySelectCallback = BX.type.isFunction(callback) ? callback : null;

		var selectorId = this._entitySelectorId;
		if(typeof(obYNSIR[selectorId]) === "undefined")
		{
			obYNSIR[selectorId] = new CRM(
				selectorId,
				null,
				null,
				selectorId,
			...
			obYNSIR[selectorId].Init();
			obYNSIR[selectorId].AddOnSaveListener(this._entitySelectHandler);
		}
	...
	BX.YNSIRLeadConverter.prototype.onEntitySelect = function(settings)
	{
		var selectorId = this._entitySelectorId;
		obYNSIR[selectorId].RemoveOnSaveListener(this._entitySelectHandler);
	...
	BX.YNSIRLeadConverter.create = function(id, settings)
	{
		var self = new BX.YNSIRLeadConverter();
		self.initialize(id, settings);
		return self;
	};
	BX.YNSIRLeadConverter.current = null;
	if(typeof(BX.YNSIRLeadConverter.settings === "undefined"))
	{
		BX.YNSIRLeadConverter.settings = {};
	}
	if(typeof(BX.YNSIRLeadConverter.permissions === "undefined"))
	{
		BX.YNSIRLeadConverter.permissions = { contact: false, company: false, deal: false };
	}
	BX.YNSIRLeadConverter.getCurrent = function()
	{
		if(!this.current)
		{
			this.current = BX.YNSIRLeadConverter.create("current", this.settings);
		}
		return this.current;
	};
}

if(typeof(BX.YNSIRDealConverter) === "undefined")
{
	BX.YNSIRDealConverter = function()
	{
		BX.YNSIRDealConverter.superclass.constructor.apply(this);
	};
	BX.extend(BX.YNSIRDealConverter, BX.YNSIREntityConverter);
	BX.YNSIRDealConverter.prototype.registerConfig = function(config)
	{
		BX.YNSIRDealConversionScheme.mergeConfigs(config, this._config);
	};
	BX.YNSIRDealConverter.prototype.getMessage = function(name)
	{
		var m = BX.YNSIRDealConverter.messages;
		return m.hasOwnProperty(name) ? m[name] : name;
	};
	if(typeof(BX.YNSIRDealConverter.messages) === "undefined")
	{
		BX.YNSIRDealConverter.messages = {};
	}
	BX.YNSIRDealConverter.create = function(id, settings)
	{
		var self = new BX.YNSIRDealConverter();
		self.initialize(id, settings);
		return self;
	};
	BX.YNSIRDealConverter.current = null;
	if(typeof(BX.YNSIRDealConverter.settings === "undefined"))
	{
		BX.YNSIRDealConverter.settings = {};
	}
	if(typeof(BX.YNSIRDealConverter.permissions === "undefined"))
	{
		BX.YNSIRDealConverter.permissions = { invoice: false, quote: false };
	}
	BX.YNSIRDealConverter.getCurrent = function()
	{
		if(!this.current)
		{
			this.current = BX.YNSIRDealConverter.create("current", this.settings);
		}
		return this.current;
	};
}

if(typeof(BX.YNSIRQuoteConverter) === "undefined")
{
	BX.YNSIRQuoteConverter = function()
	{
		BX.YNSIRQuoteConverter.superclass.constructor.apply(this);
	};
	BX.extend(BX.YNSIRQuoteConverter, BX.YNSIREntityConverter);
	BX.YNSIRQuoteConverter.prototype.registerConfig = function(config)
	{
		BX.YNSIRQuoteConversionScheme.mergeConfigs(config, this._config);
	};
	BX.YNSIRQuoteConverter.prototype.getMessage = function(name)
	{
		var m = BX.YNSIRQuoteConverter.messages;
		return m.hasOwnProperty(name) ? m[name] : name;
	};
	if(typeof(BX.YNSIRQuoteConverter.messages) === "undefined")
	{
		BX.YNSIRQuoteConverter.messages = {};
	}
	BX.YNSIRQuoteConverter.create = function(id, settings)
	{
		var self = new BX.YNSIRQuoteConverter();
		self.initialize(id, settings);
		return self;
	};
	BX.YNSIRQuoteConverter.current = null;
	if(typeof(BX.YNSIRQuoteConverter.settings === "undefined"))
	{
		BX.YNSIRQuoteConverter.settings = {};
	}
	if(typeof(BX.YNSIRQuoteConverter.permissions === "undefined"))
	{
		BX.YNSIRQuoteConverter.permissions = { invoice: false, quote: false };
	}
	BX.YNSIRQuoteConverter.getCurrent = function()
	{
		if(!this.current)
		{
			this.current = BX.YNSIRQuoteConverter.create("current", this.settings);
		}
		return this.current;
	};
}

if(typeof(BX.YNSIREntityFieldSynchronizationEditor) === "undefined")
{
	BX.YNSIREntityFieldSynchronizationEditor = function()
	{
		this._id = "";
		this._settings = {};
	...
	BX.YNSIREntityFieldSynchronizationEditor.prototype =
	{
		initialize: function(id, settings)
		{
			this._id = id;
			...
			this._closingNotifier = BX.YNSIRNotifier.create(this);
		},
		getSetting: function (name, defaultval)
		{
			return typeof(this._settings[name]) != 'undefined' ? this._settings[name] : defaultval;
		...
		show: function()
		{
			...
			var id = this.getId();
			if(BX.YNSIREntityFieldSynchronizationEditor.windows[id])
			{
				BX.YNSIREntityFieldSynchronizationEditor.windows[id].destroy();
				delete BX.YNSIREntityFieldSynchronizationEditor.windows[id];
			}
			...
			BX.YNSIREntityFieldSynchronizationEditor.windows[id] = this._popup;
			this._popup.show();
		},
		close: function()
		{
		...
		getMessage: function(name)
		{
			var m = BX.YNSIREntityFieldSynchronizationEditor.messages;
				...
				var label = BX.create("LABEL",
					{
						...
						text: BX.YNSIREntityType.getCaptionByName(entityTypeName)
					}
				);
		...
		onPopupDestroy: function()
		{
			...
			this._resizer = null;
			this._popup = null;
			delete BX.YNSIREntityFieldSynchronizationEditor.windows[this.getId()];
		}
	};
	BX.YNSIREntityFieldSynchronizationEditor.windows = {};
	if(typeof(BX.YNSIREntityFieldSynchronizationEditor.messages) == "undefined")
	{
		BX.YNSIREntityFieldSynchronizationEditor.messages = {};
	}
	BX.YNSIREntityFieldSynchronizationEditor.create = function(id, settings)
	{
		var self = new BX.YNSIREntityFieldSynchronizationEditor();
		self.initialize(id, settings);
		return self;
	};
}
...
if(typeof(BX.YNSIRLeadConversionSchemeSelector) === "undefined")
{
	BX.YNSIRLeadConversionSchemeSelector = function()
	{
		BX.YNSIRLeadConversionSchemeSelector.superclass.constructor.apply(this);
	};
	BX.extend(BX.YNSIRLeadConversionSchemeSelector, BX.YNSIRConversionSchemeSelector);
	BX.YNSIRLeadConversionSchemeSelector.prototype.prepareItems = function()
	{
		var isDealPermitted = BX.YNSIRLeadConverter.permissions["deal"];
		var isContactPermitted = BX.YNSIRLeadConverter.permissions["contact"];
		var isCompanyPermitted = BX.YNSIRLeadConverter.permissions["company"];
		...
		if(isDealPermitted)
		{
			...
			{
				schemes.push(BX.YNSIRLeadConversionScheme.dealcontactcompany);
			}
			if(isContactPermitted)
			{
				schemes.push(BX.YNSIRLeadConversionScheme.dealcontact);
			}
			if(isCompanyPermitted)
			{
				schemes.push(BX.YNSIRLeadConversionScheme.dealcompany);
			}

			schemes.push(BX.YNSIRLeadConversionScheme.deal);
		}
		if(isContactPermitted && isCompanyPermitted)
		{
			schemes.push(BX.YNSIRLeadConversionScheme.contactcompany);
		}
		if(isContactPermitted)
		{
			schemes.push(BX.YNSIRLeadConversionScheme.contact);
		}
		if(isCompanyPermitted)
		{
			schemes.push(BX.YNSIRLeadConversionScheme.company);
		}

		var items = BX.YNSIRLeadConversionScheme.getListItems(schemes);
		if(isContactPermitted || isCompanyPermitted)
		{
			items.push({
				value: "CUSTOM",
				text: BX.YNSIRLeadConverter.getCurrent().getMessage("openEntitySelector")
			});
		}

		return items;
	};
	BX.YNSIRLeadConversionSchemeSelector.prototype.prepareConfig = function()
	{
		return BX.YNSIRLeadConversionScheme.createConfig(this._scheme);
	};
	BX.YNSIRLeadConversionSchemeSelector.prototype.getSchemeDescription = function(scheme)
	{
		return BX.YNSIRLeadConversionScheme.getDescription(scheme);
	};
	BX.YNSIRLeadConversionSchemeSelector.prototype.processMenuItemClick = function(item)
	{
		var value = item["value"];
		if(value === "CUSTOM")
		{
			BX.YNSIRLeadConverter.getCurrent().openEntitySelector(BX.delegate(this.onEntitySelect, this));
		}
		else
		{
	...
	BX.YNSIRLeadConversionSchemeSelector.prototype.onEntitySelect = function(result)
	{
		...
		BX.YNSIRLeadConverter.getCurrent().convert(
			this._entityId,
			result["config"],
	...
	BX.YNSIRLeadConversionSchemeSelector.prototype.convert = function()
	{
		BX.YNSIRLeadConverter.getCurrent().convert(
			this._entityId,
			this.prepareConfig(),
	...
	BX.YNSIRLeadConversionSchemeSelector.create = function(id, settings)
	{
		var self = new BX.YNSIRLeadConversionSchemeSelector();
		self.initialize(id, settings);
		return self;
	};
}

if(typeof(BX.YNSIRDealConversionSchemeSelector) === "undefined")
{
	BX.YNSIRDealConversionSchemeSelector = function()
	{
		BX.YNSIRDealConversionSchemeSelector.superclass.constructor.apply(this);
	};
	BX.extend(BX.YNSIRDealConversionSchemeSelector, BX.YNSIRConversionSchemeSelector);
	BX.YNSIRDealConversionSchemeSelector.prototype.prepareItems = function()
	{
		var schemes = [];
		if(BX.YNSIRDealConverter.permissions["invoice"])
		{
			schemes.push(BX.YNSIRDealConversionScheme.invoice);
		}
		if(BX.YNSIRDealConverter.permissions["quote"])
		{
			schemes.push(BX.YNSIRDealConversionScheme.quote);
		}
		return BX.YNSIRDealConversionScheme.getListItems(schemes);
	};
	BX.YNSIRDealConversionSchemeSelector.prototype.prepareConfig = function()
	{
		return BX.YNSIRDealConversionScheme.createConfig(this._scheme);
	};
	BX.YNSIRDealConversionSchemeSelector.prototype.getSchemeDescription = function(scheme)
	{
		return BX.YNSIRDealConversionScheme.getDescription(scheme);
	};
	BX.YNSIRDealConversionSchemeSelector.prototype.convert = function()
	{
		BX.YNSIRDealConverter.getCurrent().convert(
			this._entityId,
			this.prepareConfig(),
			this.getSetting("originUrl", "")
		);
	};
	BX.YNSIRDealConversionSchemeSelector.create = function(id, settings)
	{
		var self = new BX.YNSIRDealConversionSchemeSelector();
		self.initialize(id, settings);
		return self;
	};
}

if(typeof(BX.YNSIRQuoteConversionSchemeSelector) === "undefined")
{
	BX.YNSIRQuoteConversionSchemeSelector = function()
	{
		BX.YNSIRQuoteConversionSchemeSelector.superclass.constructor.apply(this);
	};
	BX.extend(BX.YNSIRQuoteConversionSchemeSelector, BX.YNSIRConversionSchemeSelector);
	BX.YNSIRQuoteConversionSchemeSelector.prototype.prepareItems = function()
	{
		var schemes = [];
		if(BX.YNSIRQuoteConverter.permissions["deal"])
		{
			schemes.push(BX.YNSIRQuoteConversionScheme.deal);
		}
		if(BX.YNSIRQuoteConverter.permissions["invoice"])
		{
			schemes.push(BX.YNSIRQuoteConversionScheme.invoice);
		}
		return BX.YNSIRQuoteConversionScheme.getListItems(schemes);
	};
	BX.YNSIRQuoteConversionSchemeSelector.prototype.prepareConfig = function()
	{
		return BX.YNSIRQuoteConversionScheme.createConfig(this._scheme);
	};
	BX.YNSIRQuoteConversionSchemeSelector.prototype.getSchemeDescription = function(scheme)
	{
		return BX.YNSIRQuoteConversionScheme.getDescription(scheme);
	};
	BX.YNSIRQuoteConversionSchemeSelector.prototype.convert = function()
	{
		BX.YNSIRQuoteConverter.getCurrent().convert(
			this._entityId,
			this.prepareConfig(),
			this.getSetting("originUrl", "")
		);
	};
	BX.YNSIRQuoteConversionSchemeSelector.items = {};
	BX.YNSIRQuoteConversionSchemeSelector.create = function(id, settings)
	{
		var self = new BX.YNSIRQuoteConversionSchemeSelector();
		self.initialize(id, settings);
		this.items[self.getId()] = self;
		return self;
	};
...
//region BX.YNSIRRequisitePresetListLoader
BX.YNSIRRequisitePresetListLoader = function()
{
	this._id = "";
	this._settings = {};
	this._entityTypeName = "";
...
BX.YNSIRRequisitePresetListLoader.prototype =
{
	initialize: function(id, settings)
	{
		...
		if(!BX.type.isNotEmptyString(this._entityTypeName))
		{
			throw "BX.YNSIRRequisitePresetListLoader. Could not find 'entityTypeName' parameter.";
		}
		...
		if(!BX.type.isNotEmptyString(this._serviceUrl))
		{
			throw "BX.YNSIRRequisitePresetListLoader. Could not find 'serviceUrl' parameter.";
		}
...
BX.YNSIRRequisitePresetListLoader.create = function(id, settings)
{
	var self = new BX.YNSIRRequisitePresetListLoader();
	self.initialize(id, settings);
	return self;
};
//endregion
//region BX.YNSIRRequisitePresetSelectDialog
BX.YNSIRRequisitePresetSelectDialog = function()
{
	this._id = "";
	this._settings = {};
	this._popup = null;
...
BX.YNSIRRequisitePresetSelectDialog.prototype =
{
	initialize: function(id, settings)
	{
		this._id = id;
		...
		{
			throw "BX.YNSIRRequisitePresetSelectDialog. Could not find 'list' parameter.";
		}
	...
	getMessage:function(name)
	{
		var m = BX.YNSIRRequisitePresetSelectDialog.messages;
		return m.hasOwnProperty(name) ? m[name] : name;
	},
	show: function()
	{
		...
		var id = this.getId();
		if(BX.YNSIRRequisitePresetSelectDialog.windows[id])
		{
			BX.YNSIRRequisitePresetSelectDialog.windows[id].destroy();
		}
		...
		(BX.YNSIRRequisitePresetSelectDialog.windows[id] = this._popup).show();
	},
	close: function()
	{
		if(!(this._popup && this._popup.isShown()))
...
if(typeof(BX.YNSIRRequisitePresetSelectDialog.messages) === "undefined")
{
	BX.YNSIRRequisitePresetSelectDialog.messages = {};
}
BX.YNSIRRequisitePresetSelectDialog.windows = {};
BX.YNSIRRequisitePresetSelectDialog.create = function(id, settings)
{
	var self = new BX.YNSIRRequisitePresetSelectDialog();
	self.initialize(id, settings);
	return self;
};
//endregion
//region BX.YNSIRRequisiteConverter
BX.YNSIRRequisiteConverter = function()
{
	this._id = "";
	this._settings = {};
	this._entityTypeName = "";
...
BX.YNSIRRequisiteConverter.prototype =
{
	initialize: function(id, settings)
	{
		this._id = id;
		...
		if(!BX.type.isNotEmptyString(this._entityTypeName))
		{
			throw "BX.YNSIRRequisiteConverter. Could not find 'entityTypeName' parameter.";
		}
		this._entityTypeName = this._entityTypeName.toUpperCase();
		...
		if(!BX.type.isNotEmptyString(this._serviceUrl))
		{
			throw "BX.YNSIRRequisiteConverter. Could not find 'serviceUrl' parameter.";
		}
	},
	getId: function()
	{
	...
	getMessage:function(name)
	{
		var m = BX.YNSIRRequisiteConverter.messages;
		return m.hasOwnProperty(name) ? m[name] : name;
	},
	convert: function()
	{
	...
	openPresetListLoader: function()
	{
		...
		{
			this._presetListLoader = BX.YNSIRRequisitePresetListLoader.create(
				this._id,
				{
	...
	openPresetSelector: function()
	{
		...
		{
			this._presetSelector = BX.YNSIRRequisitePresetSelectDialog.create(
				this._id,
				{
	...
	openProcessDialog: function()
	{
		if(!this._processDialog)
		{
			...
			this._processDialog = BX.YNSIRLongRunningProcessDialog.create(
				key,
				{
					serviceUrl: this._serviceUrl,
	...
	{
		if(sender.getState() === BX.YNSIRLongRunningProcessState.completed)
		{
...
if(typeof(BX.YNSIRRequisiteConverter.messages) === "undefined")
{
	BX.YNSIRRequisiteConverter.messages = {};
}
BX.YNSIRRequisiteConverter.create = function(id, settings)
{
	var self = new BX.YNSIRRequisiteConverter();
	self.initialize(id, settings);
	return self;
};
//endregion
//region BX.YNSIRPSRequisiteConverter
BX.YNSIRPSRequisiteConverter = function()
{
	this._id = "";
	this._settings = {};
	this._serviceUrl = "";
...
BX.YNSIRPSRequisiteConverter.prototype =
{
	initialize: function(id, settings)
	{
		this._id = id;
		...
		if(!BX.type.isNotEmptyString(this._serviceUrl))
		{
			throw "BX.YNSIRPSRequisiteConverter. Could not find 'serviceUrl' parameter.";
		}
	},
	getId: function()
	{
	...
	getMessage:function(name)
	{
		var m = BX.YNSIRPSRequisiteConverter.messages;
		return m.hasOwnProperty(name) ? m[name] : name;
	},
	convert: function()
	{
...
if(typeof(BX.YNSIRPSRequisiteConverter.messages) === "undefined")
{
	BX.YNSIRPSRequisiteConverter.messages = {};
}
BX.YNSIRPSRequisiteConverter.create = function(id, settings)
{
	var self = new BX.YNSIRPSRequisiteConverter();
	self.initialize(id, settings);
	return self;
};
//endregion
//region BX.YNSIRDealCategory
if(typeof(BX.YNSIRDealCategory) == "undefined")
{
	BX.YNSIRDealCategory = function()
	{
	};

	BX.YNSIRDealCategory.getDefaultValue = function()
	{
		return "0";
	};
	BX.YNSIRDealCategory.getListItems = function(infos)
	{
		if(!BX.type.isArray(infos))
		{
			infos = BX.YNSIRDealCategory.infos;
		}
	...
	if(typeof(BX.YNSIRDealCategory.infos) === "undefined")
	{
		BX.YNSIRDealCategory.infos = [];
	}
}
//endregion
//region BX.YNSIRDealCategorySelector
if(typeof(BX.YNSIRDealCategorySelector) == "undefined")
{
	BX.YNSIRDealCategorySelector = function()
	{
		this._id = "";
		this._settings = {};
	...
	BX.YNSIRDealCategorySelector.prototype =
	{
		initialize: function(id, settings)
		{
			this._id = id;
		...
		getMessage: function(name)
		{
			var m = BX.YNSIRDealCategorySelector.messages;
			return m.hasOwnProperty(name) ? m[name] : name;
		},
		...
		openMenu: function(anchor)
		{
			...
			{
				var items = BX.YNSIRDealCategory.getListItems();
	...
	if(typeof(BX.YNSIRDealCategorySelector.messages) === "undefined")
	{
		BX.YNSIRDealCategorySelector.messages = {};
	}
	BX.YNSIRDealCategorySelector.items = {};
	BX.YNSIRDealCategorySelector.create = function(id, settings)
	{
		var self = new BX.YNSIRDealCategorySelector();
		self.initialize(id, settings);
		this.items[self.getId()] = self;
		return self;
	};
...
//endregion
//region BX.YNSIRDealCategorySelectDialog
if(typeof(BX.YNSIRDealCategorySelectDialog) === "undefined")
{
	BX.YNSIRDealCategorySelectDialog = function()
	{
		this._id = "";
		this._settings = {};
		this._popup = null;
	...
	BX.YNSIRDealCategorySelectDialog.prototype =
	{
		initialize: function(id, settings)
		{
			this._id = id;
			...
			this._closeNotifier = BX.YNSIRNotifier.create(this);
		},
		getId: function()
		{
			return this._id;
		...
		getMessage: function(name)
		{
			var m = BX.YNSIRDealCategorySelectDialog.messages;
			return m.hasOwnProperty(name) ? m[name] : name;
		},
		isOpened: function()
		{
		...
		prepareContent: function()
		{
			...
			BX.HtmlHelper.setupSelectOptions(this._selector, BX.YNSIRDealCategory.getListItems());
	...
	if(typeof(BX.YNSIRDealCategorySelectDialog.messages) === "undefined")
	{
		BX.YNSIRDealCategorySelectDialog.messages = {};
	}
	BX.YNSIRDealCategorySelectDialog.create = function(id, settings)
	{
		var self = new BX.YNSIRDealCategorySelectDialog();
		self.initialize(id, settings);
		return self;
	};
}
...
if(typeof(BX.YNSIRHtmlLoader) == "undefined")
{
	BX.YNSIRHtmlLoader = function()
	{
		this._id = "";
		this._settings = {};
		this._params = {};
	...
	BX.YNSIRHtmlLoader.prototype =
	{
		initialize: function(id, settings)
		{
			...
			if(!BX.type.isNotEmptyString(this._serviceUrl))
			{
				throw "BX.YNSIRHtmlLoader: service url not found!";
			}

			this._action = this.getSetting("action");
			if(!BX.type.isNotEmptyString(this._action))
			{
				throw "BX.YNSIRHtmlLoader: action not found!";
			}
			...
			if(!BX.type.isElementNode(this._button))
			{
				throw "BX.YNSIRHtmlLoader: button element not found!";
			}
			BX.bind(this._button, "click", this._buttonClickHandler);
			...
			{
				throw "BX.YNSIRHtmlLoader: wrapper element not found!";
			}
	...
	BX.YNSIRHtmlLoader.create = function(id, settings)
	{
		var self = new BX.YNSIRHtmlLoader();
		self.initialize(id, settings);
		return self;
	}
}
if(typeof(BX.YNSIRDataLoader) == "undefined")
{
	BX.YNSIRDataLoader = function()
	{
		this._id = "";
		this._settings = {};
		this._params = {};
	...
	BX.YNSIRDataLoader.prototype =
	{
		initialize: function(id, settings)
		{
			...
			if(!BX.type.isNotEmptyString(this._serviceUrl))
			{
				throw "BX.YNSIRDataLoader: service url not found!";
			}

			this._action = this.getSetting("action");
			if(!BX.type.isNotEmptyString(this._action))
			{
				throw "BX.YNSIRDataLoader: action not found!";
			}

			this._params = this.getSetting("params", {});

			this._notifier = BX.YNSIRNotifier.create(this);
		},
		getSetting: function (name, defaultval)
		{
			return this._settings.hasOwnProperty(name) ? this._settings[name] : defaultval;
	...
	BX.YNSIRDataLoader.create = function(id, settings)
	{
		var self = new BX.YNSIRDataLoader();
		self.initialize(id, settings);
		return self;
	}
}
if(typeof(BX.YNSIRRemoteAction))
{
	BX.YNSIRRemoteAction = function()
	{
		this._id = "";
		this._settings = {};
		this._serviceUrl = "";
	...
	BX.YNSIRRemoteAction.prototype =
	{
		initialize: function(id, settings)
		{
			this._id = id;
			...
			if(!BX.type.isNotEmptyString(this._serviceUrl))
			{
				throw "BX.YNSIRRemoteAction: service url not found!";
			}
	...
	BX.YNSIRRemoteAction.items = {};
	BX.YNSIRRemoteAction.create = function(id, settings)
	{
		var self = new BX.YNSIRRemoteAction();
		self.initialize(id, settings);
		this.items[self.getId()] = self;
		return self;
	}
...
if(typeof(BX.YNSIRDeletionConfirmDialog) === "undefined")
{
	BX.YNSIRDeletionConfirmDialog = function()
	{
		this._id = "";
		this._settings = {};
		this._name = "";
	...
	BX.YNSIRDeletionConfirmDialog.prototype =
	{
		initialize: function(id, settings)
		{
			this._id = id;
			...
			{
				throw "BX.YNSIRDeletionConfirmDialog: Could not find parameter 'path'.";
			}
	...
	BX.YNSIRDeletionConfirmDialog.create = function(id, settings)
	{
		var self = new BX.YNSIRDeletionConfirmDialog();
		self.initialize(id, settings);
		return self;
	};
}
			...
			BX.SocNetLogDestination.init(
				{
					...
					destSort: {},
					allowAddUser: false,
					allowSearchYNSIREmailUsers: false,
...
if(typeof(BX.YNSIRSearchContentManager) == "undefined")
{
	BX.YNSIRSearchContentManager = function()
	{
		this._id = "";
		this._settings = {};
		this._entityTypeName = "";
	...
	BX.YNSIRSearchContentManager.prototype =
	{
		initialize: function(id, settings)
		{
			...
			if(!BX.type.isNotEmptyString(this._entityTypeName))
			{
				throw "BX.YNSIRSearchContentManager. Could not find entity type name.";
			}
		...
		getMessage: function(name)
		{
			var m = BX.YNSIRSearchContentManager.messages;
			return m.hasOwnProperty(name) ? m[name] : name;
		},
		rebuildIndex: function()
		{
			...
			if(!BX.type.isNotEmptyString(serviceUrl))
			{
				throw "BX.YNSIRSearchContentManager. Could not find service url.";
			}
	...
	if(typeof(BX.YNSIRSearchContentManager.messages) == "undefined")
	{
		BX.YNSIRSearchContentManager.messages = {};
	}
	BX.YNSIRSearchContentManager.items = {};
	BX.YNSIRSearchContentManager.create = function(id, settings)
	{
		var self = new BX.YNSIRSearchContentManager();
		self.initialize(id, settings);
		this.items[self.getId()] = self;
		return self;
	};
