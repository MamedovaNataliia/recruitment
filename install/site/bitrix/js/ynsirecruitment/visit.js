(function()
{
	...
	BX.YNSIRActivityVisit = function(config)
	{
		var self = this;
	...
	BX.YNSIRActivityVisit.create = function(config)
	{
		return new BX.YNSIRActivityVisit(config);
	};

	BX.YNSIRActivityVisit.prototype.init = function()
	{
		var self = this;
	...
	BX.YNSIRActivityVisit.prototype.getId = function()
	{
		return this.id;
	};

	BX.YNSIRActivityVisit.prototype.setId = function(id)
	{
		this.id = id;
	};

	BX.YNSIRActivityVisit.prototype.getMainNode = function()
	{
		return this.mainNode;
	};

	BX.YNSIRActivityVisit.prototype.setMainNode = function(mainNode)
	{
		this.mainNode = mainNode;
	};

	BX.YNSIRActivityVisit.prototype.getPopup = function()
	{
		return this.popup;
	};

	BX.YNSIRActivityVisit.prototype.setPopup = function(popup)
	{
		this.popup = popup;
	};

	BX.YNSIRActivityVisit.prototype.getNode = function(name, scope)
	{
		if (!scope)
	...
	BX.YNSIRActivityVisit.prototype.showEdit = function()
	{
		var self = this;
		var params = {
			ajax_action: 'EDIT'
		...
		self._checkConsent(function ()
		{
			self._checkRecognizeConsent(function (){
				self._createAjaxPopup(params, function()
				{
					...
					if(BX.YNSIRRecorder.isSupported())
					{
						self.recorder = new BX.YNSIRRecorder({
	...
	BX.YNSIRActivityVisit.prototype.saveActivity = function(next)
	{
		var self = this;
		var finishButton = self.getNode('button-finish');
		...
		if(this.recorder)
		{
			...
			{
				var formData = new FormData(self.getNode('visit-form'));
	...
	BX.YNSIRActivityVisit.prototype.dispose = function()
	{
		...
		if(obYNSIR && obYNSIR.visitYNSIRSelector)
		{
			obYNSIR.visitYNSIRSelector.RemoveOnSaveListener(this._selectEntityHandler);
		}
	};

	BX.YNSIRActivityVisit.prototype._checkConsent = function(next)
	{
		if (this.hasConsent)
		{
			next();
	...
	BX.YNSIRActivityVisit.prototype._checkRecognizeConsent = function(next)
	{
		var self = this;
	...
	BX.YNSIRActivityVisit.prototype._saveRecognizeConsent = function(next)
	{
		var params = {
	...
	BX.YNSIRActivityVisit.prototype._createAjaxPopup = function(params, next)
	{
		params['sessid'] = BX.bitrix_sessid();
		params['HAS_RECOGNIZE_CONSENT'] = (this.hasRecognizeConsent ? 'Y' : 'N');

		var self = this;
		var wrapper = BX.create('div', {style:	{'min-width': (self.hasFaceId ? '989px' : '550px'), 'min-height': '639px', 'background-color': '#37404b'}});
	...
	BX.YNSIRActivityVisit.prototype.setOwner = function(entity)
	{
		var self = this;
	...
	BX.YNSIRActivityVisit.prototype._onCreateButtonClick = function(e)
	{
		var target = e.target;
	...
	BX.YNSIRActivityVisit.prototype._onExternalEvent = function(params)
	{
		var self = this;
		var entity;
		...
		if(params.key === "onYNSIREntityCreate" && this.externalRequests[value.context])
		{
			if(this.externalRequests[value.context])
			{
				...
				else if (this.externalRequests[value.context]['type'] == 'add')
				{
					if (value.entityTypeName == 'DEAL')
						this.createdDeals.push(parseInt(value.entityInfo.id));
					else if (value.entityTypeName == 'INVOICE')
						this.createdInvoices.push(parseInt(value.entityInfo.id));
	...
	BX.YNSIRActivityVisit.prototype._onAddButtonClick = function(e)
	{
		var target = e.target;
		var url = target.dataset.url;
	...
	BX.YNSIRActivityVisit.prototype._onFinishButtonClick = function (e)
	{
		var self = this;
		if(this.failed)
		{
		...
		else
		{
			...
			{
				self.getPopup().close();
				BX.YNSIRActivityVisit.runCallback('onVisitCreated', {});
			});
		}
	};

	BX.YNSIRActivityVisit.prototype._onPopupClose = function()
	{
	...
	BX.YNSIRActivityVisit.prototype._reloadOwnerCard = function(entity, next)
	{
		var self = this;
		var requestData = {
			'sessid': BX.bitrix_sessid(),
	...
	BX.YNSIRActivityVisit.prototype._updateTimer = function()
	{
	...
	BX.YNSIRActivityVisit.prototype._onSelectorButtonClick = function()
	{
		this._openYNSIRSelector();
	};


	BX.YNSIRActivityVisit.prototype._openYNSIRSelector = function()
	{
		var self = this;
		if(!crmSelectorLoaded)
		{
			...
			BX.ajax({
				...
				onsuccess: function()
				{
					...
					setTimeout(self._openYNSIRSelector.bind(self), 50);
				}
			})
		}
		...
		if(obYNSIR && obYNSIR.visitYNSIRSelector)
		{
			obYNSIR.visitYNSIRSelector.Open();
			obYNSIR.visitYNSIRSelector.AddOnSaveListener(this._selectEntityHandler);
		}
	};

	BX.YNSIRActivityVisit.prototype._onSelectEntity = function(params)
	{
		var entity = {
			entityType: '',
			entityId: 0
	...
	BX.YNSIRActivityVisit.prototype._onFaceSelected = function(params)
	{
		this.hasPhoto = true;
	...
	BX.YNSIRActivityVisit.prototype._onFaceReset = function(params)
	{
		this.hasPhoto = false;
	};

	BX.YNSIRActivityVisit.prototype._onFaceSocialProfileSelected = function(profile)
	{
		this.vkProfile = profile;
		this.vkProfileChanged = true;
	};

	BX.YNSIRActivityVisit.prototype._onRecorderDeviceReady = function()
	{
		if (this.faceSearch)
			this.faceSearch.start();
	};

	BX.YNSIRActivityVisit.prototype._onRecorderDeviceFailure = function(error)
	{
	...
	BX.YNSIRActivityVisit.setCallback = function(eventName, callback)
	{
	...
	BX.YNSIRActivityVisit.runCallback = function(eventName, event)
	{
	...
	var CommunicationSearch = function(node, config)
	{
		...
		this.node = node;
		this.communicationType = BX.YNSIRCommunicationType.undefined;
	...
	CommunicationSearch.prototype.openDialog = function()
	{
		...
		this._communicationSearchController = BX.YNSIRCommunicationSearchController.create(this._communicationSearch, inputNode);
	...
	FaceSearch.prototype.__getMediaStream = function()
	{
		...
		navigator.mediaDevices.getUserMedia(this.__getConstraints()).then(function(stream)
		{
			...
			videoNode.volume = 0;
			videoNode.src = URL.createObjectURL(self.mediaStream);
			videoNode.play();
