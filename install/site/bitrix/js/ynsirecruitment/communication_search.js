...
{
	BX.YNSIRCommunicationSearch = function()
	{
		this._id = "";
		this._settings = {};
		this._provider = null;
	...
	BX.YNSIRCommunicationSearch.prototype =
	{
		initialize: function(id, settings)
		{
			...
			this._provider =  BX.YNSIRCommunicationSearchProvider.create(
				this,
				{
			...
			if(!this._provider)
			{
				throw  "BX.YNSIRCommunicationSearch. Could resolve provider for '" + entityType  + "' entity type`.";
			}
		},
		...
		_prepareDialogContent: function()
		{
			if(!this._provider)
			{
				throw  "BX.YNSIRCommunicationSearch. Could not find provider.";
			}
			...
			if(this._enableSearch)
			{
				...
				{
					id: "search",
					title: BX.YNSIRCommunicationSearch.messages["SearchTab"],
					active: !activeTab
				};
							...
							[
								BX.create(
									"SPAN",
									{
										...
										text: BX.YNSIRCommunicationSearch.messages["NoData"]
									}
								)
							]
					}
		...
		_prepareTabContent: function(data)
		{
			...
			for(var i = 0; i < data.length; i++)
			{
				...
				if(itemCommData.length === 0)
				{
					...
					var item = BX.YNSIRCommunication.create(
						this,
						{
	...
	BX.YNSIRCommunicationSearch.create = function(id, settings)
	{
		var self = new BX.YNSIRCommunicationSearch();
		self.initialize(id, settings);
		return self;
	};

	BX.YNSIRCommunicationType =
	{
		undefined: "",
		phone: "PHONE",
		email: "EMAIL"
	...
	BX.YNSIRCommunicationSearchProvider = function()
	{
		this._manager = null;
		this._settings = {};
		this._entityType = "";
	...
	BX.YNSIRCommunicationSearchProvider.prototype =
	{
		initialize: function(manager, settings)
		{
			if(!manager)
			{
				throw "BX.YNSIRCommunicationSearchProvider. Manager is not defined.";
			}
			...
			{
				this._data = BX.YNSIRCommunicationSearchProvider.getEntityData(
					this._entityType,
					this._entityId,
					this._commType
				);
			...
			for(var i = 0; i < data.length; i++)
			{
				...
				for(var j = 0; j < items.length; j++)
				{
					...
					if(commType === "")
					{
						...
						return BX.YNSIRCommunication.create(
							this,
							{
								"type": commType,
		...
		processDataResponse: function(responseData)
		{
			...
			BX.YNSIRCommunicationSearchProvider.setEntityData(
				this._entityType,
				this._entityId,
				this._commType,
				this._data
	...
	BX.YNSIRCommunicationSearchProvider.entityData = {};
	BX.YNSIRCommunicationSearchProvider.getEntityData = function(entityType, entityId, type)
	{
		if(type === "")
		{
			type = "PERS";
	...
	BX.YNSIRCommunicationSearchProvider.setEntityData = function(entityType, entityId, type, data)
	{
		if(type === "")
		{
			type = "PERS";
	...
	BX.YNSIRCommunicationSearchProvider.create = function(manager, settings)
	{
		var self = new BX.YNSIRCommunicationSearchProvider();
		self.initialize(manager, settings);
		return self;
	};

	BX.YNSIRCommunication = function()
	{
		this._settings = {};
		this._manager = null;
	};

	BX.YNSIRCommunication.prototype =
	{
		initialize: function(manager, settings)
		{
			if(!manager)
			{
				throw "BX.YNSIRCommunication. Manager is not defined.";
			}
	...
	BX.YNSIRCommunication.create = function(manager, settings)
	{
		var self = new BX.YNSIRCommunication();
		self.initialize(manager, settings);
		return self;
	};

	BX.YNSIRCommunicationSearchController = function()
	{
		this._id = '';
		this._manager = null;
		this._input = null;
	...
	BX.YNSIRCommunicationSearchController.prototype =
	{
	...
	BX.YNSIRCommunicationSearchController.create = function(manager, input)
	{
		var self = new BX.YNSIRCommunicationSearchController();
