BX.YNSIRInstantEditor = function ()
{
	this._id = '';
	this._settings = {};
	this._fields = [];
...
BX.YNSIRInstantEditor.prototype =
{
	initialize: function (id, settings)
	{
		...
		BX.onCustomEvent('YNSIRInstantEditorCreated', [ this ]);
	},
	getId: function()
	{
		return this._id;
	...
	registerField: function(fieldContainer)
	{
		...
		var field = BX.YNSIRInstantEditorField.create(fieldContainer, button, this);
	...
	{
		BX.onCustomEvent(this, 'YNSIRInstantEditorFieldValueSaved', [ name, value ]);
	},
		...
		BX.ajax(
			{
				...
				{
					BX.onCustomEvent(self, 'YNSIRInstantEditorFieldValueSaved', [ name, value ]);
	...
	setFieldValue: function(name, val)
	{
		...
		BX.onCustomEvent(this, 'YNSIRInstantEditorFieldValueChanged', [ name, val ]);
	},
	setFieldReadOnly: function(name, readonly)
	{
		...
		BX.onCustomEvent('YNSIRInstantEditorSetFieldReadOnly', [ this, name, readonly ]);
		var fields = this.getFieldsByName(name);
...
BX.YNSIRInstantEditor._default = null;
BX.YNSIRInstantEditor.getDefault = function()
{
	return this._default;
};

BX.YNSIRInstantEditor.items = {};
BX.YNSIRInstantEditor.create = function(id, settings)
{
	if(!BX.type.isNotEmptyString(id))
	{
		throw 'BX.YNSIRInstantEditor.create: id is not defined!';
	}

	var self = new BX.YNSIRInstantEditor();
...
BX.YNSIRInstantEditorFieldMode =
{
	edit: 1,
	view: 2
};

BX.YNSIRInstantEditorField = function()
{
	this._mode = BX.YNSIRInstantEditorFieldMode.view;
...
BX.YNSIRInstantEditorField.prototype =
{
	initialize:function (container, button, manager)
	{
		if(!container)
		{
			throw "BX.YNSIRInstantEditorField.initialize: 'container' is not defined!";
		}
		this._container = container;

		if(!button)
		{
			throw "BX.YNSIRInstantEditorField.initialize: 'button' is not defined!";
		}
		this._button = button;
		button.setAttribute('title', BX.YNSIRInstantEditorMessages.editButtonTitle);

		if(!manager)
		{
			throw "BX.YNSIRInstantEditorField.initialize: 'manager' is not defined!";
		}
	...
	processUpdate: function(result)
	{
		...
		if(this._type === 'phone' && this.getCalltoFormat() === BX.YNSIRCalltoFormat.custom)
		{
	...
	{
		return this._manager.getSetting('callToFormat', BX.YNSIRCalltoFormat.slashless);
	},
	...
	{
		if(this._mode !== BX.YNSIRInstantEditorFieldMode.edit)
		{
			return;
		}
	...
	_switch2Edit: function(force)
	{
		...
		if(this._isReadOnly || (!force && this._mode === BX.YNSIRInstantEditorFieldMode.edit))
		{
			return;
		}
		...
		this._mode = BX.YNSIRInstantEditorFieldMode.edit;
	},
	_switch2View: function(force)
	{
		...
		if(!force && this._mode === BX.YNSIRInstantEditorFieldMode.view)
		{
			return;
		}
		...
		if(button)
		{
			if(this._isReadOnly)
			{
				...
				button.setAttribute('title', BX.YNSIRInstantEditorMessages.lockButtonTitle);
			}
			else
			{
...
BX.YNSIRInstantEditorField.create = function(container, button, manager)
{
	var self = new BX.YNSIRInstantEditorField();
	self.initialize(container, button, manager);
	return self;
};


BX.YNSIRInstantEditorMessages =
{
