...
BX.YNSIR.RequisitePresetSelectorClass = (function ()
{
	...
	RequisitePresetSelectorClass.prototype = {
		...
		{
			return this.editor.getMessage(msgId);
		},
...
BX.YNSIR.RequisitePopupFormManagerClass = (function ()
{
	...
	RequisitePopupFormManagerClass.prototype = {
		...
		{
			if(BX.type.isFunction(window["cssQuery"]))
			{
				var inputs = window["cssQuery"]("input[type='text'][data-requisite='field'],textarea[data-requisite='field']", this.wrapper);
		...
		onLoadRequestSuccess: function(data)
		{
			...
			BX.addCustomEvent(window, "YNSIRRequisiteEditFormCreate", this.formCreateHandler);
			BX.addCustomEvent(window, "YNSIRFormSettingManagerCreate", BX.delegate(this.onFormManagerCreate, this));
		...
		{
			BX.removeCustomEvent(window, "YNSIRRequisiteEditFormCreate", this.formCreateHandler);

			this.formId = eventArgs["formId"];
			...
			var countryId = BX.type.isNumber(eventArgs["countryId"])
				? eventArgs["countryId"] : 0;

			if(!(enableClientResolution && countryId > 0))
			{
				return;
			}
			...
			{
				case 1:
					typeId = BX.YNSIR.RequisiteFieldType.itin;
					inputName = "RQ_INN";
					break;
				case 14:
					typeId = BX.YNSIR.RequisiteFieldType.sro;
					inputName = "RQ_EDRPOU";
					break;
			}
			...
			if (inputName.length > 0)
			{
				...
				if(input)
				{
					BX.YNSIR.RequisiteFieldController.create(
						inputName,
						{
							countryId: countryId,
							typeId: typeId,
			...
			var editors = BX.YNSIRMultipleAddressEditor.getItemsByFormId(this.formId);
			if(editors.length > 0)
			{
				this.multiAddressEditor = editors[0];
				BX.addCustomEvent(
					this.multiAddressEditor,
					"YNSIRMultipleAddressItemCreated",
		...
		onFormManagerCreate: function(sender)
		{
			...
			BX.addCustomEvent(this.formSettingManager, "YNSIRFormSettingManagerReloadForm", BX.delegate(this.onFormReload, this));
		},
		...
		register: function()
		{
			BX.YNSIR[this._index] = this;
		},
		unregister: function()
		{
			delete BX.YNSIR[this._index];
		},
		destroy: function()
		{
			this.unregister();
...
if(typeof(BX.YNSIR.RequisiteFieldController) === "undefined")
{
	BX.YNSIR.RequisiteFieldController = function()
	{
		this._id = "";
		this._settings = {};
		this._countryId = 0;
		this._typeId = BX.YNSIR.RequisiteFieldType.undefined;
		this._input = null;
		this._value = "";
		this._needle = "";
		this._timeoutId = 0;
	...
	BX.YNSIR.RequisiteFieldController.prototype =
	{
		initialize: function(id, settings)
		{
			...
			this._typeId = this.getSetting("typeId", BX.YNSIR.RequisiteFieldType.undefined);

			this._serviceUrl = this.getSetting("serviceUrl", "");
			if(!BX.type.isNotEmptyString(this._serviceUrl))
			{
				throw "BX.YNSIR.RequisiteFieldController: Could not find 'serviceUrl' parameter in settings.";
			}

			this._input = this.getSetting("input");
			if(!BX.type.isElementNode(this._input))
			{
				throw "BX.YNSIR.RequisiteFieldController: Could not fild 'input' parameter in settings.";
			}
		...
		{
			var result = false;
			if(this._typeId === BX.YNSIR.RequisiteFieldType.itin)
			{
			...
			else if(this._typeId === BX.YNSIR.RequisiteFieldType.sro)
			{
				this._needle = this._value.replace(/[^0-9]/g, "");
				//Ukraine
		...
		openDialog: function(searchResult)
		{
			...
			this._dialog = BX.YNSIR.ExternalRequisiteDialog.create(
				this._id,
	...
	BX.YNSIR.RequisiteFieldController.create = function(id, settings)
	{
		var self = new BX.YNSIR.RequisiteFieldController();
		self.initialize(id, settings);
		return self;
	};
}

if(typeof(BX.YNSIR.ExternalRequisiteDialog) === "undefined")
{
	BX.YNSIR.ExternalRequisiteDialog = function()
	{
		this._id = "";
		this._settings = {};
		this._callbacks = {};
	...
	BX.YNSIR.ExternalRequisiteDialog.prototype =
	{
		initialize: function(id, settings)
		{
			...
			{
				throw "BX.YNSIR.ExternalRequisiteDialog: Could not fild 'items' parameter in settings.";
			}
		...
		getMessage: function(msgId)
		{
			var messages = BX.YNSIR.ExternalRequisiteDialog.messages;
			return messages.hasOwnProperty(msgId) ? messages[msgId] : msgId;
		},
		open: function()
		{
			var id = this.getId();
			if(BX.YNSIR.ExternalRequisiteDialog.windows[id])
			{
				BX.YNSIR.ExternalRequisiteDialog.windows[id].destroy();
			}
			...
			BX.YNSIR.ExternalRequisiteDialog.windows[id] = this._dialog;
			this._dialog.show();
		},
		close: function()
		{
		...
		prepareContent: function()
		{
			...
			if(qty > 0)
			{
				...
				for(var i = 0; i < qty; i++)
				{
					var item = BX.YNSIR.ExternalRequisiteDialogItem.create(
						"",
	...
	if(typeof(BX.YNSIR.ExternalRequisiteDialog.messages) === "undefined")
	{
		BX.YNSIR.ExternalRequisiteDialog.messages =
		{
		};
	}
	BX.YNSIR.ExternalRequisiteDialog.items = {};
	BX.YNSIR.ExternalRequisiteDialog.windows = {};
	BX.YNSIR.ExternalRequisiteDialog.create = function(id, settings)
	{
		var self = new BX.YNSIR.ExternalRequisiteDialog();
		self.initialize(id, settings);
		BX.YNSIR.ExternalRequisiteDialog.items[self.getId()] = self;
		return self;
	};
}

if(typeof(BX.YNSIR.ExternalRequisiteDialogItem) === "undefined")
{
	BX.YNSIR.ExternalRequisiteDialogItem = function()
	{
		this._id = "";
		this._settings = {};
		this._dialog = null;
	...
	BX.YNSIR.ExternalRequisiteDialogItem.prototype =
	{
		initialize: function(id, settings)
		{
			...
			if(!this._dialog)
			{
				throw "BX.YNSIR.ExternalRequisiteDialogItem: Could not fild 'dialog' parameter in settings.";
			}

			this._container = this.getSetting("container");
			if(!BX.type.isElementNode(this._container))
			{
				throw "BX.YNSIR.ExternalRequisiteDialogItem: Could not fild 'container' parameter in settings.";
			}

			this._data = this.getSetting("data");
			if(!BX.type.isPlainObject(this._data))
			{
				throw "BX.YNSIR.ExternalRequisiteDialogItem: Could not fild 'data' parameter in settings.";
			}
		},
		getId: function()
		{
	...
	BX.YNSIR.ExternalRequisiteDialogItem.create = function(id, settings)
	{
		var self = new BX.YNSIR.ExternalRequisiteDialogItem();
		self.initialize(id, settings);
		return self;
	};
}