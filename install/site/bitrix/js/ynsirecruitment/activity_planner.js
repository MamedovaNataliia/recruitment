...
(function(BX)
{
	...
	var YNSIR_ACTIVITY_PLANNER_ID_ATTRIBUTE = 'data-crm-act-planner';
	var DEFAULT_AJAX_URL = '/bitrix/components/ynsirecruitment/activity.planner/ajax.php?site_id=' + BX.message('SITE_ID');
	var COMMUNICATIONS_AJAX_URL = '/bitrix/components/ynsirecruitment/activity.editor/ajax.php?siteID='+BX.message('SITE_ID')+'&sessid='+BX.bitrix_sessid();
	...
	Planner.Manager = {
		...
		{
			var wrapper = BX.findParent(child, {attr: YNSIR_ACTIVITY_PLANNER_ID_ATTRIBUTE});
			if (wrapper)
			{
				return this.get(wrapper.getAttribute(YNSIR_ACTIVITY_PLANNER_ID_ATTRIBUTE));
			}
			return null;
		},
		/**
	...
	Planner.prototype.getPlannerId = function()
	{
		if (typeof this.plannerId === 'undefined')
			this.plannerId = 'ynsir-act-planner-' + Math.round(Math.random() * 100000);
	...
	Planner.prototype.getNodebyName = function(name, scope)
	{
		if (!scope)
			scope = this.getPlannerNode();
	...
	Planner.prototype._createAjaxPopup = function(params, next)
	{
		...
		BX.ajax({
			...
			onsuccess: function (HTML)
			{
				...
				wrapper.setAttribute(YNSIR_ACTIVITY_PLANNER_ID_ATTRIBUTE, me.getPlannerId());
	...
	Planner.prototype.showEdit = function (params)
	{
		...
		this._createAjaxPopup(params, function(editorNode)
		{
			...
			//update by nhatth2
			// LOCATION
			var location_select = me.getNode('location-select');
			if(typeof location_select !== 'undefine' && location_select != null) {
                var Location = me.getNode('location-select').value;
                me.Location = Location;
                BX.bind(me.getNode('location-select'), 'change', function () {
                    me.onLocationChange();
                });
            }
			...
			var guestsContainernode = me.getNode('guests-container');
			var OrderContainernode = me.getNode('order-container');
			if (responsibleContainernode)
			{
                var refreshPlanner = true;
				if(guestsContainernode) var refreshPlanner = false;
			...
			if (guestsContainernode)
			{
                me.guestsDestination = new Destination(
					me.getNode('guests-container'),
					'guests',
					{
						containerTpl: destinationContainerTpl,
						itemTpl: destinationItemTpl,
						valueInputName: 'guestsId[U][]',
						selected: destinationEntities.attendees,
						selectOne: false,
                        refreshPlanner:true,
						required: false,
						events: {
							...
							select: function(params)
							{
                                if (params.item.hasOwnProperty('entityId'))
                                {
                                    var id = parseInt(params.item.entityId);
                                    if (!isNaN(id) && id > 0)
                                    {
                                        if (me.userGuests.indexOf(id) < 0) // Add new user
                                        {
                                            me.userGuests.push(id);
                                        }
                                    }
                                }
								// me.checkPlannerState();
							...
							deleted: function(params) {
                                if (params.item.hasOwnProperty('entityId'))
                                {
                                    ...
                                    if (!isNaN(id) && id > 0)
                                    {
                                        if (me.userGuests.indexOf(id) >= 0) // Add new user
                                        {
                                            me.userGuests.splice(me.userGuests.indexOf(id), 1);
                                        }
                                    }
                                }
                                me.checkPlannerState();
            ...
            {
                me.ordersDestination = new Destination(
                    me.getNode('order-container'),
                    'order',
                    {
                        containerTpl: destinationContainerTpl,
                        itemTpl: destinationItemTpl,
                        valueInputName: 'ORDER[]',
                        selected: destinationEntities.orders,
                        selectOne: true,
                        refreshPlanner:true,
                        required: false,
                        events: {
	...
	Planner.prototype.onPopupClose = function()
	{
		...
		if (this.guestsDestination)
		{
			this.guestsDestination.onPlannerClose();
		}
		if (this.ordersDestination)
		{
			this.ordersDestination.onPlannerClose();
		}
	};
	...
	Planner.prototype.saveActivity = function()
	{
		...
		{
			alert(BX.message('CRM_ACTIVITY_PLANNER_DATES_ERR')+'wew');
			return;
		}
    ...
    Planner.prototype.onLocationChange = function()
    {
    	var me = this;
        var Location = me.getNode('location-select').value;
	...
	Planner.prototype.checkPlannerState = function (params)
	{
		...
		var
			me = this,
			dayLength = 86400000,
			updateParams = {users: [],location:''},
			fromDate = this.getStartTime(),
			toDate = this.getEndTime();

        params = null;
        // guestsId
        var userHost = me.getNodebyName('responsibleId').value;
        updateParams.users.push(parseInt(userHost));

		if (me.userGuests.indexOf(parseInt(userHost)) >= 0) // Remove if current USER is host
		{
			me.userGuests.splice(me.userGuests.indexOf(parseInt(userHost)), 1);
		}

		for(var i = 0; i< me.userGuests.length; i ++)
		{
			updateParams.users.push(me.userGuests[i]);
		}

        updateParams.location = me.Location;
	...
	Planner.prototype.createDiskUploader = function(values, layout)
	{
		...
		me.diskUploader = BX.YNSIRDiskUploader.create(
			'',
			{
				msg:
				{
	...
	var PlannerToolbar = {
		...
		executeActionById: function(id)
		{
			...
			if (action)
			{
				(new BX.YNSIR.Activity.Planner()).showEdit(action.params);
	...
	BX.YNSIR.Activity.Planner = Planner;
	BX.YNSIR.Activity.PlannerToolbar = PlannerToolbar;
	...
	var Destination = function(container, type, config)
	{
		...
		this.refreshPlanner = config.refreshPlanner || false;
	...
	Destination.prototype.initDialog = function(next)
	{
       ...
       var isCrmFeed = false;
        if (this.type === 'order')
        {
            ...
            itemsLast =  {
                deals: data.LAST.ORDERJOB || {},
                crm: []
            };
            searchUrl = DEFAULT_AJAX_URL + '&ajax_action=SEARCH_DESTINATION_JOB_ORDER';
        }
        if (!me.inited)
        {
            me.inited = true;
	...
	Destination.prototype.addItem = function(item, type)
	{
		...
		if (!BX.findChild(items, { attr : { 'data-id' : item.id }}, false, false))
		{
			...
			BX.addClass(container, container.getAttribute('data-class-prefix') + (me.type == 'responsible'||me.type == 'guests' ? 'users' : 'crm'));
			...
			BX.bind(containerDelete, 'click', function(e) {
				...
				else
				{
					me.initDialog(function() {
                        ...
                        me.fireEvent('deleted', {item: item});
	...
	Destination.prototype.addItems = function(items)
	{
		...
		if(this.refreshPlanner)
        	this.fireEvent('RefreshPlanner');
    };
	...
	var Communications = function(container, config)
	{
		...
		this.bindContainer.appendChild(BX.clone(config.containerTpl));
		var tagNode = this.de('destination-tag');
		...
		var communicationType = BX.YNSIRCommunicationType.undefined;
		if (config.communicationType === 'PHONE')
			communicationType = BX.YNSIRCommunicationType.phone;
		...
		this._communicationSearch = BX.YNSIRCommunicationSearch.create(this.id, {
		...
		if (communicationType === BX.YNSIRCommunicationType.phone)
		{
	...
	Communications.prototype.openDialog = function()
	{
		...
		this._communicationSearchController = BX.YNSIRCommunicationSearchController.create(this._communicationSearch, input);
	...
	// Temporary use BX.YNSIRActivityProvider as proxy for old editor.
	if(typeof(BX.YNSIRActivityProvider) == 'undefined')
	{
		...
		BX.YNSIRActivityProvider.prototype =
		{
			initialize: function (settings, editor, options, parentActivity)
			{
				...
				var ownerID = 12;//this.getSetting('ownerID', '');
			...
			openDialog: function (mode)
			{
				...
				if (mode === BX.YNSIRDialogMode.edit)
				{
                    ...
                    (new BX.YNSIR.Activity.Planner()).showEdit({ID:id});
					return true;
				}

				this._dlgMode = mode;

				var dlgId = 'YNSIRActivityProviderView' + id;
				...
				this._titleMenu = BX.YNSIRActivityMenu.create('',
					{
						...
						'enableEmails': this._editor.isEmailsEnabled() && this.getType() !== BX.YNSIRActivityType.email
					},
					{
			...
			_prepareViewDlgButtons: function ()
			{
				...
				if (this.getType() === BX.YNSIRActivityType.email && this._parentActivity)
				{
				...
				else
				{
					...
					if(
						...
						&& (
							this.getType() ===  BX.YNSIRActivityType.call
							|| this.getType() === BX.YNSIRActivityType.meeting
		...
		BX.YNSIRActivityProvider.create = function (settings, editor, options, parentActivity)
		{
	...
	//region BX.YNSIRCustomActivityType
	if(typeof(BX.YNSIRCustomActivityType) == "undefined")
	{
		BX.YNSIRCustomActivityType = function()
		{
		};

		BX.YNSIRCustomActivityType.getListItems = function(infos)
		{
			if(!BX.type.isArray(infos))
			{
				infos = BX.YNSIRCustomActivityType.infos;
			}
		...
		BX.YNSIRCustomActivityType.prepareEditorParams = function(id, params)
		{
		...
		BX.YNSIRCustomActivityType.getInfo = function(id)
		{
			for(var i = 0, l = BX.YNSIRCustomActivityType.infos.length; i < l; i++)
			{
				var info = BX.YNSIRCustomActivityType.infos[i];
				if(info["id"] == id)
				{
					return info;
				}
		...
		if(typeof(BX.YNSIRCustomActivityType.infos) === "undefined")
		{
			BX.YNSIRCustomActivityType.infos = [];
		}
	}
	//endregion
	//region BX.YNSIRCustomActivityTypeSelector
	if(typeof(BX.YNSIRCustomActivityTypeSelector) == "undefined")
	{
		BX.YNSIRCustomActivityTypeSelector = function()
		{
			this._id = "";
			this._settings = {};
			this._ownerTypeId = 0;
		...
		BX.YNSIRCustomActivityTypeSelector.prototype =
		{
			...
			getMessage: function(name)
			{
				var m = BX.YNSIRCustomActivityTypeSelector.messages;
			...
			openCreationDialog: function(typeId)
			{
				...
				BX.YNSIRCustomActivityType.prepareEditorParams(typeId, params);
			...
			openMenu: function(anchor)
			{
				...
				{
					var items = BX.YNSIRCustomActivityType.getListItems();
		...
		if(typeof(BX.YNSIRCustomActivityTypeSelector.messages) === "undefined")
		{
			BX.YNSIRCustomActivityTypeSelector.messages = {};
		}
		BX.YNSIRCustomActivityTypeSelector.items = {};
		BX.YNSIRCustomActivityTypeSelector.create = function(id, settings)
		{
			var self = new BX.YNSIRCustomActivityTypeSelector();
	...
	if(typeof(BX.YNSIRCallListHelper) == "undefined")
	{
		BX.YNSIRCallListHelper = function(){};

		BX.YNSIRCallListHelper.createCallList = function(params, successCallback)
		{
			BX.ajax({
				...
				'data':
				{
					...
					'sessid': BX.bitrix_sessid(),
					'ENTITY_TYPE': params.entityType+'sss',
		...
		BX.YNSIRCallListHelper.addToCallList = function(params)
		{
			...
			BX.ajax({
				...
				onsuccess: function(response)
				{
					...
					else if(response && response.SUCCESS && response.DATA)
					{
						...
						{
							window.alert(response.DATA.MESSAGE+'23');
						}
						BX.localStorage.set(
							"onYNSIRCallListUpdate",
							{
