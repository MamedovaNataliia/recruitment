if(typeof(BX.YNSIRDealStageManager) === "undefined")
{
	BX.YNSIRDealStageManager = function() {};

	BX.YNSIRDealStageManager.prototype =
	{
		getInfos: function(typeId)
		{
			...
			return (BX.type.isArray(BX.YNSIRDealStageManager.infos[typeId]) ? BX.YNSIRDealStageManager.infos[typeId] : []);
		},
		getMessage: function(name)
		{
			var msgs = BX.YNSIRDealStageManager.messages;
	...
	BX.YNSIRDealStageManager.current = new BX.YNSIRDealStageManager();
	BX.YNSIRDealStageManager.infos =
		{
			"category_0":
				[
	...
	BX.YNSIRDealStageManager.messages = {};
}

if(typeof(BX.YNSIRLeadStatusManager) === "undefined")
{
	BX.YNSIRLeadStatusManager = function() {};

	BX.YNSIRLeadStatusManager.prototype =
	{
		getInfos: function(typeId) { return BX.YNSIRLeadStatusManager.infos; },
		getMessage: function(name)
		{
			var msgs = BX.YNSIRLeadStatusManager.messages;
			return BX.type.isNotEmptyString(msgs[name]) ? msgs[name] : "";
		}
	};

	BX.YNSIRLeadStatusManager.current = new BX.YNSIRLeadStatusManager();
	BX.YNSIRLeadStatusManager.infos =
	[
	...
	BX.YNSIRLeadStatusManager.messages = {};
}

if(typeof(BX.YNSIRLeadTerminationControl) === "undefined")
{
	BX.YNSIRLeadTerminationControl = function()
	{
		this._id = "";
		this._settings = null;
		this._entityId = "";
	...
	BX.YNSIRLeadTerminationControl.prototype =
	{
		...
		onDialogOpen: function(sender)
		{
			...
			this._selector = BX.YNSIRLeadConversionSchemeSelector.create(
	...
	BX.YNSIRLeadTerminationControl.create = function(id, settings)
	{
		var self = new BX.YNSIRLeadTerminationControl();
		self.initialize(id, settings);
		return self;
	};
}

if(typeof(BX.YNSIRQuoteStatusManager) === "undefined")
{
	BX.YNSIRQuoteStatusManager = function() {};

	BX.YNSIRQuoteStatusManager.prototype =
	{
		getInfos: function(typeId) { return BX.YNSIRQuoteStatusManager.infos; },
		getMessage: function(name)
		{
			var msgs = BX.YNSIRQuoteStatusManager.messages;
			return BX.type.isNotEmptyString(msgs[name]) ? msgs[name] : "";
		},
		prepareDialogControls: function(dialog)
		{
	...
	BX.YNSIRQuoteStatusManager.current = new BX.YNSIRQuoteStatusManager();
	BX.YNSIRQuoteStatusManager.infos =
		[
	...
	BX.YNSIRQuoteStatusManager.messages = {};
}

if(typeof(BX.YNSIRInvoiceStatusManager) === "undefined")
{
	BX.YNSIRInvoiceStatusManager = function() {
		this.dlg = null;
	};

	BX.YNSIRInvoiceStatusManager.prototype =
	{
		getSetting: function(name, defaultval)
		{
			return (typeof(BX.YNSIRInvoiceStatusManager.settings[name]) !== 'undefined') ?
				BX.YNSIRInvoiceStatusManager.settings[name] : defaultval;
		},
		setSetting: function(name, val)
		{
			BX.YNSIRInvoiceStatusManager.settings[name] = val;
		},
		getInfos: function(typeId) { return BX.YNSIRInvoiceStatusManager.infos; },
		getMessage: function(name)
		{
			var msgs = BX.YNSIRInvoiceStatusManager.messages;
			return BX.type.isNotEmptyString(msgs[name]) ? msgs[name] : "";
		},
		prepareDialogControls: function(dialog)
		{
	...
	BX.YNSIRInvoiceStatusManager.current = new BX.YNSIRInvoiceStatusManager();
	BX.YNSIRInvoiceStatusManager.settings = {};
	BX.YNSIRInvoiceStatusManager.statusInfoValues = [];
	BX.YNSIRInvoiceStatusManager.infos =
		[
	...
	BX.YNSIRInvoiceStatusManager.messages = {};
	BX.YNSIRInvoiceStatusManager.failureDialogEventsBinded = false;

	BX.YNSIRInvoiceStatusManager.failureDialogEventsBind = function() {
		if (!BX.YNSIRInvoiceStatusManager.failureDialogEventsBinded)
		{
			BX.YNSIRInvoiceStatusManager.failureDialogEventsBinded = true;
			BX.addCustomEvent("YNSIRProcessFailureDialogContentCreated", function(val) {
				var self = BX.YNSIRInvoiceStatusManager.current;
				...
				if (entityType === "INVOICE")
				{
					...
					{
						var statusInfoValues = BX.YNSIRInvoiceStatusManager.statusInfoValues[entityId];
						var content = null;
						content = BX.create(
							"DIV",
							{
			...
			BX.addCustomEvent("YNSIRProcessFailureDialogValueChanged", function(failDlg, val) {
				var self = BX.YNSIRInvoiceStatusManager.current;
			...
			BX.addCustomEvent("YNSIRProgressControlBeforeFailureDialogClose", function(progressControl, failDlg) {
				var entityType = failDlg.getEntityType();
				if (entityType === "INVOICE")
				{
					var self = BX.YNSIRInvoiceStatusManager.current;
			...
			BX.addCustomEvent("YNSIRProgressControlBeforeSave", function(progressControl, params)
			{
					...
					for (var name in self.saveParams)
					{
						if (entityId > 0)
						{
							if (name === "REASON_MARKED_SUCCESS")
								...
								valName = name;
							BX.YNSIRInvoiceStatusManager.statusInfoValues[entityId][valName] = self.saveParams[name];
						}
...
if(typeof(BX.YNSIRProgressControl) === "undefined")
{
	BX.YNSIRProgressControl = function()
	{
		this._id = "";
		this._settings = null;
	...
	BX.YNSIRProgressControl.prototype =
	{
		initialize: function(id, settings)
		{
			this._id = id;
			...
			{
				this._manager = BX.YNSIRDealStageManager.current;
			}
			else if(this._entityType === 'LEAD')
			{
				this._manager = BX.YNSIRLeadStatusManager.current;
				this._terminationControl = BX.YNSIRLeadTerminationControl.create(
					this._id,
					BX.YNSIRParamBag.create({ entityId: this._entityId, conversionScheme: this.getSetting("conversionScheme", null) })
				);
			}
			else if(this._entityType === 'QUOTE')
			{
				this._manager = BX.YNSIRQuoteStatusManager.current;
			}
			else if(this._entityType === 'INVOICE')
			{
				this._manager = BX.YNSIRInvoiceStatusManager.current;
			}
		...
		_openTerminationDialog: function()
		{
			...
			this._terminationDlg = BX.YNSIRProcessTerminationDialog.create(
		...
		_openFailureDialog: function()
		{
			...
			var apologies = this._findAllStepInfoBySemantics("apology");
			this._failureDlg = BX.YNSIRProcessFailureDialog.create(
				(this._id + "_FAILURE"),
				BX.YNSIRParamBag.create(
					{
						//"title": this._manager.getMessage("dialogTitle"),
		...
		_onFailureDialogClose: function(dialog, params)
		{
			...
			BX.onCustomEvent(this, 'YNSIRProgressControlBeforeFailureDialogClose', [ this, this._failureDlg ]);
		...
		_save: function()
		{
			...
			BX.onCustomEvent(this, 'YNSIRProgressControlBeforeSave', [ this, data ]);
			...
			BX.ajax(
				{
					...
					{
						BX.onCustomEvent(self, 'YNSIRProgressControlAfterSaveSucces', [ self, data ]);
						BX.YNSIRProgressControl._synchronize(self);
					},
					"onfailure": function(data)
					{
						BX.onCustomEvent(self, 'YNSIRProgressControlAfterSaveFailed', [ self, data ]);
					}
				}
			);
		},
	...
	BX.YNSIRProgressControl.items = {};
	BX.YNSIRProgressControl.create = function(id, settings)
	{
		var self = new BX.YNSIRProgressControl();
		self.initialize(id, settings);
		this.items[id] = self;
		return self;
	};
	BX.YNSIRProgressControl._synchronize = function(item)
	{
...
if(typeof(BX.YNSIRProgressStep) === "undefined")
{
	BX.YNSIRProgressStep = function()
	{
		this._id = "";
		this._settings = null;
		this._control = null;
	...
	BX.YNSIRProgressStep.prototype =
	{
		initialize: function(id, settings)
		{
			this._id = id;
	...
	BX.YNSIRProgressStep.create = function(id, settings)
	{
		var self = new BX.YNSIRProgressStep();
		self.initialize(id, settings);
		return self;
	};
}

if(typeof(BX.YNSIRProcessTerminationDialog) === "undefined")
{
	BX.YNSIRProcessTerminationDialog = function()
	{
		this._id = "";
		this._settings = null;
		this._terminationControl = null;
	...
	BX.YNSIRProcessTerminationDialog.prototype =
	{
		initialize: function(id, settings)
		{
			this._id = id;
			...
			this._openNotifier = BX.YNSIRNotifier.create(this);
		...
		_prepareContent: function()
		{
			...
			else
			{
				...
				if(success)
				{
					...
					BX.YNSIRSubscriber.subscribe(
						this.getId() + "_" + successId,
						successButton, "click", BX.delegate(this._onButtonClick, this),
						BX.YNSIRParamBag.create({ id: successId, preventDefault: true })
					);
				}
			}
			...
			else
			{
				...
				if(failure)
				{
					...
					BX.YNSIRSubscriber.subscribe(
						...
						BX.YNSIRParamBag.create({ id: failureId, preventDefault: true })
					);
				}
			}
	...
	BX.YNSIRProcessTerminationDialog.create = function(id, settings)
	{
		var self = new BX.YNSIRProcessTerminationDialog();
		self.initialize(id, settings);
		return self;
	}
}

if(typeof(BX.YNSIRProcessFailureDialog) === "undefined")
{
	BX.YNSIRProcessFailureDialog = function()
	{
		this._id = "";
		this._settings = null;
		this._popup = null;
	...
	BX.YNSIRProcessFailureDialog.prototype =
	{
		initialize: function(id, settings)
		{
			this._id = id;
			...
			{
				throw "BX.YNSIRProcessFailureDialog: 'success' setting is not found!";
			}

			this._failureInfo = this.getSetting("failure", null);
			if(!this._failureInfo)
			{
				throw "BX.YNSIRProcessFailureDialog: 'failure' setting is not found!";
			}
		...
		setValue: function(val, refresh)
		{
			...
			BX.onCustomEvent(this, 'YNSIRProcessFailureDialogValueChanged', [ this, val ]);
		},
		...
		_prepareContent: function()
		{
			...
			for(var j = 0; j < infos.length; j++)
			{
				...
				BX.YNSIRSubscriber.subscribe(
					...
					BX.YNSIRParamBag.create({ "id": curInfoId })
				);
			...
			BX.onCustomEvent(this, 'YNSIRProcessFailureDialogContentCreated', [ this, wrapper ]);
	...
	BX.YNSIRProcessFailureDialog.create = function(id, settings)
	{
		var self = new BX.YNSIRProcessFailureDialog();
		self.initialize(id, settings);
		return self;
	}
}
