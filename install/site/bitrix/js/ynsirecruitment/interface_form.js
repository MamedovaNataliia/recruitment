function BxYNSIRInterfaceForm(name, aTabs)
{
	var _this = this;
	...
	this.Initialize = function()
	{
		...
		BX.onCustomEvent(window, 'YNSIRInterfaceFormCreated', [ this ]);
	};
	...
	this.SelectTab = function(tab_id)
	{
		...
		BX.onCustomEvent(
			window,
			'BX_YNSIR_INTERFACE_FORM_TAB_SELECTED',
...
BX.CmrSidebarFieldSelector.prototype =
{
	...
	setCurrentItemId: function(itemId, save)
	{
		...
		if(save)
		{
			var editor = BX.YNSIRInstantEditor.getDefault();
			if(editor)
			{
...
BX.YNSIRSidebarUserSelector = function()
{
	this._id = '';
	this._settings = {};
	this._button = null;
...
BX.YNSIRSidebarUserSelector.prototype =
{
	initialize: function(id, button, container, componentName, options)
	{
		...
		{
			throw 'BX.YNSIRSidebarUserSelector: button is not defined';
		}
		...
		{
			throw 'BX.YNSIRSidebarUserSelector: container is not defined';
		}
		...
		{
			throw 'BX.YNSIRSidebarUserSelector: componentName is not defined';
		}
		...
		this._userInfoProvider = BX.YNSIRUserInfoProvider.getItemById(this.getStringOption('userInfoProviderId'));
		...
		if(this._fieldId !== '')
		{
			...
			if(editorId !== '')
			{
				var editor = BX.YNSIRInstantEditor.items[editorId];
				if(editor)
				{
		...
		this._userInfoProvider.getInfo(
			...
			function(userInfo)
			{
				...
				if(self._fieldId.length > 0)
				{
					...
					if(!editor)
					{
						editor = BX.YNSIRInstantEditor.getDefault();
					}
	...
	_setupEditor: function(editor)
	{
		if(this._editor)
		{
			BX.removeCustomEvent(
				this._editor,
				'YNSIRInstantEditorFieldValueSaved',
...
BX.YNSIRSidebarUserSelector.create = function(id, button, container, componentName, options)
{
	var self = new BX.YNSIRSidebarUserSelector();
	self.initialize(id, button, container, componentName, options);
	return self;
};

BX.YNSIRUserSearchField = function()
{
	this._id = '';
...
BX.YNSIRUserSearchField.prototype =
{
	initialize: function(id, search_input, data_input, componentName, user)
	{
		...
		{
			throw  "BX.YNSIRUserSearchField: 'search_input' is not defined!";
		}
		this._search_input = search_input;

		if(!BX.type.isElementNode(data_input))
		{
			throw  "BX.YNSIRUserSearchField: 'data_input' is not defined!";
		}
		this._data_input = data_input;

		if(!BX.type.isNotEmptyString(componentName))
		{
			throw  "BX.YNSIRUserSearchField: 'componentName' is not defined!";
		}
...
BX.YNSIRUserSearchField.items = {};

BX.YNSIRUserSearchField.create = function(id, search_input, data_input, componentName, user)
{
	var self = new BX.YNSIRUserSearchField();
	self.initialize(id, search_input, data_input, componentName, user);
	this.items[id] = self;
	return self;
};

BX.YNSIRUserLinkField = function()
{
	this._settings = {};
	this._container = null;
	this._fieldId = '';
...
BX.YNSIRUserLinkField.prototype =
{
	initialize: function(settings)
	{
		...
		if(!this._container)
		{
			throw 'BX.YNSIRUserLinkField: container is not found';
		}

		this._userInfoProvider = BX.YNSIRUserInfoProvider.getItemById(this.getSetting('userInfoProviderId', ''));
...
BX.YNSIRUserLinkField.create = function(settings)
{
	var self = new BX.YNSIRUserLinkField();
	self.initialize(settings);
	return self;
};

BX.YNSIRUserInfo = function()
{
	this._data = {};
};

BX.YNSIRUserInfo.prototype =
{
...
BX.YNSIRUserInfo.items = {};
BX.YNSIRUserInfo.create = function(data)
{
	var self = new BX.YNSIRUserInfo();
	self.initialize(data);
	this.items[self.getId()] = self;
	return self;
};

BX.YNSIRUserInfoProvider = function()
{
	this._id = '';
	this._settings = {};
	this._serviceUrl = '';
...
BX.YNSIRUserInfoProvider.prototype =
{
	initialize: function(id, settings)
	{
		if(!BX.type.isNotEmptyString(id))
		{
			throw 'BX.YNSIRUserInfoProvider: id is not defined';
		}
		...
		if(serviceUrl === '')
		{
			throw 'BX.YNSIRUserInfoProvider: serviceUrl is not found';
		}
		...
		BX.ajax(
			{
				...
				onsuccess: function(data)
					{
						var item = BX.YNSIRUserInfo.create(data['USER_INFO'] ? data['USER_INFO'] : {});
	...
	{
		var msg = BX.YNSIRUserInfoProvider.messages;
...
BX.YNSIRUserInfoProvider.items = {};
BX.YNSIRUserInfoProvider.getItemById = function(id)
{
	return typeof(this.items[id]) ? this.items[id] : null;
};
BX.YNSIRUserInfoProvider.createIfNotExists = function(id, settings)
{
	...
	var self = new BX.YNSIRUserInfoProvider();
	self.initialize(id, settings);
	this.items[self.getId()] = self;
	return self;
};

if(typeof(BX.YNSIRUserInfoProvider.messages) === 'undefined')
{
	BX.YNSIRUserInfoProvider.messages = {};
}

BX.YNSIRDateLinkField = function()
{
	this._dataElem = null;
	this._viewElem = null;
	this._settings = {};
...
BX.YNSIRDateLinkField.prototype =
{
	initialize: function(dataElem, viewElem, settings)
	{
		if(!BX.type.isElementNode(dataElem))
		{
			throw "BX.YNSIRDateLinkField: 'dataElem' is not defined!";
		}
...
BX.YNSIRDateLinkField.create = function(dataElem, viewElem, settings)
{
	var self = new BX.YNSIRDateLinkField();
	self.initialize(dataElem, viewElem, settings);
	return self;
};

//region BX.YNSIRCompanyContactData
BX.YNSIRCompanyContactData = function()
{
	this._companyId = 0;
	this._contactIds = [];
};

BX.YNSIRCompanyContactData.prototype =
{
...
BX.YNSIRCompanyContactData.create = function(params)
{
	var self = new BX.YNSIRCompanyContactData();
	self.initialize(params);
	return self;
};
//endregion
//region BX.YNSIRCompanyContactEditor
BX.YNSIRCompanyContactEditor = function()
{
	this._id = '';
	this._settings = {};
	this._data = {};
...
BX.YNSIRCompanyContactEditor.prototype =
{
	initialize: function(id, settings)
	{
		this._id = BX.type.isNotEmptyString(id) ? id : "YNSIR_COMPANY_CONTACT_EDITOR" + Math.random();
		this._settings = settings ? settings : {};

		this._data = this.getSetting("data", null);
		if(!(this._data instanceof BX.YNSIRCompanyContactData))
		{
			this._data = BX.YNSIRCompanyContactData.create(null);
		}
	},
	getSetting: function (name, defaultval)
	{
...
BX.YNSIRCompanyContactEditor.create = function(id, settings)
{
	var self = new BX.YNSIRCompanyContactEditor();
	self.initialize(id, settings);
	return self;
};

BX.YNSIREntityEditor = function()
{
	this._id = '';
	this._settings = {};
...
BX.YNSIREntityEditor.prototype =
{
	initialize: function(id, settings, data, info)
	{
		this._id = BX.type.isNotEmptyString(id) ? id : 'YNSIR_ENTITY_EDITOR' + Math.random();
		this._settings = settings ? settings : {};
		...
		if(!data)
		{
			throw "BX.YNSIREntityEditor: Could not find data!";
		}

		this._data = data;

		this._info = info ? info : BX.YNSIREntityInfo.create();

		var selectorId = this.getSetting('entitySelectorId', '');
		if(obYNSIR && obYNSIR[selectorId])
		{
			var selector = this._selector = obYNSIR[selectorId];
		...
		if(!c)
		{
			throw "BX.YNSIREntityEditor: Could not find field container!";
		}
	...
	getMessage: function(name)
	{
		var msgs = BX.YNSIREntityEditor.messages;
	...
	openDialog: function(anchor, mode)
	{
		...
		{
			case 'CONTACT':
				this._dlg = BX.YNSIRContactEditDialog.create(
					this._id,
				...
				break;
			case 'COMPANY':
				this._dlg = BX.YNSIRCompanyEditDialog.create(
					this._id,
					this.getSetting('dialog', {}),
					this._data,
					BX.delegate(this._onSaveDialogData, this));
	...
	_prepareData: function(settings)
	{
		...
		if(typeName === 'CONTACT')
		{
			...
			return BX.YNSIRContactData.create(settings);
		}
		if(typeName === 'COMPANY')
		{
			if(settings && enablePrefix)
				settings['id'] = 'CO_' + settings['id'];
			return BX.YNSIRCompanyData.create(settings);
		}
		if(typeName === 'LEAD')
		{
			if(settings && enablePrefix)
				settings['id'] = 'L_' + settings['id'];
			return BX.YNSIRLeadData.create(settings);
		}
		if(typeName === 'DEAL')
		{
			if(settings && enablePrefix)
				settings['id'] = 'D_' + settings['id'];
			return BX.YNSIRDealData.create(settings);
		}
		if(typeName === 'QUOTE')
		{
			if(settings && enablePrefix)
				settings['id'] = 'Q_' + settings['id'];
			return BX.YNSIRQuoteData.create(settings);
		}
		return null;
	},
	_onDeleteButtonClick: function(e)
	{
		...
		BX.onCustomEvent('YNSIREntitySelectorChangeValue', [this.getId(), this.getTypeName(), 0, this]);
	},
	...
	_onExternalEvent: function(params)
	{
			...
			&& BX.type.isPlainObject(this._externalRequestData[context]))
		{
			...
			if(!isCanceled)
			{
				...
				this._info = BX.YNSIREntityInfo.create(entityInfo);
				...
				if(newDataInput)
				{
					...
					BX.onCustomEvent(
						"YNSIREntitySelectorChangeValue",
		...
		BX.ajax(
			{
				...
				onsuccess: function(data)
				{
					...
					{
						self._showDialogError('BX.YNSIREntityEditor: Could not find contact data!');
					}
					else
					{
						self._data = self._prepareData(data['DATA']);
						self._info = BX.YNSIREntityInfo.create(data['INFO'] ? data['INFO'] : {});

						var newDataInput = BX(self.getSetting('newDataInputId', ''));
						if(newDataInput)
						{
							newDataInput.value = self._data.getId();
							BX.onCustomEvent('YNSIREntitySelectorChangeValue', [self.getId(), self.getTypeName(), self._data.getId(), self]);
						}
	...
	_continueLayout: function(afterLayout)
	{
		...
		this._blockArea = new BX.YNSIR.EntityEditorBlockAreaClass({
			editor: this,
	...
	_onEntitySelect: function(settings)
	{
		...
		this._info = BX.YNSIREntityInfo.create(item);

		this._setRqLinkedId(0, 0);

		var self = this;
		this.layout(function() {
			BX.onCustomEvent('YNSIREntitySelectorChangeValue', [self.getId(), self.getTypeName(), item['id'], self]);
		});
	},
...
if(typeof(BX.YNSIREntityEditor.messages) === 'undefined')
{
	BX.YNSIREntityEditor.messages = {};
}

BX.YNSIREntityEditor.items = {};

BX.YNSIREntityEditor.create = function(id, settings, data, info)
{
	var self = new BX.YNSIREntityEditor();
	self.initialize(id, settings, data, info);
	this.items[id] = self;
	return self;
};

BX.YNSIRContactEditDialog = function()
{
	this._id = '';
	this._settings = {};
	this._dlg = null;
...
BX.YNSIRContactEditDialog.prototype =
{
	initialize: function(id, settings, data, onSaveCallback)
	{
		this._id = BX.type.isNotEmptyString(id) ? id : 'YNSIR_CONTACT_EDIT_DIALOG_' + Math.random();
		this._settings = settings ? settings : {};
		this._data = data ? data : BX.YNSIRContactData.create();
	...
	_prepareContent: function()
	{
		...
		wrapper.appendChild(BX.YNSIRPopupWindowHelper.prepareTextField({ id: this._getElementId('lastName'), title: this.getSetting('lastNameTitle', 'Last Name'), value: data.getLastName() }));
		wrapper.appendChild(BX.YNSIRPopupWindowHelper.prepareTextField({ id: this._getElementId('name'), title: this.getSetting('nameTitle', 'Name'), value: data.getName() }));
		wrapper.appendChild(BX.YNSIRPopupWindowHelper.prepareTextField({ id: this._getElementId('secondName'), title: this.getSetting('secondNameTitle', 'Second Name'), value: data.getSecondName() }));
		if(this.getSetting('enableEmail', true))
		{
			wrapper.appendChild(BX.YNSIRPopupWindowHelper.prepareTextField({ id: this._getElementId('email'), title: this.getSetting('emailTitle', 'E-mail'), value: data.getEmail() }));
		}
		if(this.getSetting('enablePhone', true))
		{
			wrapper.appendChild(BX.YNSIRPopupWindowHelper.prepareTextField({ id: this._getElementId('phone'), title: this.getSetting('phoneTitle', 'Phone'), value: data.getPhone() }));
		}
		if(this.getSetting('enableExport', true))
		{
			...
			wrapper.appendChild(BX.YNSIRPopupWindowHelper.prepareCheckBoxField({ id: this._getElementId('export'), title: this.getSetting('exportTitle', 'Enable Export'), value: data.isExportable() }));
		}
		return wrapper;
	},
	_prepareButtons: function()
	{
		return BX.YNSIRPopupWindowHelper.prepareButtons(
			[
				{
					type: 'button',
					settings:
...
BX.YNSIRContactEditDialog.create = function(id, settings, data, onSaveCallback)
{
	var self = new BX.YNSIRContactEditDialog();
	self.initialize(id, settings, data, onSaveCallback);
	return self;
};

BX.YNSIRCompanyEditDialog = function()
{
	this._id = '';
	this._settings = {};
	this._dlg = null;
...
BX.YNSIRCompanyEditDialog.prototype =
{
	initialize: function(id, settings, data, onSaveCallback)
	{
		this._id = BX.type.isNotEmptyString(id) ? id : 'YNSIR_COMPANY_EDIT_DIALOG_' + Math.random();
		this._settings = settings ? settings : {};
		this._data = data ? data : BX.YNSIRCompanyData.create();
		this._onSaveCallback = BX.type.isFunction(onSaveCallback) ? onSaveCallback : null;
	},
	getSetting: function (name, defaultval)
	{
	...
	_prepareContent: function()
	{
		...
		wrapper.appendChild(BX.YNSIRPopupWindowHelper.prepareTextField({ id: this._getElementId('title'), title: this.getSetting('titleTitle', 'Title'), value: data.getTitle() }));
		wrapper.appendChild(BX.YNSIRPopupWindowHelper.prepareSelectField({ id: this._getElementId('companyType'), title: this.getSetting('companyTypeTitle', 'Company Type'), value: data.getCompanyType(), items: this.getSetting('companyTypeItems', null) } ));
		wrapper.appendChild(BX.YNSIRPopupWindowHelper.prepareSelectField({ id: this._getElementId('industry'), title: this.getSetting('industryTitle', 'Industry'), value: data.getIndustry(), items: this.getSetting('industryItems', null) }));
		wrapper.appendChild(BX.YNSIRPopupWindowHelper.prepareTextField({ id: this._getElementId('email'), title: this.getSetting('emailTitle', 'E-mail'), value: data.getEmail() }));
		wrapper.appendChild(BX.YNSIRPopupWindowHelper.prepareTextField({ id: this._getElementId('phone'), title: this.getSetting('phoneTitle', 'Phone'), value: data.getPhone() }));
		return wrapper;
	},
...
BX.YNSIRCompanyEditDialog.create = function(id, settings, data, onSaveCallback)
{
	var self = new BX.YNSIRCompanyEditDialog();
	self.initialize(id, settings, data, onSaveCallback);
	return self;
};

BX.YNSIRContactData = function()
{
	this._id = 0;
...
BX.YNSIRContactData.create = function(settings)
{
	var self = new BX.YNSIRContactData();
	self.initialize(settings);
	return self;
};

BX.YNSIRCompanyData = function()
{
	this._id = 0;
	this._title = this._companyType = this._industry = this._email = this._phone = this._addressLegal = '';
};

BX.YNSIRCompanyData.prototype =
{
	initialize: function(settings)
	{
		if(!settings)
...
BX.YNSIRCompanyData.create = function(settings)
{
	var self = new BX.YNSIRCompanyData();
	self.initialize(settings);
	return self;
};

BX.YNSIRLeadData = function()
{
	this._id = 0;
	this._title = this._name = this._secondName = this._lastName = this._email = this._phone = '';
};

BX.YNSIRLeadData.prototype =
{
	initialize: function(settings)
	{
		if(!settings)
...
BX.YNSIRLeadData.create = function(settings)
{
	var self = new BX.YNSIRLeadData();
	self.initialize(settings);
	return self;
};

BX.YNSIRDealData = function()
{
	this._id = this._dealPrice = 0;
	this._title = this._dealType = '';
};

BX.YNSIRDealData.prototype =
{
	initialize: function(settings)
	{
		if(!settings)
...
BX.YNSIRDealData.create = function(settings)
{
	var self = new BX.YNSIRDealData();
	self.initialize(settings);
	return self;
};

BX.YNSIRQuoteData = function()
{
	this._id = this._quotePrice = 0;
	this._title = this._quoteType = '';
};

BX.YNSIRQuoteData.prototype =
{
	initialize: function(settings)
	{
		if(!settings)
...
BX.YNSIRQuoteData.create = function(settings)
{
	var self = new BX.YNSIRQuoteData();
	self.initialize(settings);
	return self;
};

//region BX.YNSIRCalltoFormat
...
//endregion
//region BX.YNSIREntityInfo
if(typeof(BX.YNSIREntityInfo) === "undefined")
{
	BX.YNSIREntityInfo = function()
	{
		this._settings = {};
	};
	BX.YNSIREntityInfo.prototype =
	{
		initialize: function(settings)
		{
		...
		getTypeName: function()
		{
			...
			{
				result = BX.YNSIREntityType.verifyName(this.getSetting("type", ""));
			}
			return result;
		},
		getTypeId: function()
		{
			return BX.YNSIREntityType.resolveId(this.getTypeName());
		},
		getTitle: function()
		{
		...
		{
			return this.getSetting("image", "");
		},
		getLargeImageUrl: function()
		{
			return this.getSetting("largeImage", "");
		},
		...
		getMultiFieldsByType: function(type)
		{
			...
			for (var i = 0; i < fields.length; i++)
			{
				...
				{
					results.push({ "TYPE": type,  "VALUE": field["VALUE"], "ID": field["ID"], "ENTITY_ID": field["ENTITY_ID"]});
				}
			}
			return results;
		},
		...
		prepareRequisiteData: function(context)
		{
			...
			for(var i = 0; i < requisites.length; i++)
			{
				...
				var viewData = BX.type.isPlainObject(data["viewData"]) ? data["viewData"] : {};
				requisite["viewData"] = viewData;
	...
	BX.YNSIREntityInfo.equals = function(a, b)
	{
		return (a.getId() === b.getId() && a.getTypeName() === b.getTypeName());
	};
	BX.YNSIREntityInfo.create = function(settings)
	{
		var self = new BX.YNSIREntityInfo();
		self.initialize(settings);
		return self;
	};
}
//endregion
//region BX.YNSIREntityRequisiteInfo
if(typeof(BX.YNSIREntityRequisiteInfo) === "undefined")
{
	BX.YNSIREntityRequisiteInfo = function()
	{
		this._settings = {};
		this._requisiteId = 0;
	...
	BX.YNSIREntityRequisiteInfo.prototype =
	{
		initialize: function(settings)
		{
			...
			for(var i = 0; i < this._data.length; i++)
			{
				...
				var viewData = item["viewData"] = BX.type.isPlainObject(data["viewData"]) ? data["viewData"] : {};
	...
	BX.YNSIREntityRequisiteInfo.create = function(settings)
	{
		var self = new BX.YNSIREntityRequisiteInfo();
		self.initialize(settings);
		return self;
	};
}
//endregion
//region BX.YNSIRMultipleEntitySummaryView
if(typeof(BX.YNSIRMultipleEntitySummaryView) === "undefined")
{
	BX.YNSIRMultipleEntitySummaryView = function()
	{
		this._id = "";
		this._settings = {};
	...
	BX.YNSIRMultipleEntitySummaryView.prototype =
	{
		initialize: function(id, settings)
		{
			this._id = id;
			...
			{
				throw "YNSIRMultipleEntitySummaryView: Could not find 'entityType' parameter in settings.";
			}
			...
			if(!BX.type.isElementNode(this._container))
			{
				...
				{
					throw "YNSIRMultipleEntitySummaryView: Could not find 'containerId' parameter in settings.";
				}

				this._container = BX(this._containerId);
				if(!BX.type.isElementNode(this._container))
				{
					throw "YNSIRMultipleEntitySummaryView: Could not find container element.";
				}
			}
			...
			{
				throw "YNSIRMultipleEntitySummaryView: Could not find 'entityInfos' parameter in settings.";
			}
			...
			if(!this._areEntitiesLoaded)
			{
				...
				{
					throw "YNSIRMultipleEntitySummaryView: Could not find owner config.";
				}

				this._loaderCfg = this.getSetting("loader", null);
				if(!BX.type.isPlainObject(this._loaderCfg))
				{
					throw "YNSIRMultipleEntitySummaryView: Could not find loader config.";
				}
			}
			...
			this._deletionNotifier = BX.YNSIRNotifier.create(this);
		},
		getSetting: function(name, defaultval)
		{
			return this._settings.hasOwnProperty(name) ? this._settings[name] : defaultval;
		...
		getMessage: function(name)
		{
			var m = BX.YNSIRMultipleEntitySummaryView.messages;
		...
		initializaControls: function()
		{
			...
			for(var i = 0; i < this._entityCount; i++)
			{
				...
				var isStub = !(entityInfo instanceof BX.YNSIREntityInfo) && !BX.type.isPlainObject(entityInfo);

				var view = BX.YNSIREntitySummaryView.create(
		...
		loadEntities: function()
		{
			...
			this._dataLoader = BX.YNSIRDataLoader.create(
				this._id,
				{
	...
	BX.YNSIRMultipleEntitySummaryView.create = function(id, settings)
	{
		var self = new BX.YNSIRMultipleEntitySummaryView();
		self.initialize(id, settings);
		return self;
	};
}
//endregion
//region BX.YNSIREntitySummaryView
if(typeof(BX.YNSIREntitySummaryView) === "undefined")
{
	BX.YNSIREntitySummaryView = function()
	{
		this._id = "";
		this._settings = {};
		this._hasLayout = false;
	...
	BX.YNSIREntitySummaryView.prototype =
	{
		initialize: function(id, settings)
		{
			this._id = id;
			...
			if(!BX.type.isElementNode(this._container))
			{
				...
				if(!BX.type.isElementNode(this._container))
				{
					throw "YNSIREntitySummaryView: Could not find 'container' parameter in settings.";
				}

			}

			var entityInfo = this.getSetting("entityInfo");
			this._entityInfo = entityInfo instanceof BX.YNSIREntityInfo
				? entityInfo : BX.YNSIREntityInfo.create(entityInfo);
			...
			if(this._enableRequisites)
			{
				...
				if(requisiteInfo instanceof BX.YNSIREntityRequisiteInfo)
				{
					this._requisiteInfo = requisiteInfo;
				}
				else
				{
					this._requisiteInfo = BX.YNSIREntityRequisiteInfo.create(
						{
							requisiteId: 0,
							bankDetailId: 0,
			...
			this._requisiteChangeNotifier = BX.YNSIRNotifier.create(this);
			this._deletionNotifier = BX.YNSIRNotifier.create(this);
		},
		getSetting: function(name, defaultval)
		{
		...
		getMessage: function(name)
		{
			var m = BX.YNSIREntitySummaryView.messages;
			return m.hasOwnProperty(name) ? m[name] : name;
		},
		isStub: function()
		{
		...
		layout: function()
		{
			...
			{
				this._clientPanel = BX.YNSIRClientPanel.create("",
					{
	...
	if(typeof(BX.YNSIREntitySummaryView.messages) === "undefined")
	{
		BX.YNSIRMultipleEntitySummaryView.messages = {};
	}
	BX.YNSIREntitySummaryView.create = function(id, settings)
	{
		var self = new BX.YNSIREntitySummaryView();
		self.initialize(id, settings);
		return self;
	};
}
//endregion
//region BX.YNSIRClientPanel
if(typeof(BX.YNSIRClientPanel) === "undefined")
{
	BX.YNSIRClientPanel = function()
	{
		this._id = "";
		this._settings = {};
		this._entityInfo = null;
	...
	BX.YNSIRClientPanel.prototype =
	{
		initialize: function(id, settings)
		{
			this._id = BX.type.isNotEmptyString(id) ? id : "YNSIR_CLIENT_PANEL_" + Math.random();
			this._settings = BX.type.isPlainObject(settings) ? settings : {};

			this._container = this.getSetting("container");
			if(!BX.type.isElementNode(this._container))
			{
				throw "YNSIRClientPanel: Could not find 'container' parameter in settings.";
			}

			this._entityInfo = this.getSetting("entityInfo");
			if(!(this._entityInfo instanceof BX.YNSIREntityInfo))
			{
				throw "YNSIRClientPanel: Could not find 'entityInfo' parameter in settings.";
			}
			...
			if(this._enableRequisites)
			{
				...
				if(!(this._requisiteInfo instanceof BX.YNSIREntityRequisiteInfo))
				{
					throw "YNSIRClientPanel: Could not find 'requisiteInfo' parameter in settings.";
				}
			}
		...
		getMessage: function(name)
		{
			var m = BX.YNSIRClientPanel.messages;
			return m.hasOwnProperty(name) ? m[name] : name;
		},
		layout: function()
		{
			...
			if(typeName === BX.YNSIREntityType.names.contact)
			{
				BX.addClass(wrapper, "crm-client-selector-tab-contact");
			}
			else if(typeName === BX.YNSIREntityType.names.company)
			{
		...
		setActiveTab: function(tabName)
		{
			...
			if(tabName === "main")
			{
				if(!this._multifieldPanel)
				{
					this._multifieldPanel = BX.YNSIRClientMultiFieldPanel.create(this._id,
						{
			...
			else if(tabName === "requisite")
			{
				...
				{
					this._requisitePanel = BX.YNSIRClientRequisitePanel.create(this._id,
						{
							container: activeTab["node"],
							entityInfo: this._entityInfo,
							requisiteInfo: this._requisiteInfo,
	...
	if(typeof(BX.YNSIRClientPanel.messages) === "undefined")
	{
		BX.YNSIRClientPanel.messages = {};
	}
	BX.YNSIRClientPanel.create = function(id, settings)
	{
		var self = new BX.YNSIRClientPanel();
		self.initialize(id, settings);
		return self;
	};
}
//endregion
//region BX.YNSIRClientMultiFieldPanel
if(typeof(BX.YNSIRClientMultiFieldPanel) === "undefined")
{
	BX.YNSIRClientMultiFieldPanel = function()
	{
		this._id = "";
		this._settings = {};
		this._hasLayout = false;
	...
	BX.YNSIRClientMultiFieldPanel.prototype =
	{
		initialize: function(id, settings)
		{
			this._id = BX.type.isNotEmptyString(id) ? id : "YNSIR_CLIENT_MULTIFIELDS" + Math.random();
			this._settings = settings ? settings : {};
			...
			if(!(this._entityInfo instanceof BX.YNSIREntityInfo))
			{
				throw "YNSIRClientMultiFieldPanel: Could not find 'entityInfo' parameter in settings.";
			}

			this._container = this.getSetting("container");
			if(!BX.type.isElementNode(this._container))
			{
				throw "YNSIRClientMultiFieldPanel: Could not find 'container' parameter in settings.";
			}
		},
		getSetting: function(name, defaultval)
		{
		...
		{
			var item = BX.YNSIRClientPanelCommunication.create(
				"",
				{
					container: container,
	...
	BX.YNSIRClientMultiFieldPanel.create = function(id, settings)
	{
		var self = new BX.YNSIRClientMultiFieldPanel();
		self.initialize(id, settings);
		return self;
	}
}
//endregion
//region BX.YNSIRClientPanelCommunication
if(typeof(BX.YNSIRClientPanelCommunication) === "undefined")
{
	BX.YNSIRClientPanelCommunication = function()
	{
		this._id = "";
		this._settings = {};
	...
	BX.YNSIRClientPanelCommunication.prototype =
	{
		initialize: function(id, settings)
		{
			this._id = BX.type.isNotEmptyString(id) ? id : "YNSIR_CLIENT_COMM" + Math.random();
			this._settings = settings ? settings : {};

			this._fieldInfo = this.getSetting("fieldInfo");
			if(!BX.type.isPlainObject(this._fieldInfo))
			{
				throw "YNSIRClientPanelCommunication: Could not find 'fieldInfo' parameter in settings.";
			}
			...
			if(!BX.type.isElementNode(this._container))
			{
				throw "YNSIRClientPanelCommunication: Could not find 'container' parameter in settings.";
			}
		},
		getSetting: function(name, defaultval)
		{
		...
		layout: function()
		{
			...
			if(type === "PHONE")
			{
				var callToFormat = BX.YNSIRClientPanelCommunication.callToFormat;
				...
				this._link = BX.create("A",
					{
						...
						props: { href: (callToFormat === BX.YNSIRCalltoFormat.standard ? "callto://" : "callto:") + BX.util.urlencode(value) },
						text: value
					}
				);
				...
				this._link = BX.create("A",
					{
						...
						props: { href: "mailto:" + BX.util.urlencode(value) },
						text: value
					}
				);
				innerWrapper.appendChild(this._link);
		...
		onButtonClick: function(e)
		{
			...
			else if(type === "PHONE" && typeof(BX.YNSIRSipManager) !== 'undefined')
			{
				BX.YNSIRSipManager.startCall(
					...
					{
						ENTITY_TYPE:  BX.YNSIRSipManager.resolveSipEntityTypeName(this._entityTypeName),
	...
	if(typeof(BX.YNSIRClientPanelCommunication.callToFormat) !== "undefined")
	{
		BX.YNSIRClientPanelCommunication.callToFormat = BX.YNSIRCalltoFormat.bitrix;
	}
	BX.YNSIRClientPanelCommunication.create = function(id, settings)
	{
		var self = new BX.YNSIRClientPanelCommunication();
		self.initialize(id, settings);
		return self;
	}
}
//endregion
//region BX.YNSIRClientRequisitePanel
if(typeof(BX.YNSIRClientRequisitePanel) === "undefined")
{
	BX.YNSIRClientRequisitePanel = function()
	{
		this._id = "";
		this._settings = {};
		this._hasLayout = false;
	...
	BX.YNSIRClientRequisitePanel.prototype =
	{
		initialize: function(id, settings)
		{
			this._id = BX.type.isNotEmptyString(id) ? id : "YNSIR_CLIENT_REQUISITE" + Math.random();
			this._settings = settings ? settings : {};

			this._entityInfo = this.getSetting("entityInfo");
			if(!(this._entityInfo instanceof BX.YNSIREntityInfo))
			{
				throw "YNSIRClientRequisitePanel: Could not find 'entityInfo' parameter in settings.";
			}

			this._requisiteInfo = this.getSetting("requisiteInfo");
			if(!(this._requisiteInfo instanceof BX.YNSIREntityRequisiteInfo))
			{
				throw "YNSIRClientRequisitePanel: Could not find 'requisiteInfo' parameter in settings.";
			}
			...
			if(!BX.type.isElementNode(this._container))
			{
				throw "YNSIRClientRequisitePanel: Could not find 'container' parameter in settings.";
			}

			this._readOnly = !!this.getSetting("readOnly");
			this._changeNotifier = BX.YNSIRNotifier.create(this);
		},
		getSetting: function(name, defaultval)
		{
			return this._settings.hasOwnProperty(name) ? this._settings[name] : defaultval;
		...
		layout: function()
		{
			...
			for(var i = 0; i < requisites.length; i++)
			{
				...
				var item = BX.YNSIRClientRequisitePanelItem.create(
	...
	BX.YNSIRClientRequisitePanel.create = function(id, settings)
	{
		var self = new BX.YNSIRClientRequisitePanel();
		self.initialize(id, settings);
		return self;
	};
}
//endregion
//region BX.YNSIRClientRequisitePanelItem
if(typeof(BX.YNSIRClientRequisitePanelItem) === "undefined")
{
	BX.YNSIRClientRequisitePanelItem = function()
	{
		this._id = "";
		this._settings = {};
		this._name = "";
	...
	BX.YNSIRClientRequisitePanelItem.prototype =
	{
		initialize: function (id, settings)
		{
			this._id = BX.type.isNotEmptyString(id) ? id : "YNSIR_CLIENT_REQUISITE_ITEM" + Math.random();
			this._settings = settings ? settings : {};
			...
			if(!BX.type.isElementNode(this._container))
			{
				throw "YNSIRClientRequisitePanelItem: Could not find 'container' parameter in settings.";
			}

			this._data = this.getSetting("data");
			if(!BX.type.isPlainObject(this._data))
			{
				throw "YNSIRClientRequisitePanelItem: Could not find 'data' parameter in settings.";
			}
			...
			this._changeNotifier = BX.YNSIRNotifier.create(this);
			this._bankDetailSelectNotifier = BX.YNSIRNotifier.create(this);
		},
		getSetting: function(name, defaultval)
		{
			return this._settings.hasOwnProperty(name) ? this._settings[name] : defaultval;
		...
		getMessage: function(name)
		{
			var m = BX.YNSIRClientRequisitePanelItem.messages;
			return m.hasOwnProperty(name) ? m[name] : name;
		},
		getRequisiteId: function()
		{
	...
	if(typeof(BX.YNSIRClientRequisitePanelItem.messages) === "undefined")
	{
		BX.YNSIRClientRequisitePanelItem.messages = {};
	}
	BX.YNSIRClientRequisitePanelItem.create = function(id, settings)
	{
		var self = new BX.YNSIRClientRequisitePanelItem();
		self.initialize(id, settings);
		return self;
	};
}
//endregion
//region BX.YNSIRCompositeClientSelector
if(typeof(BX.YNSIRCompositeClientSelector) === "undefined")
{
	BX.YNSIRCompositeClientSelector = function()
	{
		this._id = "";
		this._settings = {};
	...
	BX.YNSIRCompositeClientSelector.prototype =
	{
		initialize: function(id, settings)
		{
			this._id = id;
			...
			if(!BX.type.isNotEmptyString(this._containerId))
			{
				throw "YNSIRCompositeClientSelector: Could not find 'containerId' parameter in settings.";
			}

			this._container = BX(this._containerId);
			if(!BX.type.isElementNode(this._container))
			{
				throw "YNSIRCompositeClientSelector: Could not find container element.";
			}

			this._owner = this.getSetting("owner", null);
			if(!BX.type.isPlainObject(this._owner))
			{
				throw "YNSIRCompositeClientSelector: Could not find owner config.";
			}
			...
			var primaryEntityInfo = BX.type.isPlainObject(primaryEntityData) ? BX.YNSIREntityInfo.create(primaryEntityData) : null;
			...
			{
				var info = BX.YNSIREntityInfo.create(secondaryEntityData[i]);
				if(info.getId() > 0)
				{
		...
		getMessage: function(name)
		{
			var m = BX.YNSIRCompositeClientSelector.messages;
			return m.hasOwnProperty(name) ? m[name] : name;
		},
		getEntityTypeName: function()
		{
		...
		setEntityTypeName: function(entityType)
		{
			...
			if(this._hasLayout)
			{
				this._typeSelectorButtonContent.innerHTML = BX.util.htmlspecialchars(
					BX.YNSIREntityType.getCaptionByName(this._entityTypeName) + " "
				);
		...
		setPrimaryEntity: function(entityInfo)
		{
			...
			if(entityInfo instanceof BX.YNSIREntityInfo)
			{
		...
		{
			this._primaryEntityRequisiteInfo = BX.YNSIREntityRequisiteInfo.create(
				{
		...
		getCreateUrl: function()
		{
			if(this._entityTypeName === BX.YNSIREntityType.names.company)
			{
				return this.getCreateCompanyUrl();
			}
			else if(this._entityTypeName === BX.YNSIREntityType.names.contact)
			{
		...
		prepareEntitySelector: function()
		{
			...
			if (obYNSIR[this._entitySelectorId])
			{
				obYNSIR[this._entitySelectorId].Clear();
				delete obYNSIR[this._entitySelectorId];
			}
			...
			obYNSIR[this._entitySelectorId].AddOnSaveListener(this._entitySelectHandler);
		},
		prepareViews: function()
		{
			...
			{
				var skipPrimary = this._primaryEntityInfo.getTypeName() === BX.YNSIREntityType.names.contact;
				for(var j = 0; j < this._secondaryEntityInfos.length; j++)
				{
					var entityInfo = this._secondaryEntityInfos[j];
					if(skipPrimary && BX.YNSIREntityInfo.equals(entityInfo, this._primaryEntityInfo))
					{
						continue;
					}
				...
				this._primaryView = BX.YNSIREntitySummaryView.create(
					this._id,
					{
			...
			if(!this._enableMultiplicity && this._primaryEntityInfo && this._primaryEntityInfo.getTypeName() === BX.YNSIREntityType.names.contact)
			{
			...
			if(this._primaryEntityInfo || secondaryEntityInfos.length > 0)
			{
				...
				if(this._primaryEntityInfo && this._primaryEntityInfo.getTypeName() === BX.YNSIREntityType.names.company)
				{
				...
				var enableMarking = (this._primaryEntityInfo
						...
						: this._entityTypeName) === BX.YNSIREntityType.names.contact;

				this._auxiliaryGroup = BX.YNSIRClientPanelGroup.create(
					this._id,
					{
		...
		layout: function()
		{
			...
			if(!this._readOnly)
			{
				...
				this._typeSelectorButtonContent = BX.create("SPAN",
					{ text: BX.YNSIREntityType.getCaptionByName(this.getEntityTypeName()) + " " }
				);
		...
		notify: function(target)
		{
			...
			BX.onCustomEvent("YNSIRClientSelectorChange", [ this, eventArgs ]);
		},
				...
				this._selectorMenu = BX.CmrSelectorMenu.create(
					this._id,
					{
						items:
							[
								{
									text: BX.YNSIREntityType.getCaption(BX.YNSIREntityType.enumeration.company),
									value: BX.YNSIREntityType.names.company
								},
								{
									text: BX.YNSIREntityType.getCaption(BX.YNSIREntityType.enumeration.contact),
									value: BX.YNSIREntityType.names.contact
								}
							]
					}
				);
		...
		onEntitySelectClick: function(e)
		{
			...
			if (obYNSIR[this._entitySelectorId])
			{
				var selector = obYNSIR[this._entitySelectorId];
		...
		onEntitySelect: function(settings)
		{
			...
			var entityInfo = BX.YNSIREntityInfo.create(item);
			if(this._entityTypeName === BX.YNSIREntityType.names.company)
			{
			...
			else if(this._entityTypeName === BX.YNSIREntityType.names.contact)
			{
				...
				else
				{
					if(this._primaryEntityInfo)
					{
						...
						if(primaryEntityTypeName === BX.YNSIREntityType.names.contact)
						{
							this.removeSecondaryEntity(this._primaryEntityInfo);
						}
						else if(primaryEntityTypeName === BX.YNSIREntityType.names.company)
						{
							this.removeAllSecondaryEntities();
						}
					}
		...
		onPrimaryViewDelete: function(sender)
		{
			...
			if(this._primaryEntityInfo.getTypeName() === BX.YNSIREntityType.names.contact)
			{
				this.removeSecondaryEntity(this._primaryEntityInfo);
			}
		...
		onExternalEvent: function(params)
		{
				...
				&& BX.type.isPlainObject(this._externalRequestData[context]))
			{
				...
				{
					var entityInfo = BX.YNSIREntityInfo.create(value["entityInfo"]);
					if(this._entityTypeName === BX.YNSIREntityType.names.company)
					{
						this.removeAllSecondaryEntities();
						this.setPrimaryEntity(entityInfo);
		...
		onLoadSecondaryEntityInfos: function(sender, result)
		{
			...
			for(var i = 0; i < l; i++)
			{
				this.addSecondaryEntity(BX.YNSIREntityInfo.create(entityData[i]));
			}
			this.prepareViews();
		}
	};
	if(typeof(BX.YNSIRCompositeClientSelector.messages) === "undefined")
	{
		BX.YNSIRCompositeClientSelector.messages = {};
	}
	BX.YNSIRCompositeClientSelector.create = function(id, settings)
	{
		var self = new BX.YNSIRCompositeClientSelector();
		self.initialize(id, settings);
		return self;
	}
}
//endregion
//region BX.YNSIRMultipleClientSelector
if(typeof(BX.YNSIRMultipleClientSelector) === "undefined")
{
	BX.YNSIRMultipleClientSelector = function()
	{
		this._id = "";
		this._settings = {};
	...
	BX.YNSIRMultipleClientSelector.prototype =
	{
		initialize: function(id, settings)
		{
			this._id = id;
			...
			if(!BX.type.isNotEmptyString(this._containerId))
			{
				throw "YNSIRMultipleClientSelector: Could not find 'containerId' parameter in settings.";
			}

			this._container = BX(this._containerId);
			if(!BX.type.isElementNode(this._container))
			{
				throw "YNSIRMultipleClientSelector: Could not find container element.";
			}

			this._entityTypeName = BX.YNSIREntityType.verifyName(this.getSetting("entityType", ""));
			if(this._entityTypeName === BX.YNSIREntityType.names.undefined)
			{
				throw "YNSIRMultipleClientSelector: Could not find 'entityTypeName' parameter in settings.";
			}
			...
			{
				var info = BX.YNSIREntityInfo.create(entityData[i]);
				if(info.getId() > 0)
				{
			...
			{
				throw "YNSIRMultipleClientSelector: Lazy load is supported in read only mode only.";
			}
		...
		getMessage: function(name)
		{
			var m = BX.YNSIRMultipleClientSelector.messages;
			return m.hasOwnProperty(name) ? m[name] : name;
		},
		getContext: function()
		{
		...
		prepareView: function(params)
		{
			...
			if(entityCount > 0)
			{
				...
				this._view = BX.YNSIRMultipleEntitySummaryView.create(this._id, settings);
				this._view.layout();
		...
		onEntitySelectClick: function(e)
		{
			...
			if (obYNSIR[this._entitySelectorId])
			{
				obYNSIR[this._entitySelectorId].Open();
			}
		},
		onEntityCreateClick: function(e)
		{
		...
		onEntitySelect: function(settings)
		{
			...
			var index = this.addEntityInfo(BX.YNSIREntityInfo.create(item));
			if(index >= 0)
			{
		...
		onExternalEvent: function(params)
		{
				...
				&& BX.type.isPlainObject(this._externalRequestData[context]))
			{
				...
				if(!isCanceled && BX.type.isPlainObject(value["entityInfo"]))
				{
					var index = this.addEntityInfo(BX.YNSIREntityInfo.create(value["entityInfo"]));
					if(index >= 0)
					{
						this.prepareView({ index: index });
					}
	...
	if(typeof(BX.YNSIRMultipleClientSelector.messages) === "undefined")
	{
		BX.YNSIRMultipleClientSelector.messages = {};
	}
	BX.YNSIRMultipleClientSelector.create = function(id, settings)
	{
		var self = new BX.YNSIRMultipleClientSelector();
		self.initialize(id, settings);
		return self;
	};
}
//endregion
//region BX.YNSIRSingleClientSelector
if(typeof(BX.YNSIRSingleClientSelector) === "undefined")
{
	BX.YNSIRSingleClientSelector = function()
	{
		this._id = "";
		this._settings = {};
	...
	BX.YNSIRSingleClientSelector.prototype =
	{
		initialize: function(id, settings)
		{
			this._id = id;
			...
			if(!BX.type.isNotEmptyString(this._containerId))
			{
				throw "YNSIRSingleClientSelector: Could not find 'containerId' parameter in settings.";
			}

			this._container = BX(this._containerId);
			if(!BX.type.isElementNode(this._container))
			{
				throw "YNSIRSingleClientSelector: Could not find container element.";
			}

			this._entityTypeName = BX.YNSIREntityType.verifyName(this.getSetting("entityType", ""));
			if(this._entityTypeName === BX.YNSIREntityType.names.undefined)
			{
				throw "YNSIRSingleClientSelector: Could not find 'entityTypeName' parameter in settings.";
			}
			...
			{
				this.setEntityInfo(BX.YNSIREntityInfo.create(entityData));
			}
		...
		getMessage: function(name)
		{
			var m = BX.YNSIRSingleClientSelector.messages;
			return m.hasOwnProperty(name) ? m[name] : name;
		},
		getContext: function()
		{
		...
		prepareView: function()
		{
			...
			if(this._entityInfo !== null)
			{
				this._view = BX.YNSIREntitySummaryView.create(this._id,
					{
						entityInfo: this._entityInfo,
						requisiteInfo: null,
						container: this._wrapper,
		...
		onEntitySelect: function(settings)
		{
			...
			if(item)
			{
				this.setEntityInfo(BX.YNSIREntityInfo.create(item));
				this.prepareView();
			}
		},
		onEntityDelete: function(sender, view)
		...
		onExternalEvent: function(params)
		{
				...
				&& BX.type.isPlainObject(this._externalRequestData[context]))
			{
				...
				if(!isCanceled && BX.type.isPlainObject(value["entityInfo"]))
				{
					this.setEntityInfo(BX.YNSIREntityInfo.create(value["entityInfo"]));
					this.prepareView();
				}
	...
	if(typeof(BX.YNSIRSingleClientSelector.messages) === "undefined")
	{
		BX.YNSIRSingleClientSelector.messages = {};
	}
	BX.YNSIRSingleClientSelector.create = function(id, settings)
	{
		var self = new BX.YNSIRSingleClientSelector();
		self.initialize(id, settings);
		return self;
	};
}
//endregion
//region BX.YNSIRClientPanelGroup
if(typeof(BX.YNSIRClientPanelGroup) === "undefined")
{
	BX.YNSIRClientPanelGroup = function()
	{
		this._id = "";
		this._settings = {};
	...
	BX.YNSIRClientPanelGroup.prototype =
	{
		initialize: function(id, settings)
		{
			this._id = id;
			...
			this._id = BX.type.isNotEmptyString(id) ? id : "YNSIR_CLIENT_PANEL_GROUP" + Math.random();
			this._settings = BX.type.isPlainObject(settings) ? settings : {};

			this._container = this.getSetting("container");
			if(!BX.type.isElementNode(this._container))
			{
				throw "YNSIRClientPanelGroup: Could not find 'container' parameter in settings.";
			}

			this._entityTypeName = this.getSetting("entityType", "");
			if(!BX.type.isNotEmptyString(this._entityTypeName))
			{
				throw "YNSIRClientPanelGroup: Could not find 'entityType' parameter in settings.";
			}
		...
		getMessage: function(name)
		{
			var m = BX.YNSIRClientPanelGroup.messages;
			return m.hasOwnProperty(name) ? m[name] : name;
		},
		...
		layout: function()
		{
			...
			if(!this._readOnly)
			{
				...
				obYNSIR[entitySelectorId].AddOnSaveListener(this._entitySelectHandler);
		...
		{
			return (
				BX.YNSIRClientPanelGroupItem.create("",
					{
						parent: this,
		...
		{
			if(!this._readonly && obYNSIR[this._entitySelectorId])
			{
				var selector = obYNSIR[this._entitySelectorId];
				//HACK: To prevent display of 'minus' icon.
				selector.ClearSelectItems();
		...
		onEntitySelect: function(settings)
		{
			...
			var item = this.addItem(BX.YNSIREntityInfo.create(data))
			if(item)
			{
				item.layout();
			}
		...
		onExternalEvent: function(params)
		{
				...
				&& BX.type.isPlainObject(this._externalRequestData[context]))
			{
				...
				if(!isCanceled && BX.type.isPlainObject(value["entityInfo"]))
				{
					...
					var item = this.addItem(BX.YNSIREntityInfo.create(value["entityInfo"]))
					if(item)
					{
						item.layout();
					}
	...
	if(typeof(BX.YNSIRClientPanelGroup.messages) === "undefined")
	{
		BX.YNSIRClientPanelGroup.messages = {};
	}
	BX.YNSIRClientPanelGroup.create = function(id, settings)
	{
		var self = new BX.YNSIRClientPanelGroup();
		self.initialize(id, settings);
		return self;
	};
}
//endregion
//region BX.YNSIRClientPanelGroupItem
if(typeof(BX.YNSIRClientPanelGroupItem) === "undefined")
{
	BX.YNSIRClientPanelGroupItem = function()
	{
		this._id = "";
		this._settings = {};
		this._parent = null;
	...
	BX.YNSIRClientPanelGroupItem.prototype =
	{
		initialize: function(id, settings)
		{
			this._id = id;
			...
			this._id = BX.type.isNotEmptyString(id) ? id : "YNSIR_CLIENT_PANEL_GROUP_ITEM" + Math.random();
			this._settings = BX.type.isPlainObject(settings) ? settings : {};

			this._parent = this.getSetting("parent");
			if(!(this._parent instanceof BX.YNSIRClientPanelGroup))
			{
				throw "YNSIRClientPanelGroupItem: Could not find 'parent' parameter in settings.";
			}
			this._container = this.getSetting("container");
			if(!BX.type.isElementNode(this._container))
			{
				throw "YNSIRClientPanelGroupItem: Could not find 'container' parameter in settings.";
			}

			this._entityTypeName = this.getSetting("entityType", "");
			if(!BX.type.isNotEmptyString(this._entityTypeName))
			{
				throw "YNSIRClientPanelGroupItem: Could not find 'entityType' parameter in settings.";
			}

			this._entityInfo = this.getSetting("entityInfo");
			if(!(this._entityInfo instanceof BX.YNSIREntityInfo))
			{
				throw "YNSIRClientPanelGroupItem: Could not find 'entityInfo' parameter in settings.";
			}

			this._readOnly = !!this.getSetting("readOnly");
			this._enableMarking = !!this.getSetting("enableMarking");

			this._deleteNotifier = BX.YNSIRNotifier.create(this);
			this._markNotifier = BX.YNSIRNotifier.create(this);
		},
		getSetting: function(name, defaultval)
		{
			return this._settings.hasOwnProperty(name) ? this._settings[name] : defaultval;
		...
		{
			var comm = BX.YNSIRClientPanelCommunication.create(
				"",
				{
					container: container,
					entityId: this.getEntityId(),
	...
	BX.YNSIRClientPanelGroupItem.create = function(id, settings)
	{
		var self = new BX.YNSIRClientPanelGroupItem();
		self.initialize(id, settings);
		return self;
	};
}
...
BX.YNSIRPopupWindowHelper = {};
BX.YNSIRPopupWindowHelper.prepareButtons = function(data)
{
	var result = [];
...
BX.YNSIRPopupWindowHelper.prepareTextField = function(settings)
{
	return BX.create(
		'DIV',
		{
...
BX.YNSIRPopupWindowHelper.prepareSelectField = function(settings)
{
...
BX.YNSIRPopupWindowHelper.prepareTextAreaField = function(settings)
{
	return BX.create(
		'DIV',
		{
...
BX.YNSIRPopupWindowHelper.prepareCheckBoxField = function(settings)
{
...
BX.YNSIRPopupWindowHelper.prepareTitle = function(text)
{
	return BX.create(
		'DIV',
		{
...
BX.YNSIREntityDetailViewDialog = function()
{
	this._id = '';
	this._dlg = null;
	this._settings = {};
...
BX.YNSIREntityDetailViewDialog.prototype =
{
	initialize: function(id, settings)
	{
		this._id = BX.type.isNotEmptyString(id) ? id : 'YNSIR_ENTITY_DETAIL_VIEW_DIALOG_' + Math.random();
		this._settings = settings ? settings : {};
	},
	getId: function()
	{
...
BX.YNSIREntityDetailViewDialog.items = {};
BX.YNSIREntityDetailViewDialog.create = function(id, settings)
{
	var self = new BX.YNSIREntityDetailViewDialog();
	self.initialize(id, settings);
	this.items[self.getId()] = self;
	return self;
};

BX.YNSIREntityDetailViewDialog.ensureCreated = function(id, settings)
{
	return typeof(this.items[id]) !== 'undefined' ? this.items[id] : this.create(id, settings);
};

BX.YNSIRContactEditor = function()
{
	this._id = '';
	this._settings = {};
	this._dlg = null;
...
BX.YNSIRContactEditor.prototype =
{
	initialize: function(id, settings)
	{
		this._id = id;
		...
		this._data = BX.YNSIRContactData.create(initData);
	},
	getSetting: function (name, defaultval)
	{
		return typeof(this._settings[name]) != 'undefined' ? this._settings[name] : defaultval;
		...
		BX.ajax(
			{
				...
				onsuccess: function(data)
				{
					...
					{
						self._showDialogError('BX.YNSIRContactEditor: Could not find contact data!');
					}
					else
					{
						self._data = BX.YNSIRContactData.create(data['DATA']);
...
BX.YNSIRContactEditor.create = function(id, settings)
{
	var self = new BX.YNSIRContactEditor();
	self.initialize(id, settings);
	return self;
};

BX.YNSIRSonetSubscription = function()
{
	this._id = '';
	this._settings = {};
};

BX.YNSIRSonetSubscription.prototype =
{
	initialize: function(id, settings)
	{
		this._id = id;
...
BX.YNSIRSonetSubscription.items = {};
BX.YNSIRSonetSubscription.create = function(id, settings)
{
	var self = new BX.YNSIRSonetSubscription();
	self.initialize(id, settings);
	this.items[id] = self;
	return self;
};

if(typeof(BX.YNSIRFormTabLazyLoader) == "undefined")
{
	BX.YNSIRFormTabLazyLoader = function()
	{
		this._id = "";
		this._settings = {};
		this._container = null;
	...
	BX.YNSIRFormTabLazyLoader.prototype =
	{
		initialize: function(id, settings)
		{
	...
	BX.YNSIRFormTabLazyLoader.items = {};
	BX.YNSIRFormTabLazyLoader.create = function(id, settings)
	{
		var self = new BX.YNSIRFormTabLazyLoader();
		self.initialize(id, settings);
		this.items[self.getId()] = self;
		return self;
	};
...
if(typeof(BX.YNSIRCustomDragItem) === "undefined")
{
	BX.YNSIRCustomDragItem = function()
	{
		this._id = "";
		this._settings = {};
		this._node = null;
	...
	BX.YNSIRCustomDragItem.prototype =
	{
		initialize: function(id, settings)
		{
			if(typeof(jsDD) === "undefined")
			{
				throw "YNSIRCustomDragItem: Could not find jsDD API.";
			}
			...
			if(!this._node)
			{
				throw "YNSIRCustomDragItem: The 'node' parameter is not defined in settings or empty.";
			}

			this._enableDrag = this.getSetting("enableDrag", true);
			this._ghostOffset = this.getSetting("ghostOffset", { x: 0, y: 0 });

			this._dragNotifier = BX.YNSIRNotifier.create(this);
		...
		{
			throw "YNSIRCustomDragItem: The 'createGhostNode' function is not implemented.";
		},
		...
		{
			throw "YNSIRCustomDragItem: The 'removeGhostNode' function is not implemented.";
		},
		...
		_onDragStart: function()
		{
			...
			BX.YNSIRCustomDragItem.currentDragged = this;

			BX.onCustomEvent('YNSIRDragItemDragStart', [this]);
		...
		_onDragStop: function(x, y)
		{
			...
			this._isInDragMode = false;
			if(BX.YNSIRCustomDragItem.currentDragged === this)
			{
				BX.YNSIRCustomDragItem.currentDragged = null;
			}

			BX.onCustomEvent('YNSIRDragItemDragStop', [this]);
		...
		{
			BX.onCustomEvent('YNSIRDragItemDragRelease', [this]);
		},
	...
	BX.YNSIRCustomDragItem.currentDragged = null;
	BX.YNSIRCustomDragItem.emulateDrag = function()
	{
...
if(typeof(BX.YNSIRCustomDragContainer) === "undefined")
{
	BX.YNSIRCustomDragContainer = function()
	{
		this._id = "";
		this._settings = {};
		this._node = null;
	...
	BX.YNSIRCustomDragContainer.prototype =
	{
		initialize: function(id, settings)
		{
			if(typeof(jsDD) === "undefined")
			{
				throw "YNSIRCustomDragContainer: Could not find jsDD API.";
			}
			...
			if(!this._node)
			{
				throw "YNSIRCustomDragContainer: The 'node' parameter is not defined in settings or empty.";
			}

			this._dragFinishNotifier = BX.YNSIRNotifier.create(this);
			this.doInitialize();
			this.bindEvents();
		},
		doInitialize: function()
		...
		{
			throw "YNSIRCustomDragContainer: The 'createPlaceHolder' function is not implemented.";
		},
		removePlaceHolder: function()
		{
			throw "YNSIRCustomDragContainer: The 'removePlaceHolder' function is not implemented.";
		},
		...
		{
			return BX.YNSIRCustomDragContainer.defaultPriority;
		},
		...
		{
			var draggedItem = BX.YNSIRCustomDragItem.currentDragged;
			if(!draggedItem)
			{
				return;
			}
		...
		_onDragFinish: function(node, x, y)
		{
			...
			BX.YNSIRCustomDragContainer.refresh();
		},
	...
	BX.YNSIRCustomDragContainer.defaultPriority = 100;
	BX.YNSIRCustomDragContainer.refresh = function()
	{
		jsDD.refreshDestArea();
	};
}

BX.YNSIRDragDropBinState = { suspend: 0, wait: 1, ready: 2, open: 3, close: 4 };

if(typeof(BX.YNSIRDragDropBin) === "undefined")
{
	BX.YNSIRDragDropBin = function()
	{
		this._state = BX.YNSIRDragDropBinState.suspend;
	...
	BX.extend(BX.YNSIRDragDropBin, BX.YNSIRCustomDragContainer);
	BX.YNSIRDragDropBin.prototype.doInitialize = function()
	{
		BX.addCustomEvent(window, "YNSIRDragItemDragStart", this._chargeDragStartHandler);
		BX.addCustomEvent(window, "YNSIRDragItemDragStop", this._chargeDragStopHandler);
		BX.addCustomEvent(window, "YNSIRDragItemDragRelease", this._chargeDragReleaseHandler);

		this.cacheWorkareaRect();
		BX.bind(window, "resize", BX.delegate(this._onWindowResize, this));
	};
	BX.YNSIRDragDropBin.prototype.getPriority = function()
	{
		return 10;
	};
	BX.YNSIRDragDropBin.prototype.createPlaceHolder = function(pos)
	{
	};
	BX.YNSIRDragDropBin.prototype.removePlaceHolder = function()
	{
	};
	BX.YNSIRDragDropBin.prototype.processDragOver = function(pos)
	{
		if(this._chargeItem)
		{
			this._enableChargeItem = false;
		}
		this.setState(BX.YNSIRDragDropBinState.open);
	};
	BX.YNSIRDragDropBin.prototype.processDragOut = function()
	{
		if(this._chargeItem)
		{
			this._enableChargeItem = true;
		}
		this.setState(BX.YNSIRDragDropBinState.ready);
	};
	BX.YNSIRDragDropBin.prototype.processDragStop = function()
	{
		this.setState(BX.YNSIRDragDropBinState.suspend);
	};
	BX.YNSIRDragDropBin.prototype.processDragRelease = function()
	{
		this.setState(BX.YNSIRDragDropBinState.suspend);
	};
	BX.YNSIRDragDropBin.prototype.processItemDrop = function()
	{
		if(this._chargeItem)
		{
		...
		this.setState(BX.YNSIRDragDropBinState.close);
		window.setTimeout(BX.delegate(this.reset, this), 1000);
		BX.onCustomEvent(this, "YNSIRDragDropBinItemDrop", [ this, this.getDraggedItem() ]);
	};
	BX.YNSIRDragDropBin.prototype.getState = function()
	{
		return this._state;
	};
	BX.YNSIRDragDropBin.prototype.reset = function()
	{
		this.setState(BX.YNSIRDragDropBinState.suspend);
	};
	BX.YNSIRDragDropBin.prototype.setState = function(state)
	{
		state = parseInt(state);
		if(state < BX.YNSIRDragDropBinState.suspend || state > BX.YNSIRDragDropBinState.close)
		{
			state = BX.YNSIRDragDropBinState.suspend;
		}
		...
		if(this._state >= BX.YNSIRDragDropBinState.wait)
		{
			classNames.push("crm-cart-start");
		}
		if(this._state >= BX.YNSIRDragDropBinState.ready)
		{
			classNames.push("crm-cart-active");
		}
		if(this._state >= BX.YNSIRDragDropBinState.open)
		{
			classNames.push("crm-cart-hover");
		}
		if(this._state === BX.YNSIRDragDropBinState.close)
		{
		...
		window.setTimeout(BX.delegate(BX.YNSIRCustomDragItem.emulateDrag, this), 400);
		window.setTimeout(BX.delegate(BX.YNSIRCustomDragItem.emulateDrag, this), 800);
	};
	BX.YNSIRDragDropBin.prototype._onChargeDragStart = function(item)
	{
		...
		this.setState(BX.YNSIRDragDropBinState.wait);
	};
	BX.YNSIRDragDropBin.prototype._onChargeDragStop = function(item)
	{
	...
	BX.YNSIRDragDropBin.prototype._onChargeDragRelease = function(item)
	{
		if(!this._enableChargeItem || this._chargeItem !== item)
		{
			return;
	...
	BX.YNSIRDragDropBin.prototype._onChargeDrag = function(item, x, y)
	{
		if(this._enableChargeItem && this._chargeItem === item)
		{
			this.adjust();
	...
	BX.YNSIRDragDropBin.prototype._onWindowResize = function(e)
	{
		this.cacheWorkareaRect();
	};
	BX.YNSIRDragDropBin.prototype.cacheWorkareaRect = function()
	{
	...
	BX.YNSIRDragDropBin.prototype.adjust = function()
	{
		if(!this._chargeItem)
		{
			return;
		...
		var isReady = this._state >= BX.YNSIRDragDropBinState.ready;
		if(isReady !== ((this._workareaRect.right - ghostRect.left) <= this._readyThreshold))
		{
			isReady = !isReady;
			this.setState(isReady ? BX.YNSIRDragDropBinState.ready : BX.YNSIRDragDropBinState.wait);
		}
	};
	BX.YNSIRDragDropBin.prototype.getMessage = function(name, defaultval)
	{
		var m = BX.YNSIRDragDropBin.messages;
		return m.hasOwnProperty(name) ? m[name] : defaultval;
	};
	BX.YNSIRDragDropBin.prototype.showPromptingIfRequired = function(container)
	{
	...
	BX.YNSIRDragDropBin.prototype.showPrompting = function(container)
	{
	...
	BX.YNSIRDragDropBin.prototype.hidePrompting = function()
	{
		if(!this._promptingWrapper)
		{
			return;
	...
	BX.YNSIRDragDropBin.prototype.demo = function()
	{
		this.setState(BX.YNSIRDragDropBinState.wait);

		var self = this;
		window.setTimeout(function(){ self.setState(BX.YNSIRDragDropBinState.ready); }, 1000);
		window.setTimeout(function(){ self.setState(BX.YNSIRDragDropBinState.open); }, 1500);
		window.setTimeout(function(){ self.setState(BX.YNSIRDragDropBinState.close); }, 2000);
	};
	BX.YNSIRDragDropBin.prototype._onDemoButtonClick = function(e)
	{
		this.demo();
		return BX.PreventDefault(e);
	};
	BX.YNSIRDragDropBin.prototype._onClosePromptingButtonClick = function(e)
	{
		this.hidePrompting();
		return BX.PreventDefault(e);
	};
	BX.YNSIRDragDropBin.instance = null;
	BX.YNSIRDragDropBin.getInstance = function()
	{
		if(this.instance)
		{
		...
		var self = new BX.YNSIRDragDropBin();
		self.initialize("default", { node: node });
		return (this.instance = self);
	};

	if(typeof(BX.YNSIRDragDropBin.messages) === "undefined")
	{
		BX.YNSIRDragDropBin.messages = {};
	}
}

if(typeof(BX.YNSIRLocalitySearchField) === "undefined")
{
	BX.YNSIRLocalitySearchField = function()
	{
		this._id = "";
		this._settings = {};
		this._localityType = "";
	...
	BX.YNSIRLocalitySearchField.prototype =
	{
		initialize: function(id, settings)
		{
			...
			{
				throw  "BX.YNSIRLocalitySearchField: localityType is not found!";
			}

			this._serviceUrl = this.getSetting("serviceUrl");
			if(!BX.type.isNotEmptyString(this._serviceUrl))
			{
				throw  "BX.YNSIRLocalitySearchField: serviceUrl is not found!";
			}

			this._searchInput = this.findElement("searchInput");
			if(!BX.type.isElementNode(this._searchInput))
			{
				throw  "BX.YNSIRLocalitySearchField: searchInput is not found!";
			}

			this._dataInput = this.findElement("dataInput");
			if(!BX.type.isElementNode(this._dataInput))
			{
				throw  "BX.YNSIRLocalitySearchField: dataInputId is not found!";
			}
		...
		prepareMenuItem: function(data)
		{
			...
			if(code === "")
			{
				throw  "BX.YNSIRLocalitySearchField: could not find item code!";
			}
	...
	BX.YNSIRLocalitySearchField.create = function(id, settings)
	{
		var self = new BX.YNSIRLocalitySearchField();
		self.initialize(id, settings);
		return self;
	};
}

if(typeof(BX.YNSIRAddressType) === "undefined")
{
	BX.YNSIRAddressType =
	{
		undefined: 0,
...
if(typeof(BX.YNSIRMultipleAddressEditor) === "undefined")
{
	BX.YNSIRMultipleAddressEditor = function()
	{
		this._id = "";
		this._settings = {};
	...
	BX.YNSIRMultipleAddressEditor.prototype =
	{
		initialize: function(id, settings)
		{
			...
			{
				throw "BX.YNSIRMultipleAddressEditor: Could not find a parameter named 'scheme'.";
			}

			this._data = this.getSetting("data");
			if(!BX.type.isPlainObject(this._data))
			{
				throw "BX.YNSIRMultipleAddressEditor: Could not find a parameter named 'data'.";
			}

			this._typeInfos = this.getSetting("typeInfos");
			if(!BX.type.isArray(this._typeInfos))
			{
				throw "BX.YNSIRMultipleAddressEditor: Could not find a parameter named 'typeInfos'.";
			}

			this._fieldLabels = this.getSetting("fieldLabels");
			if(!BX.type.isPlainObject(this._fieldLabels))
			{
				throw "BX.YNSIRMultipleAddressEditor: Could not find a parameter named 'fieldLabels'.";
			}

			this._container = this.getSetting("container");
			if(!BX.type.isElementNode(this._container))
			{
				throw "BX.YNSIRMultipleAddressEditor: Could not find a parameter named 'container'.";
			}
			...
			for(var key in this._data)
			{
				...
				this._items[itemId] = BX.YNSIRMultipleAddressItemEditor.create(
					itemId,
					{
						typeId: typeId,
		...
		getMessage: function(name)
		{
			var msg = BX.YNSIRMultipleAddressEditor.messages;
		...
		createItem: function(typeId, originatorId)
		{
			...
			var item = BX.YNSIRMultipleAddressItemEditor.create(
				itemId,
				{
					typeId: typeId,
					fields: {},
			...
			BX.onCustomEvent(this, "YNSIRMultipleAddressItemCreated", [this, item]);
	...
	if(typeof(BX.YNSIRMultipleAddressEditor.messages) === "undefined")
	{
		BX.YNSIRMultipleAddressEditor.messages =
		{
		};
	}
	BX.YNSIRMultipleAddressEditor.items = {};
	BX.YNSIRMultipleAddressEditor.getItemsByFormId = function(formId)
	{
		var item;
	...
	BX.YNSIRMultipleAddressEditor.create = function(id, settings)
	{
		var self = new BX.YNSIRMultipleAddressEditor();
		self.initialize(id, settings);
		BX.YNSIRMultipleAddressEditor.items[self.getId()] = self;
		return self;
	};
}

if(typeof(BX.YNSIRMultipleAddressItemEditor) === "undefined")
{
	BX.YNSIRMultipleAddressItemEditor = function()
	{
		this._id = "";
		this._settings = {};
		this._typeId = 0;
	...
	BX.YNSIRMultipleAddressItemEditor.prototype =
	{
		initialize: function(id, settings)
		{
			...
			if(!(this._editor instanceof BX.YNSIRMultipleAddressEditor))
			{
				throw "BX.YNSIRMultipleAddressItemEditor: Could not find a parameter named 'editor'.";
			}

			this._container = this.getSetting("container");
			if(!BX.type.isElementNode(this._container))
			{
				throw "BX.YNSIRMultipleAddressItemEditor: Could not find a parameter named 'container'.";
			}
			...
			if(this._hasLayout)
			{
				...
				{
					throw "BX.YNSIRMultipleAddressItemEditor: Could not find the 'Delete' button.";
				}
		...
		getMessage: function(name)
		{
			var msg = BX.YNSIRMultipleAddressItemEditor.messages;
			return msg.hasOwnProperty(name) ? msg[name] : name;
		},
		getTypeId: function()
		{
		...
		layout: function()
		{
			...
			for(var i = 0; i < scheme.length; i++)
			{
				...
				if(type === "locality")
				{
					...
					BX.YNSIRLocalitySearchField.create(
						qualifiedName,
						{
	...
	if(typeof(BX.YNSIRMultipleAddressItemEditor.messages) === "undefined")
	{
		BX.YNSIRMultipleAddressItemEditor.messages =
		{
		};
	}

	BX.YNSIRMultipleAddressItemEditor.create = function(id, settings)
	{
		var self = new BX.YNSIRMultipleAddressItemEditor();
		self.initialize(id, settings);
		return self;
	};
}

BX.namespace("BX.YNSIR");

BX.YNSIR.EntityEditorBlockAreaClass = (function ()
{
	...
	EntityEditorBlockAreaClass.prototype = {
		...
		continueInitialize: function()
		{
			if (this.container)
			{
				...
				this.visualization = new BX.YNSIR.EntityEditorBlockAreaVisualizationClass(
					{
						blockArea: this,
			...
			if (this.entityInfoList instanceof Array && n > 0)
			{
				for (var i = 0; i < n; i++)
				{
					if (this.entityInfoList[i] instanceof BX.YNSIREntityInfo)
		...
		addBlock: function(entityInfo)
		{
			...
			var block = new BX.YNSIR.EntityEditorBlockClass({
				editor: this.editor,
				blockArea: this,
...
BX.YNSIR.EntityEditorBlockAreaVisualizationClass = (function()
{
...
BX.YNSIR.EntityEditorBlockClass = (function ()
{
	...
	EntityEditorBlockClass.prototype = {
		initialize: function()
		{
			if (this.container)
			{
				...
				{
					var typeClass = "";
					if (this.entityInfo instanceof BX.YNSIREntityInfo)
					{
...
if(typeof(BX.YNSIR.RequisiteBankDetailsArea) === "undefined")
{
	BX.YNSIR.RequisiteBankDetailsArea = function()
	{
		this._id = null;
		this.container = null;
		this.nextNode = null;
	...
	BX.YNSIR.RequisiteBankDetailsArea.prototype =
	{
		...
		addBlock: function(bankDetailId, bankDetailData)
		{
			...
			var block = new BX.YNSIR.RequisiteBankDetailsBlock({
				mode: this.mode,
				bankDetailId: bankDetailId,
	...
	BX.YNSIR.RequisiteBankDetailsArea.items = {};
	BX.YNSIR.RequisiteBankDetailsArea.create = function (id, parameters)
	{
		var self = new BX.YNSIR.RequisiteBankDetailsArea();
...
if(typeof(BX.YNSIR.RequisiteBankDetailsBlock) === "undefined")
{
	BX.YNSIR.RequisiteBankDetailsBlock = function (parameters)
	{
	...
	BX.YNSIR.RequisiteBankDetailsBlock.prototype =
	{
		initialize: function()
		{
