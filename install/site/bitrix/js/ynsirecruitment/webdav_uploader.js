BX.YNSIRWebDavUploader = function()
{
	this._id = "";
	this._settings = {};
	this._hasLayout = false;
	this._mode = BX.YNSIRInterfaceMode.edit;
	this._values = [];
	this._canceled = [];
...
BX.YNSIRWebDavUploader.prototype =
{
	initialize: function(id, settings)
	{
		...
		this._mode = parseInt(this.getSetting("mode", BX.YNSIRInterfaceMode.edit));
	...
	_openSection: function(link, dialogName)
	{
		if (dialogName == this._dialogName)
		{
			BX.WebDavFileDialog.target[dialogName] =
				BX.YNSIRWebDavUploader.addSessionParam(
					BX.YNSIRWebDavUploader.addUrlParam(link, "dialog2=Y")
				);
		}
	},
	...
	{
		var selectUrl = BX.YNSIRWebDavUploader.addSessionParam(
			BX.YNSIRWebDavUploader.addUrlParam(
	...
	_showElement: function(data, mode)
	{
		...
		if(mode === BX.YNSIRInterfaceMode.edit)
		{
	...
	_prepareElementShowUrl: function(elementId, url)
	{
		...
		{
			return BX.YNSIRWebDavUploader.addSessionParam(BX.YNSIRWebDavUploader.addUrlParam(url, "inline=Y&IFRAME=Y"));
		}

		return BX.YNSIRWebDavUploader.addSessionParam(
			BX.YNSIRWebDavUploader.addUrlParam(
	...
	{
		var urlMove = BX.YNSIRWebDavUploader.addSessionParam(
			BX.YNSIRWebDavUploader.addUrlParam(
	...
	layout: function(parent)
	{
		...
		else if(mode === BX.YNSIRInterfaceMode.view)
		{
	...
	GetUploadDialog: function(agent)
	{
		...
		var uploadUrl = BX.YNSIRWebDavUploader.addSessionParam(
			BX.YNSIRWebDavUploader.addUrlParam(
...
BX.YNSIRWebDavUploader.items = {};
BX.YNSIRWebDavUploader.create = function(id, settings)
{
	if(!BX.type.isNotEmptyString(id))
	{
		id = 'BX_YNSIR_FILEUPLOADER_' + Math.random();
	}

	var self = new BX.YNSIRWebDavUploader();
	self.initialize(id, settings);
	this.items[id] = self;
	return self;
};

BX.YNSIRWebDavUploader.addUrlParam = function(url, params)
{
	if (!BX.type.isNotEmptyString(url))
	{
		return "";
...
BX.YNSIRWebDavUploader.addSessionParam = function(url)
{
