...
$options = array(
	...
	'important' => $activity['PRIORITY'] == YNSIRActivityPriority::High
);
	...
	<div class="crm-task-list-container">
		<div class="crm-task-list-header crm-task-list-header-image <?=$arResult['TYPE_ICON']?>">
			...
			<div class="crm-task-list-header-description">
				<span class="crm-task-list-header-description-item"><?=GetMessage('YNSIR_ACTIVITY_PLANNER_VIEW_DATE_AND_TIME')?>:</span>
		...
		<?if ($arResult['DOC_BINDINGS'] && 0):?>
		<div class="crm-task-list-docs">
			<div class="crm-task-list-docs-item"><?=GetMessage('YNSIR_ACTIVITY_PLANNER_VIEW_DOCUMENTS')?>:</div>
		...
		<div class="crm-task-list-person">
			<div class="crm-task-list-person-item"><?=GetMessage('YNSIR_ACTIVITY_PLANNER_RECEIVER')?>:</div>
						...
						<span class="crm-task-list-person-user-info">
							...
							<div class="crm-task-list-person-info-contacts">
								<?if (!empty($communication['FM']['PHONE'])):
									...
									$entityType = 'YNSIR_'.strtoupper(YNSIROwnerType::ResolveName($communication['ENTITY_TYPE_ID']));
									$entityID = $communication['ENTITY_ID'];
								?>
								<div class="crm-task-list-person-info-phone-block">
									<span class="crm-task-list-person-info-phone"><?=GetMessage('YNSIR_ACTIVITY_PLANNER_TEL')?>:</span>
									...
									<span class="crm-task-list-person-info-phone-icon">
										...
										<span class="crm-task-list-person-info-phone-icon-element"
											onclick="if(typeof(window['BXIM']) === 'undefined') { window.alert('<?=GetMessageJS('YNSIR_SIP_NO_SUPPORTED')?>'); return; } BX.YNSIRSipManager.startCall({ number:'<?=CUtil::JSEscape($fm['VALUE'])?>', enableInfoLoading: true }, { ENTITY_TYPE: '<?=CUtil::JSEscape($entityType)?>', ENTITY_ID: '<?=CUtil::JSEscape($entityID)?>', SRC_ACTIVITY_ID: '<?=CUtil::JSEscape($activity['ID'])?>'}, true, this);"></span>
									</span>
									<?endif?>
								</div>
								<?endif?>
								...
								<div class="crm-task-list-person-info-mail-block">
									<span class="crm-task-list-person-info-mail"><?=GetMessage('YNSIR_ACTIVITY_PLANNER_EMAIL')?>:</span>
									<span class="crm-task-list-person-info-phone-item">
		...
		<div class="crm-task-list-extra">
				<span class="crm-task-list-extra-button">
					<span class="crm-task-list-extra-button-item" data-role="additional-switcher"><?=GetMessage('YNSIR_ACTIVITY_PLANNER_ADDITIONAL')?></span>
				...
				<span class="crm-task-list-extra-item">
					<label class="crm-task-list-extra-item-element">
						...
						<span class="crm-task-list-extra-text"><?=GetMessage('YNSIR_ACTIVITY_PLANNER_CHECK_COMPLETED')?></span>
					</label>
				</span>
		...
		<div class="crm-task-list-extra-inner" data-role="additional-fields">
			<div class="crm-task-list-extra-inner-container">
				...
				<div class="crm-task-list-receiver">
					<div class="crm-task-list-receiver-item"><?=GetMessage('YNSIR_ACTIVITY_PLANNER_FILES')?>:</div><!--crm-task-list-receiver-name-->
				...
				<div class="crm-task-list-receiver">
					<div class="crm-task-list-receiver-item"><?=GetMessage('YNSIR_ACTIVITY_PLANNER_RESPONSIBLE_USER')?>:</div><!--crm-task-list-receiver-name-->
					<div class="crm-task-list-options-item-open-inner">
				...
				<?endif?>
                <?if (!empty($arResult['ATTENDEES'])):?>
                    <div class="crm-task-list-receiver">
                        <div class="crm-task-list-receiver-item"><?=GetMessage('YNSIR_ACTIVITY_PLANNER_GUEST_USER')?>:</div><!--crm-task-list-receiver-name-->
                        <div class="crm-task-list-options-item-open-inner">
						<span class="crm-task-list-options-destination-wrap">
                            <?foreach ($arResult['ATTENDEES'] as $ATTENDEE):?>
                            <span class="crm-task-list-inline-selector-item">
                                <span class="crm-task-list-options-destination">
                                    <a href="<?=htmlspecialcharsbx($ATTENDEE['URL'])?>" target="_blank"
                                       class="crm-task-list-options-destination-text"><?=htmlspecialcharsbx($ATTENDEE['NAME'])?></a>
                                </span>
                            </span>
                            <?endforeach;?>
						</span>
                ...
                <?endif?>
                <?if ($arResult['ACTIVITY']['REFERENCE_TYPE_ID'] == YNSIROwnerType::Order && !empty($arResult['ACTIVITY']['REFERENCE_ID'])):?>
                    <?
                    $arFilter['ID'] = $arResult['ACTIVITY']['REFERENCE_ID'];
                    ...
                    ?>
                    <div class="crm-task-list-receiver">
                        <div class="crm-task-list-receiver-item"><?=GetMessage('YNSIR_ACTIVITY_PLANNER_JOB_ORDER')?>:</div><!--crm-task-list-receiver-name-->
                        <div class="crm-task-list-options-item-open-inner">
						<span class="crm-task-list-options-destination-wrap">
                                <span class="crm-task-list-inline-selector-item">
                                <span class="crm-task-list-options-destination">
                                    <a href="/recruitment/job-order/detail/<?=$arResult['ACTIVITY']['REFERENCE_ID']?>/" target="_blank"
                                       class="crm-task-list-options-destination-text"><?=$title?></a>
                                </span>
                            </span>
						</span>
                        </div>
