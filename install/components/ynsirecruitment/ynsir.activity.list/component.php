...
// PARSE PARAMS
$arParams['PATH_TO_ACTIVITY_LIST'] = CrmCheckPath('PATH_TO_ACTIVITY_LIST', $arParams['PATH_TO_ACTIVITY_LIST'], COption::GetOptionString('ynsirecruitment', 'path_to_activity_list'));
...
$currentUserPermissions = YNSIRPerms_::GetCurrentUserPermissions();
$currentUserID = $arResult['CURRENT_USER_ID'] = $USER::GetId();
$currentUserName = $arResult['CURRENT_USER_NAME'] = CYNSIRViewHelper::GetFormattedUserName($currentUserID, $arParams['NAME_TEMPLATE']);
...
for($i = count($bindings); $i >= 0; $i--)
{
	...
	$arResult['OWNER_UID'] .=  strtolower(YNSIROwnerType::ResolveName($ownerTypeID)).($ownerID > 0 ? '_'.$ownerID : '');
}
...
$arResult['UID'] = $arResult['GRID_ID'] = 'YNSIR_ACTIVITY_LIST_'.($arResult['PREFIX'] !== '' ? $arResult['PREFIX'] : strtoupper($arResult['OWNER_UID']));
$arResult['IS_INTERNAL'] = $arResult['OWNER_UID'] !== '';

if(count($arBindingFilter) === 1)
{
	$arBinding = $arBindingFilter[0];
	$arResult['OWNER_TYPE'] = YNSIROwnerType::ResolveName($arBinding['OWNER_TYPE_ID']);
...
$arResult['OWNER_TYPE_ID'] = YNSIROwnerType::ResolveID($arResult['OWNER_TYPE']);
...
elseif(!empty($arParams['~ORDER_ID']) && intval($arParams['~ORDER_ID']) > 0)
{
	$arFilter['__INNER_FILTER_JOB_ORDER'] = array(
		'LOGIC' => 'AND',
		'REFERENCE_TYPE_ID' => YNSIROwnerType::Order,
		'REFERENCE_ID' => $arParams['~ORDER_ID']
	);
}
...
$arResult['HEADERS'][] = array('id' => 'SUBJECT', 'type'=> 'text', 'name' => GetMessage('YNSIR_ACTIVITY_COLUMN_SUBJECT'), 'width' => 200, 'default' => true, 'editable' => true);
$arResult['HEADERS'][] = array('id' => 'START_TIME', 'type'=> 'date', 'name' => GetMessage('YNSIR_ACTIVITY_COLUMN_START'), 'default' => false, 'editable' => true, 'class' => 'datetime');
$arResult['HEADERS'][] = array('id' => 'END_TIME', 'type'=> 'date', 'name' => GetMessage('YNSIR_ACTIVITY_COLUMN_END_2'), 'default' => false, 'editable' => true, 'class' => 'datetime');
$arResult['HEADERS'][] = array('id' => 'DEADLINE', 'type'=> 'date', 'name' => GetMessage('YNSIR_ACTIVITY_COLUMN_DEADLINE'), 'sort' => 'DEADLINE', 'default' => true, 'editable' => false, 'class' => 'datetime');

$arResult['HEADERS'][] = array('id' => 'DESCRIPTION', 'type'=> 'text', 'name' => GetMessage('YNSIR_ACTIVITY_COLUMN_DESCRIPTION'), 'default' => false, 'editable' => true);
$arResult['HEADERS'][] = array('id' => 'RESPONSIBLE_FULL_NAME', 'type'=> 'text', 'name' => GetMessage('YNSIR_ACTIVITY_COLUMN_RESPONSIBLE'), 'sort' => 'RESPONSIBLE_FULL_NAME', 'default' => true, 'editable' => false, 'class' => 'username');
$arResult['HEADERS'][] = array('id' => 'COMPLETED', 'type'=> 'list', 'name' => GetMessage('YNSIR_ACTIVITY_COLUMN_COMPLETED'), 'sort' => 'COMPLETED', 'default' => true, 'prevent_default' => false, 'editable' => array('items' => array('N' => GetMessage('YNSIR_ACTIVITY_STATUS_NOT_COMPLETED'), 'Y' => GetMessage('YNSIR_ACTIVITY_STATUS_COMPLETED'))));
$arResult['HEADERS'][] = array('id' => 'CREATED', 'type'=> 'date', 'name' => GetMessage('YNSIR_ACTIVITY_COLUMN_CREATED'), 'sort' => 'CREATED', 'default' => false, 'editable' => false, 'class' => 'date');

$arResult['FILTER'] = array();
$arResult['FILTER_PRESETS'] = array();

$typeListItems = array(
	strval(YNSIRActivityType::Meeting) => YNSIRActivityType::ResolveDescription(YNSIRActivityType::Meeting),
	strval(YNSIRActivityType::Call).'.'.strval(YNSIRActivityDirection::Incoming) => GetMessage('YNSIR_ACTIVITY_INCOMING_CALL'),
	strval(YNSIRActivityType::Call).'.'.strval(YNSIRActivityDirection::Outgoing) => GetMessage('YNSIR_ACTIVITY_OUTGOING_CALL'),
	strval(YNSIRActivityType::Task) => YNSIRActivityType::ResolveDescription(YNSIRActivityType::Task),
	strval(YNSIRActivityType::Email).'.'.strval(YNSIRActivityDirection::Incoming) => GetMessage('YNSIR_ACTIVITY_INCOMING_EMAIL'),
	strval(YNSIRActivityType::Email).'.'.strval(YNSIRActivityDirection::Outgoing) => GetMessage('YNSIR_ACTIVITY_OUTGOING_EMAIL')
);

$providers = YNSIRActivity::GetProviders();
...
$arResult['FILTER'] = array(
	...
	array('id' => "{$filterFieldPrefix}COMPLETED", 'name' => GetMessage('YNSIR_ACTIVITY_FILTER_COMPLETED'), 'type'=> 'list', 'items'=> array('Y' => GetMessage('YNSIR_ACTIVITY_FILTER_ITEM_COMPLETED'), 'N' => GetMessage('YNSIR_ACTIVITY_FILTER_ITEM_NOT_COMPLETED')), 'params' => array('multiple' => 'Y'), 'default' => true),
	array('id' => "{$filterFieldPrefix}TYPE_ID", 'name' => GetMessage('YNSIR_ACTIVITY_FILTER_TYPE_ID'), 'type'=> 'list', 'items'=> $typeListItems, 'params' => array('multiple' => 'Y'), 'default' => true),
	array('id' => "{$filterFieldPrefix}PRIORITY", 'name' => GetMessage('YNSIR_ACTIVITY_FILTER_PRIORITY'), 'type'=> 'list', 'items'=> YNSIRActivityPriority::PrepareFilterItems(), 'params' => array('multiple' => 'Y'), 'default' => true),
	array(
		'id' => "{$filterFieldPrefix}RESPONSIBLE_ID",
		'name' => GetMessage('YNSIR_ACTIVITY_FILTER_RESPONSIBLE'),
		'default' => true,
		'type' => 'custom_entity',
		'selector' => array(
			'TYPE' => 'user',
	...
	array('id' => "{$filterFieldPrefix}START",  'name' => GetMessage('YNSIR_ACTIVITY_FILTER_START'), 'default' => false, 'type' => 'date'),
	array('id' => "{$filterFieldPrefix}END",  'name' => GetMessage('YNSIR_ACTIVITY_FILTER_END_2'), 'default' => false, 'type' => 'date'),
	array('id' => "{$filterFieldPrefix}DEADLINE",  'name' => GetMessage('YNSIR_ACTIVITY_FILTER_DEADLINE'), 'default' => true, 'type' => 'date'),
	array('id' => "{$filterFieldPrefix}CREATED",  'name' => GetMessage('YNSIR_ACTIVITY_FILTER_CREATED'), 'default' => true, 'type' => 'date')
);

$arResult['FILTER_PRESETS'] = array(
	'not_completed' => array(
		'name' => GetMessage('YNSIR_PRESET_NOT_COMPLETED'),
		'default' => true,
		'fields' => array(
	...
	'completed' => array(
		'name' => GetMessage('YNSIR_PRESET_COMPLETED'),
		'fields' => array(
	...
	'not_completed_all' => array(
		'name' => GetMessage('YNSIR_PRESET_NOT_COMPLETED_ALL'),
		'fields' => array(
			"{$filterFieldPrefix}COMPLETED" => array('selN' => 'N')
		)
	),
	'completed_all' => array(
		'name' => GetMessage('YNSIR_PRESET_COMPLETED_ALL'),
		'fields' => array(
			"{$filterFieldPrefix}COMPLETED" => array('selY' => 'Y')
		)
	)
...
foreach ($arFilter as $k => $v)
{
	...
	if($k === 'REFERENCE' || $k === 'CLIENT')
	{
		...
		{
			$ownerTypeID = YNSIROwnerType::ResolveID($ownerData[0]);
...
if ($permissionType === 'WRITE' && $actionData['ACTIVE'] && $actionData['METHOD'] == 'POST')
{
	...
	if ($actionName === 'delete')
	{
		...
		if(is_object($dbResult))
		{
			...
			{
				if(YNSIRActivity::CheckItemDeletePermission($arActivity, $currentUserPermissions))
				{
					YNSIRActivity::Delete($arActivity['ID']);
				}
			}
		}
	}
	elseif($actionName === 'edit')
	{
		if(isset($actionData['FIELDS']) && is_array($actionData['FIELDS']))
		{
			...
			foreach($actionData['FIELDS'] as $ID => $arSrcData)
			{
				...
				$dbActivity = YNSIRActivity::GetList(array(), array('=ID' => $ID), false, false, array('TYPE_ID'));
				$arActivity = $dbActivity ? $dbActivity->Fetch() : null;
				if(!(is_array($arActivity) && isset($arActivity['TYPE_ID']) && (int)$arActivity['TYPE_ID'] !== YNSIRActivityType::Email))
				{
					continue;
				}

				if(!YNSIRActivity::CheckItemUpdatePermission($arActivity, $currentUserPermissions))
				{
					continue;
				}
				...
				{
					YNSIRActivity::Update($ID, $arUpdateData);
				}
			}
		}
	}
	elseif($actionName === 'mark_as_completed' || $actionName === 'mark_as_not_completed')
	{
		...
		if($forAll)
		{
			...
			while($arActivity = $dbResult->Fetch())
			{
				...
				if(!YNSIRActivity::CheckCompletePermission(
				...
				YNSIRActivity::Update($arActivity['ID'], $arActivity);
			}
		}
		elseif(isset($actionData['ID']) && !empty($actionData['ID']))
		{
		...
		if(isset($actionData['RESPONSIBLE_ID']) && $actionData['RESPONSIBLE_ID'] > 0)
		{
			...
			if(is_object($dbResult))
			{
				while($arItem = $dbResult->Fetch())
				{
					...
					if(YNSIRActivity::CheckItemUpdatePermission($arItem, $currentUserPermissions))
					{
						YNSIRActivity::Update($arItem['ID'], array('RESPONSIBLE_ID' => $responsibleID));
					}
				}
			}
		}
...
elseif($pageSize > 0 && isset($_REQUEST['page']))
{
	...
	if($pageNum < 0)
	{
		//Backward mode
		$offset = -($pageNum + 1);
		$total = YNSIRActivity::GetList(array(), $arFilter, array());
...
else
{
	if(!$arResult['IS_INTERNAL']
		...
		&& isset($_SESSION['YNSIRECRUITMENT_PAGINATION_DATA'])
...
$dbRes = YNSIRActivity::GetList($arSort, $arFilter, false, false, array(), $arOptions);
...
while($arRes = $dbRes->GetNext())
{
	...
	else
	{
		...
		{
			$arRes['CAN_EDIT'] = YNSIRActivity::CheckUpdatePermission($ownerTypeID, $ownerID, $currentUserPermissions);
			$arRes['CAN_COMPLETE'] = (int)$arRes['~TYPE_ID'] !== YNSIRActivityType::Task
				? $arRes['CAN_EDIT']
				: YNSIRActivity::CheckCompletePermission(
					$ownerTypeID,
					$ownerID,
					$currentUserPermissions,
					array('FIELDS' => $arRes)
				);
			$arRes['CAN_DELETE'] = YNSIRActivity::CheckDeletePermission($ownerTypeID, $ownerID, $currentUserPermissions);
		}
		else
		{
	...
	$storageTypeID = isset($arRes['STORAGE_TYPE_ID']) ? intval($arRes['STORAGE_TYPE_ID']) : YNSIRActivityStorageType::Undefined;
	if($storageTypeID === YNSIRActivityStorageType::Undefined || !YNSIRActivityStorageType::IsDefined($storageTypeID))
	{
		$storageTypeID = $arRes['STORAGE_TYPE_ID'] = $arRes['~STORAGE_TYPE_ID'] = YNSIRActivity::GetDefaultStorageTypeID();
	}
	...
	if(!$skipFiles)
	{
		YNSIRActivity::PrepareStorageElementIDs($arRes);
		YNSIRActivity::PrepareStorageElementInfo($arRes);
	}

	//$arRes['SETTINGS'] = (isset($arRes['~SETTINGS']) && $arRes['~SETTINGS'] !== '') ? unserialize($arRes['~SETTINGS']) : array();
	//Lazy communications loading
	//$arRes['COMMUNICATIONS'] = YNSIRActivity::GetCommunications($itemID);
	$arRes['COMMUNICATIONS_LOADED'] = false;

	$description = isset($arRes['~DESCRIPTION']) ? $arRes['~DESCRIPTION'] : '';
	$descriptionType = isset($arRes['DESCRIPTION_TYPE']) ? intval($arRes['DESCRIPTION_TYPE']) : YNSIRContentType::PlainText;

	if($descriptionType === YNSIRContentType::BBCode)
	{
	...
	elseif($descriptionType === YNSIRContentType::Html)
	{
	...
	else//YNSIRContentType::PlainText and other
	{
		$arRes['DESCRIPTION_BBCODE'] = '';
	...
	if($ownerID > 0 && ($ownerTypeID === YNSIROwnerType::Candidate || $ownerTypeID === YNSIROwnerType::Order))
	{
...
{
//    YNSIROwnerType::PrepareEntityInfoBatch($ownerTypeID, $ownerInfos, false);
    $ownerInfos = array (
        10 =>
            array (
                'TITLE' => 'Candidate1',
                ...
                'RESPONSIBLE_ID' => 1,
                'IMAGE_FILE_ID' => 0,
                'SHOW_URL' => '/recruitment/candidate/detail/1/',
            ),
        9 =>
            array (
                'TITLE' => 'Candidate2',
                ...
                'RESPONSIBLE_ID' => 1,
                'IMAGE_FILE_ID' => 0,
                'SHOW_URL' => '/recruitment/candidate/detail/2/',
            ),
    );
...
//{
//	$clientInfos = YNSIRActivity::PrepareClientInfos(array_keys($items));
//
...
$arResult['ENABLE_EMAIL_ADD'] = true;//!$arResult['READ_ONLY'] && IsModuleInstalled('subscribe');
...
if($arResult['TAB_ID'] === '')
{
	if(COption::GetOptionString('ynsirecruitment', '~YNSIR_REBUILD_ACTIVITY_SEARCH_CONTENT', 'N') === 'Y')
	{
	...
	if(YNSIRPerms_::IsAdmin())
	{
		...
		if(COption::GetOptionString('ynsirecruitment', '~YNSIR_ACTIVITY_LIST_CONVERTING_CALENDAR_EVENTS', 'N') !== 'Y')
		{
			...
			{
				COption::SetOptionString('ynsirecruitment', '~YNSIR_ACTIVITY_LIST_CONVERTING_CALENDAR_EVENTS', 'Y');
			}
		}

		//Converting existing tasks
		if(COption::GetOptionString('ynsirecruitment', '~YNSIR_ACTIVITY_LIST_CONVERTING_OF_TASKS', 'N') !== 'Y')
		{
			...
			{
				COption::SetOptionString('ynsirecruitment', '~YNSIR_ACTIVITY_LIST_CONVERTING_OF_TASKS', 'Y');
			}
		}
	}
}
