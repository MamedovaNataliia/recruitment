...
<form action="<?=POST_FORM_ACTION_URI?>" name="crmPermForm" method="POST">
	...
	<tr>
		<td  valign="top" style="min-width:432px">
			<table width="100%" cellpadding="0" cellspacing="0" class="crmPermTable" id="crmPermTable" >
				<tr>
					<th>&nbsp;</th>
					<th><?=GetMessage("YNSIR_PERMS_PERM_ROLE")?></th>
				</tr>
				...
				<tr data-roleId="<?=$arRelation['RELATION']?>">
					...
					<td class="last-child">
						...
						<a href="javascript:void(0)" onclick="CrmPermRemoveRow(this.parentNode.parentNode); return false;"  class="crmPermA crmPermADelete" title="<?=GetMessage("YNSIR_PERMS_PERM_DELETE")?>"></a>
					</td>
				</tr>
				<? endforeach; ?>
				...
				<tr  class="AddPerm">
					<td colspan="2"><a name="crmUserSelect" href="javascript:void(0)" onclick="CrmSelectEntity(); return false" ><?=GetMessage("YNSIR_PERMS_PERM_ADD")?></a></td>
				</tr>
			</table>
		</td>
		<td style="padding-left:15px; min-width:192px;"  valign="top">
			<table width="100%" cellpadding="0" cellspacing="0" class="crmRoleTable" >
				<tr>
					<th><?=GetMessage("YNSIR_PERMS_ROLE_LIST")?>:</th>
				</tr>
				<tr>
					<td>
						<? foreach ($arResult['ROLE'] as $arRole): ?>
							<a href="<?=$arRole['PATH_TO_DELETE']?>" style="float:right"  title="<?=GetMessage("YNSIR_PERMS_ROLE_DELETE")?>" class="crmPermA crmPermADelete" onclick="CrmRoleDelete('<?=CUtil::JSEscape(GetMessage('YNSIR_PERMS_DLG_TITLE'))?>', '<?=CUtil::JSEscape(GetMessage('YNSIR_PERMS_DLG_MESSAGE'))?>', '<?=CUtil::JSEscape(GetMessage('YNSIR_PERMS_DLG_BTN'))?>', '<?=CUtil::JSEscape($arRole['PATH_TO_DELETE'])?>'); return false;"></a>
							<a href="<?=$arRole['PATH_TO_EDIT']?>" style="float:right" class="crmPermA crmPermAEdit" title="<?=GetMessage("YNSIR_PERMS_ROLE_EDIT")?>"></a>
							<div style="padding-bottom: 4px" algin="left">- <?=$arRole['NAME']?></div>
							<div style="clear:both"></div>
						<? endforeach; ?>
						<div class="crmRole" style="padding-left:10px"><a href="<?=$arResult['PATH_TO_ROLE_ADD']?>"><?=GetMessage("YNSIR_PERMS_ROLE_ADD")?></a></div>
					</td>
				</tr>
			</table>
		</td>
	...
	<div id="crmPermButtonBoxPlace">
		<input type="submit" value="<?=GetMessage('YNSIR_PERMS_BUTTONS_SAVE');?>">
	</div>
</form>
