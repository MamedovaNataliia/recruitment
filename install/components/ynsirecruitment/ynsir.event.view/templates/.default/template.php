...
$prefix = $arResult['GRID_ID'];
	...
	foreach($arResult['EVENT'] as $arEvent)
	{
		...
		{
			$arEvent['FILE_HTML'] = '<div class="event-detail-files"><label class="event-detail-files-title">'.GetMessage('YNSIR_EVENT_TABLE_FILES').':</label><div class="event-detail-files-list">';
		...
		if ($b_AllowDel && ($arEvent['EVENT_TYPE'] != YNSIREvent::TYPE_DELETE
			|| ($arEvent['EVENT_TYPE'] == YNSIREvent::TYPE_USER && $USER->isAdmin())))
		{
			$pathToRemove = CUtil::JSEscape($arEvent['PATH_TO_DELETE']);
			$arActions[] =  array(
				'TITLE' => GetMessage('YNSIR_EVENT_DELETE_TITLE'),
				'TEXT' => GetMessage('YNSIR_EVENT_DELETE'),
        ...
        elseif ($arEvent['EVENT_TYPE'] == YNSIREvent::TYPE_MEETING)
            $eventColor = 'color: #9c8000';
        elseif ($arEvent['EVENT_TYPE'] == YNSIREvent::TYPE_ASSOCIATE)
            $eventColor = 'color: #12af12';
			...
			$authorHtml = "<div class = \"crm-client-summary-wrapper\">
				<div class = \"crm-client-photo-wrapper\">
					<div class=\"crm-client-def-pic\">
						<img alt=\"Author Photo\" src=\"{$arEvent['CREATED_BY_PHOTO_URL']}\"/>
					</div>
				</div>
		...
		if ($arResult['EVENT_ENTITY_LINK'] == 'Y')
		{
			$arColumns['ENTITY_TYPE'] = !empty($arEvent['ENTITY_TYPE'])
				? YNSIROwnerType::GetDescription(YNSIROwnerType::ResolveID($arEvent['ENTITY_TYPE'])) : '';
		...
		array(
			...
			'EXTENSION' => array(
				...
				'CONFIG' => array(
					...
					'serviceUrl' => '/bitrix/components/ynsirecruitment/ynsir.event.view/list.ajax.php?siteID='.SITE_ID.'&'.bitrix_sessid_get(),
					'loaderData' => isset($arParams['AJAX_LOADER']) ? $arParams['AJAX_LOADER'] : null
				),
				'MESSAGES' => array(
					'deletionDialogTitle' => GetMessage('YNSIR_EVENT_DELETE_TITLE'),
					'deletionDialogMessage' => GetMessage('YNSIR_EVENT_DELETE_CONFIRM'),
					'deletionDialogButtonTitle' => GetMessage('YNSIR_EVENT_DELETE')
				)
			),
		),
		$component
...
?>
<div class="crm_notice_message"><?=GetMessage('YNSIR_IMPORT_EVENT', Array('%EMAIL%' => COption::GetOptionString('crm', 'mail', '')));?></div>
<?endif;?>
...
<script type="text/javascript">
	BX.ready(
		function()
		{
			BX.YNSIREventListManager.messages =
			{
				deletionConfirmationDlgTitle: "<?=GetMessageJS('YNSIR_EVENT_DELETE_TITLE')?>",
				deletionConfirmationDlgContent: "<?=GetMessageJS('YNSIR_EVENT_DELETE_CONFIRM')?>",
				deletionConfirmationDlgBtn: "<?=GetMessageJS('YNSIR_EVENT_DELETE')?>"
			};

			BX.YNSIREventListManager.create("<?=CUtil::JSEscape($managerID)?>",
				{
