...
$APPLICATION->SetTitle('Job Order Status');
...
$waitingFields = $arResult['WAITING_FIELDS'];
$closedFields = $arResult['CLOSED_FIELDS'];
    ...
    <div id="crm-container" class="crm-container">
        ...
        <div id="status_box" class="crm-transaction-menu">
            <? foreach ($arResult['HEADERS'] as $entityId => $headerName): ?>
                <?$entityId = $entityId == 'JO_STATUS'?'ORDER_JOB_STATUS':$entityId;?>
                <? $headerName = htmlspecialcharsbx($headerName); ?>
                <? $tabActive = (strtolower($entityId) == $arParams['entity']) ? 'status_tab_active' : '' ?>
                <a href="/recruitment/config/lists/<?=strtolower($entityId)?>/" id="status_tab_<?= $entityId ?>" class="status_tab <?= $tabActive ?>"
                    title="<?= $headerName ?>">
        ...
        <div class="crm-transaction-stage">
                    ...
                    <div id="content_<?= $entityId ?>"
                        ...
                        <div class="transaction-stage transaction-initial-stage">
                            ...
                            <div class="transaction-stage-phase" data-sort="<?= $initialFields[$entityId]['SORT'] ?>"
                                 id="field-phase-<?= $initialFields[$entityId]['ID'] ?>" data-calculate="1"
                                 data-success="1"
                                ...
                                <span id="transaction-stage-phase-icon"
                                      class="<?= $iconClass ?> transaction-stage-phase-icon"
							...
							<span id="field-title-inner-<?= $initialFields[$entityId]['ID'] ?>"
                                  class="transaction-stage-phase-title-inner">
								<span id="field-name-<?= $initialFields[$entityId]['ID'] ?>"
                        ...
                        <div class="transaction-stage transaction-initial-stage">
                            ...
                            $iconClass = '';
                            $blockClass = '';
                            $colorValue = $waitingFields[$entityId]['COLOR'] ? $waitingFields[$entityId]['COLOR'] : '#ACE9FB';
                            $style = 'background:' . $colorValue . ';';
                            $style .= 'color:' . getColorText($colorValue, $iconClass, $blockClass) . ';';
                            ?>

                            <div class="transaction-stage-title"><?= GetMessage('CRM_STATUS_TITLE_WAITING_' . $entityId) ?></div>
                            <div class="transaction-stage-phase" data-sort="<?= $waitingFields[$entityId]['SORT'] ?>"
                                 id="field-phase-<?= $waitingFields[$entityId]['ID'] ?>" data-calculate="1"
                                 data-success="1"
                                 ondblclick="BX['<?= $jsClass ?>'].editField('<?= $waitingFields[$entityId]['ID'] ?>');"
                                 style="<?= $style ?>">
                                <div id="phase-panel" data-class="transaction-stage-phase-panel"
                                     class="<?= $blockClass ?> transaction-stage-phase-panel">
                                    <? if ($waitingFields[$entityId]['SYSTEM'] == 'Y' &&
                                        !empty($waitingFields[$entityId]['NAME_INIT'])): ?>
                                        <div onclick="BX['<?= $jsClass ?>'].recoveryName('<?= $waitingFields[$entityId]['ID'] ?>',
                                                '<?= htmlspecialcharsbx($waitingFields[$entityId]['NAME_INIT']) ?>')"
                                             class="transaction-stage-phase-panel-button
									transaction-stage-phase-panel-button-refresh"
                                             title="<?= GetMessage('CRM_STATUS_LIST_RECOVERY_NAME') ?>"></div>
                                    <? endif ?>
							...
							<span class="transaction-stage-phase-icon-arrow"></span>
                                ...
                                <span id="phase-panel" data-class="transaction-stage-phase-title"
                                      class="<?= $blockClass ?> transaction-stage-phase-title">
							<span id="field-title-inner-<?= $waitingFields[$entityId]['ID'] ?>"
                                  class="transaction-stage-phase-title-inner">
								<span id="field-name-<?= $waitingFields[$entityId]['ID'] ?>"
                                      class="transaction-stage-phase-name">
									<?= $waitingFields[$entityId]['NUMBER'] ?>.
                                    <?= htmlspecialcharsbx($waitingFields[$entityId]['NAME']) ?>
								</span>
								<span onclick="BX['<?= $jsClass ?>'].editField('<?= $waitingFields[$entityId]['ID'] ?>');"
                                      title="<?= GetMessage('CRM_STATUS_EDIT_NAME') ?>"
                                      class="transaction-stage-phase-icon-edit"></span>
							</span>
						</span>

                                <input type="hidden" id="field-number-<?= $waitingFields[$entityId]['ID'] ?>"
                                       value="<?= $waitingFields[$entityId]['NUMBER'] ?>">
                                <input type="hidden"
                                       name="LIST[<?= $entityId ?>][<?= $waitingFields[$entityId]['ID'] ?>][SORT]"
                                       id="field-sort-<?= $waitingFields[$entityId]['ID'] ?>"
                                       value="<?= $waitingFields[$entityId]['SORT'] ?>">
                                <input type="hidden"
                                       name="LIST[<?= $entityId ?>][<?= $waitingFields[$entityId]['ID'] ?>][VALUE]"
                                       id="field-hidden-name-<?= $waitingFields[$entityId]['ID'] ?>"
                                       value="<?= htmlspecialcharsbx($waitingFields[$entityId]['NAME']) ?>">
                                <input type="hidden"
                                       name="LIST[<?= $entityId ?>][<?= $waitingFields[$entityId]['ID'] ?>][COLOR]"
                                       id="stage-color-<?= $waitingFields[$entityId]['ID'] ?>"
                                       value="<?= htmlspecialcharsbx($colorValue) ?>">
                                <input type="hidden"
                                       name="LIST[<?= $entityId ?>][<?= $waitingFields[$entityId]['ID'] ?>][STATUS_ID]"
                                       id="stage-status-id-<?= $waitingFields[$entityId]['ID'] ?>" data-status-id="1"
                                       value="<?= htmlspecialcharsbx($waitingFields[$entityId]['STATUS_ID']) ?>">
                            </div>
                        </div>
                        <!--FINAL-->
                        <div class="transaction-stage transaction-initial-stage--">
                            <div class="transaction-stage-final">
                                <div class="transaction-stage-final-title">
                                    <span class="transaction-stage-final-title-sub">Request</span>
                                </div>
                                <div class="transaction-stage-final-result">
                                    <div class="transaction-stage-final-failure">Reject</div>
                                    <div class="transaction-stage-final-success">Approved</div>
                                </div>
                                ...
                                <div class="transaction-stage-final-column">
                                    ...
                                    <div id="final-success-storage-<?= $entityId ?>"
                                         class="transaction-stage transaction-stage-success1">
                                        ...
                                        <div ondblclick="BX['<?= $jsClass ?>'].editField('<?= $finalFields[$entityId]['UNSUCCESSFUL']['ID'] ?>');"
                                             class="transaction-stage-phase"
                                             data-sort="<?= $finalFields[$entityId]['UNSUCCESSFUL']['SORT'] ?>"
                                             id="field-phase-<?= $finalFields[$entityId]['UNSUCCESSFUL']['ID'] ?>"
                                             data-calculate="1"
                                             style="<?= $style ?>" data-success="1">
                                            <div id="phase-panel" data-class="transaction-stage-phase-panel"
                                                 class="<?= $blockClass ?> transaction-stage-phase-panel">
									...
									<span class="transaction-stage-phase-icon-arrow"></span>
                                            ...
                                            <span id="phase-panel" data-class="transaction-stage-phase-title"
                                                  class="<?= $blockClass ?> transaction-stage-phase-title">
									<span id="field-title-inner-<?= $finalFields[$entityId]['UNSUCCESSFUL']['ID'] ?>"
                                          class="transaction-stage-phase-title-inner">
										<span id="field-name-<?= $finalFields[$entityId]['UNSUCCESSFUL']['ID'] ?>"
                                              class="transaction-stage-phase-name">
										...
										<span onclick="BX['<?= $jsClass ?>'].editField('<?= $finalFields[$entityId]['UNSUCCESSFUL']['ID'] ?>');"
                                            ...
                                            <input type="hidden"
                                                   id="field-number-<?= $finalFields[$entityId]['UNSUCCESSFUL']['ID'] ?>"
                                                   value="<?= $finalFields[$entityId]['UNSUCCESSFUL']['NUMBER'] ?>">
                                            <input type="hidden"
                                                   name="LIST[<?= $entityId ?>][<?= $finalFields[$entityId]['UNSUCCESSFUL']['ID'] ?>][SORT]"
                                                   id="field-sort-<?= $finalFields[$entityId]['UNSUCCESSFUL']['ID'] ?>"
                                                   value="<?= $finalFields[$entityId]['UNSUCCESSFUL']['SORT'] ?>">
                                            <input type="hidden"
                                                   name="LIST[<?= $entityId ?>][<?= $finalFields[$entityId]['UNSUCCESSFUL']['ID'] ?>][VALUE]"
                                                   id="field-hidden-name-<?= $finalFields[$entityId]['UNSUCCESSFUL']['ID'] ?>"
                                                   value="<?= htmlspecialcharsbx($finalFields[$entityId]['UNSUCCESSFUL']['NAME']) ?>">
                                            <input type="hidden"
                                                   name="LIST[<?= $entityId ?>][<?= $finalFields[$entityId]['UNSUCCESSFUL']['ID'] ?>][COLOR]"
                                                   id="stage-color-<?= $finalFields[$entityId]['UNSUCCESSFUL']['ID'] ?>"
                                                   value="<?= htmlspecialcharsbx($colorValue) ?>">
                                            <input type="hidden"
                                                   name="LIST[<?= $entityId ?>][<?= $finalFields[$entityId]['UNSUCCESSFUL']['ID'] ?>][STATUS_ID]"
                                                   id="stage-status-id-<?= $finalFields[$entityId]['UNSUCCESSFUL']['ID'] ?>"
                                                   data-status-id="1"
                                ...
                                <div class="transaction-stage-final-column">
                                    ...
                                    <div id="final-storage-<?= $entityId ?>"
                                         class="transaction-stage transaction-stage-succness droppable">
                                        ...
                                        <div ondblclick="BX['<?= $jsClass ?>'].editField('<?= $finalFields[$entityId]['SUCCESSFUL']['ID'] ?>');"
                                             class="transaction-stage-phase"
                                             data-sort="<?= $finalFields[$entityId]['SUCCESSFUL']['SORT'] ?>"
                                             id="field-phase-<?= $finalFields[$entityId]['SUCCESSFUL']['ID'] ?>"
                                             data-calculate="1"
                                             style="<?= $style ?>" data-success="0">
                                            <div id="phase-panel" data-class="transaction-stage-phase-panel"
                                                 class="<?= $blockClass ?> transaction-stage-phase-panel">
									...
									<span id="field-title-inner-<?= $finalFields[$entityId]['SUCCESSFUL']['ID'] ?>"
										...
										<span onclick="BX['<?= $jsClass ?>'].editField('<?= $finalFields[$entityId]['SUCCESSFUL']['ID'] ?>');"
                                            ...
                                            <input type="hidden"
                                                   id="field-number-<?= $finalFields[$entityId]['SUCCESSFUL']['ID'] ?>"
                                                   value="<?= $finalFields[$entityId]['SUCCESSFUL']['NUMBER'] ?>">
                                            <input type="hidden"
                                                   name="LIST[<?= $entityId ?>][<?= $finalFields[$entityId]['SUCCESSFUL']['ID'] ?>][SORT]"
                                                   id="field-sort-<?= $finalFields[$entityId]['SUCCESSFUL']['ID'] ?>"
                                                   value="<?= $finalFields[$entityId]['SUCCESSFUL']['SORT'] ?>">
                                            <input type="hidden"
                                                   name="LIST[<?= $entityId ?>][<?= $finalFields[$entityId]['SUCCESSFUL']['ID'] ?>][VALUE]"
                                                   id="field-hidden-name-<?= $finalFields[$entityId]['SUCCESSFUL']['ID'] ?>"
                                                   value="<?= htmlspecialcharsbx($finalFields[$entityId]['SUCCESSFUL']['NAME']) ?>">
                                            <input type="hidden"
                                                   name="LIST[<?= $entityId ?>][<?= $finalFields[$entityId]['SUCCESSFUL']['ID'] ?>][COLOR]"
                                                   id="stage-color-<?= $finalFields[$entityId]['SUCCESSFUL']['ID'] ?>"
                                                   value="<?= htmlspecialcharsbx($colorValue) ?>">
                                            <input type="hidden"
                                                   name="LIST[<?= $entityId ?>][<?= $finalFields[$entityId]['SUCCESSFUL']['ID'] ?>][STATUS_ID]"
                                                   id="stage-status-id-<?= $finalFields[$entityId]['SUCCESSFUL']['ID'] ?>"
                                                   data-status-id="1"
                                                   value="<?= htmlspecialcharsbx($finalFields[$entityId]['SUCCESSFUL']['STATUS_ID']) ?>">
                                        </div>
                                        <!--                                        <div class="transaction-stage-approval"></div>-->
										...
										<span class="transaction-stage-phase-icon-burger"></span>
                                                ...
                                                <span id="phase-panel" data-class="transaction-stage-phase-title"
                                                      class="<?= $blockClass ?> transaction-stage-phase-title">
                                        <span id="field-title-inner-<?= $field['ID'] ?>"
                                              class="transaction-stage-phase-title-inner">
                                            <span id="field-name-<?= $field['ID'] ?>"
                                                  class="transaction-stage-phase-name">
                                                <?= $field['NUMBER'] ?>.
                                            ...
                                            <span onclick="BX['<?= $jsClass ?>'].editField('<?= $field['ID'] ?>');"
                                                ...
                                                <input type="hidden" id="field-number-<?= $field['ID'] ?>"
                                                       value="<?= $field['NUMBER'] ?>">
                                                <input type="hidden"
                                                       name="LIST[<?= $entityId ?>][<?= $field['ID'] ?>][SORT]"
                                                       id="field-sort-<?= $field['ID'] ?>"
                                                       value="<?= $field['SORT'] ?>">
                                                <input type="hidden"
                                                       name="LIST[<?= $entityId ?>][<?= $field['ID'] ?>][VALUE]"
                                                       id="field-hidden-name-<?= $field['ID'] ?>"
                                                       value="<?= htmlspecialcharsbx($field['NAME']) ?>">
                                                <input type="hidden"
                                                       name="LIST[<?= $entityId ?>][<?= $field['ID'] ?>][COLOR]"
                                                       id="stage-color-<?= $field['ID'] ?>"
                                                       value="<?= htmlspecialcharsbx($colorValue) ?>">
                                                <input type="hidden"
                                                       name="LIST[<?= $entityId ?>][<?= $field['ID'] ?>][STATUS_ID]"
                                        ...
                                        <? foreach ($closedFields as $field): ?>
                                            ...
                                            $blockClass = '';
                                            $iconClass = '';
                                            $colorValue = $field['COLOR'] ? $field['COLOR'] : '#DBF199';
                                            $style = 'background:' . $colorValue . ';';
                                            $style .= 'color:' . getColorText($colorValue, $iconClass, $blockClass) . ';';
                                            ?>
                                            ...
                                            <div class="transaction-stage-phase draghandle" data-calculate="1"
                                                 data-success="1"
                                                 style="<?= $style ?>" id="field-phase-<?= $field['ID'] ?>"
                                                 data-space="<?= $field['ID'] ?>"
                        ...
                        <div id="extra-storage-<?= $entityId ?>" class="transaction-stage droppable">
                            ...
                            <? foreach ($arResult["ROWS"][$entityId] as $field): ?>
                                ...
                                <div class="transaction-stage-phase draghandle" data-calculate="1" data-success="1"
                                     ...
                                     id="field-phase-<?= $field['ID'] ?>" data-space="<?= $field['ID'] ?>"
                                     data-sort="<?= $field['SORT'] ?>"
                                     ondblclick="BX['<?= $jsClass ?>'].editField('<?= $field['ID'] ?>');">
			...
			<span class="crm-fixedbtn <? if ($blockFixed): ?>crm-fixedbtn-pin<? endif ?>"
                ...
                <input type="button" value="<?= GetMessage('CRM_STATUS_BUTTONS_CANCEL'); ?>"
                       ...
                       onclick="BX['<?= $jsClass ?>'].statusReset()">
            </div>
        </div>
...
<script type="text/javascript">
    BX.ready(function () {
        ...
        <?endif;?>
        <?if(isset($entitySettings['DELETION_CONFIRMATION'])):?>
        BX.message({CRM_STATUS_DELETE_FIELD_QUESTION_<?=$entityId?>: "<?=CUtil::JSEscape($entitySettings['DELETION_CONFIRMATION'])?>"});
        <?endif;?>
        <?endforeach;?>
