...
if($arResult['ACTIVE_TAB'] === '')
{
    ...
    $arResult['ACTIVE_ENTITY_ID'] = 'JO_STATUS';
}
    ...
    isset($_POST['ACTION']) && $_POST['ACTION'] == 'save')
{
    ...
    LocalRedirect('/recruitment/config/lists/order_job_status/');
}
...
foreach ($arTypeConfig as $entityId=>$arEntityType){
    if($entityId == 'ORDER_JOB_STATUS'){
        $arResult['HEADERS']['JO_STATUS']  = $arEntityType['NAME'];
        continue;
    }
    $arResult['HEADERS'][$entityId] = $arEntityType['NAME'];
}
foreach(JOStatus::GetEntityTypes('JO_STATUS') as $entityId => $arEntityType)
{
//    $arResult['HEADERS'][$entityId] = $arEntityType['NAME'];
    ...
    if(isset($arEntityType['SEMANTIC_INFO']) && is_array($arEntityType['SEMANTIC_INFO']))
    {
        ...
        $defaultName = GetMessage("CRM_STATUS_DEFAULT_NAME_JO_STATUS");
...
{
    if($status['ENTITY_ID'] != 'JO_STATUS') continue;
...
foreach($arResult['ENTITY'] as $entityId => $dataEntity)
{
    ...
    $arResult['CLOSED_FIELDS'][$entityId] = array();
...
//			$arResult['FINAL_FIELDS'][$entityId]['CLOSED'] = $status;
            $arResult['CLOSED_FIELDS'][$entityId] = $status;
        }
