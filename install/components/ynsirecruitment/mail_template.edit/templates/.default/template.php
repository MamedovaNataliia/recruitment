...
?>
    <div class="pagetitle-container pagetitle-align-right-container">
        <a href="/recruitment/config/mailtemplate/" class="ynsir-candidate-detail-back">
            <?= GetMessage('YNSIR_MAIL_BTN_BACK') ?>
        </a>
    </div>
<?
    $this->EndViewTarget();
        ...
        BX.ready(
            function () {
                BX.YNSIREntityFieldSelector.messages =
                    {
                        "buttonAdd": "<?=GetMessageJS('YNSIR_MAIL_TEMPLATE_ENTITY_FIELD_SELECTOR_ADD_BTN')?>"
                    }
            }
        );
    </script><?
...
foreach ($mainTabData as &$data) {
    ...
    if ($ID === 'ENTITY_TYPE_ID') {
        ...
        $selector = array(
            ...
            'name' => GetMessage('YNSIR_MAIL_TEMPLATE_ENTITY_FIELD_SELECTOR'),
            'type' => 'custom',
        ...
        <script type="text/javascript">
            BX.ready(
                function () {
                    ...
                    var selectorContainer = BX('<?=$selectorContainerID?>');
                    if (BX.type.isElementNode(selectorContainer)) {
                        var selector = BX.YNSIREntityFieldSelector.create(
                            '<?=$selectorContainerID?>',
                            {
                                'editorName': 'oLheMailTemplateBody',
                                'map': <?=CUtil::PhpToJSObject($entity)?>
                            }
                        );
                        ...
                        selector.layout(selectorContainer);
                    }
                }
            );
        </script><?
...
$arTabs[] = array(
    'id' => 'tab_1',
    'name' => GetMessage('YNSIR_MAIL_TEMPLATE_TAB_1'),
    'title' => GetMessage('YNSIR_MAIL_TEMPLATE_TAB_1_TITLE'),
    'icon' => '',
