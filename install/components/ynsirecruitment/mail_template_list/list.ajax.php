...
if ($_REQUEST['MODE'] == 'SEARCH')
{
	...
	if (!empty($search))
	{
		...
		$obRes = YNSIRJobOrder::GetListEx($arOrder, $arFilter, false, array('nTopCount' => $nPageTop), $arSelect);
		$arImages = array();
		$arLargeImages = array();
		...
		while ($arRes = $obRes->Fetch())
		{
			...
			$arData[$i] =
				array(
					...
					'title' => YNSIRJobOrder::PrepareFormattedName(
						array(
...
elseif ($action === 'REBUILD_DUPLICATE_INDEX')
{
	...
	if(!YNSIRJobOrder::CheckUpdatePermission(0))
	{
		__YNSIRContactListEndResonse(array('ERROR' => 'Access denied.'));
	}
	...
	if($totalItemQty <= 0)
	{
		$totalItemQty = YNSIRJobOrder::GetListEx(array(), array('CHECK_PERMISSIONS' => 'N'), array(), false);
	}
	...
	$dbResult = YNSIRJobOrder::GetListEx(
		array('ID' => 'ASC'),
		$filter,
		false,
		array('nTopCount' => 20),
	...
	if($itemQty > 0)
	{
		YNSIRJobOrder::RebuildDuplicateIndex($itemIDs);
...
elseif ($action === 'GET_ROW_COUNT')
{
	...
	$gridData = $_SESSION['YNSIR_MT_PAGINATION_DATA'][$gridID];
...
elseif ($action === 'GET_REQUISITE_PRESETS')
{
	...
	if(!YNSIRJobOrder::CheckReadPermission(0))
	{
		__YNSIRContactListEndResonse(array('ERROR' => 'Access denied.'));
	}
...
elseif ($action === 'BUILD_REQUISITES')
{
	...
	if(!(YNSIRJobOrder::CheckReadPermission(0) && YNSIRJobOrder::CheckUpdatePermission(0)))
	{
		__YNSIRContactListEndResonse(array('ERROR' => 'Access denied.'));
	}
	...
	if($itemQty > 0)
	{
		...
		{
			YNSIRJobOrder::CreateRequisite($itemID, $presetID);
		}
