        ...
        $arResult['PERM'] = $isPermitted;
        {
            ...
            if (is_array($resCache) && !empty($resCache['DATA'] && false)) { // not use cache
                ...
                $dbResultAssociate = YNSIRAssociateJob::GetList(
                    ...
                    false,
                    array(),
                    array('STATUS_ID', 'ORDER_JOB_ID', 'ORDER_JOB_TITLE'));
                if ($arCandidateRes = $dbResultAssociate->GetNext()) {
                    ...
                    $HtmlOrder = '<a href="' . $orderLink . '" title="' . $arCandidateRes['ORDER_JOB_TITLE'] . '">' . $arCandidateRes['ORDER_JOB_TITLE'] . '</a>';
                    $arResult['CANDIDATE']['CANDIDATE_STATUS'] = $arResult['CONFIG'][YNSIRConfig::TL_CANDIDATE_STATUS][$arCandidateRes['STATUS_ID']]['NAME_' . strtoupper(LANGUAGE_ID)] . ' <span>&rarr;</span> ' . $HtmlOrder;
                }
//get List Candidate have aready added to current JobOrder
...
//            $arResult['CANDIDATE']['MODIFIED_BY_PHOTO_URL'] = $arMODIFIBYTOOLTIP['PHOTO_URL'];
            ...
            if ($arCandidateRes = $dbResultAssociate->GetNext()) {
                ...
                $orderLink = CComponentEngine::MakePathFromTemplate($arParams['PATH_TO_ORDER_DETAIL'], array('job_order_id' => $arCandidateRes['ORDER_JOB_ID']));
                $HtmlOrder = '<a href="' . $orderLink . '" title="' . $arCandidateRes['ORDER_JOB_TITLE'] . '">' . $arCandidateRes['ORDER_JOB_TITLE'] . '</a>';
                $arResult['CANDIDATE']['CANDIDATE_STATUS'] = $arResult['CONFIG'][YNSIRConfig::TL_CANDIDATE_STATUS][$arCandidateRes['STATUS_ID']] . ' <span>&rarr;</span> ' . $HtmlOrder;
            }
//CONFIG -->
...
//HISTORY
            $arResult['FIELDS']['tab_event'][] = array(
                ...
                'name' => "Deal log",//GetMessage('YNSIR_SECTION_EVENT_MAIN'),
                'type' => 'section'
            );
