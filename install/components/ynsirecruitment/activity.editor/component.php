	...
	if (is_object($USER) && $USER->isAuthorized() && CModule::includeModule('mail'))
	{
		...
		while ($mailbox = $res->fetch())
		{
			...
			if (!empty($mailbox['OPTIONS']['flags']) && in_array('crm_connect', $mailbox['OPTIONS']['flags']))
			{
				...
				{
					if (strpos($mailbox['LOGIN'], '@') > 0 && empty($arResult['USER_YNSIR_EMAIL']))
						$arResult['USER_YNSIR_EMAIL'] = $mailbox['LOGIN'];
				}
				else
				{
					if (strpos($mailbox['NAME'], '@') > 0 && empty($arResult['SHARED_YNSIR_EMAIL']))
						$arResult['SHARED_YNSIR_EMAIL'] = $mailbox['NAME'];
				}
			}
		}
	}
...
if (empty($arResult['SHARED_YNSIR_EMAIL']))
	$arResult['SHARED_YNSIR_EMAIL'] = trim(\COption::getOptionString('crm', 'mail', ''));
...
$storageTypeID = $arResult['STORAGE_TYPE_ID'] = YNSIRActivity::GetDefaultStorageTypeID();
$arResult['ENABLE_DISK'] = false;// $storageTypeID === StorageType::Disk;
$arResult['ENABLE_WEBDAV'] = false;//$storageTypeID === StorageType::WebDav;
...
{
	$webDavPaths = CYNSIRWebDavHelper::GetPaths();
