...
if (!CModule::IncludeModule('ynsirecruitment') ||!CModule::IncludeModule('crm') || !YNSIRSecurityHelper::IsAuthorized() || !check_bitrix_sessid() || $_SERVER['REQUEST_METHOD'] != 'POST')
{
	die();
}
...
if($action === 'INDEX')
{
	...
	if($entityTypeName !== YNSIROwnerType::OrderName)
	{
		die();
	}
	...
	if($pathToShow === '')
	{
		$pathToShow = COption::GetOptionString('ynsirecruitment', 'path_to_job_order_detail');
	}
	...
	$APPLICATION->IncludeComponent('bitrix:bizproc.document',
		...
		array(
			...
			'DOCUMENT_ID' => YNSIR_JOB_ORDER."_{$entityID}",
